////////////////////////////////////////////////////////////////////
// $Id: inject_header.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
string usage = "Usage: inject_header license_file < input > output", begin = "$BEGIN_BCI2000_LICENSE$",
...
int main(int argc, char *argv[])
{
    ...
    {
        if (line.length() >= begin.length() && line.substr(line.length() - begin.length()) == begin)
        {
            ...
            {
                cerr << "Could not open " << argv[1] << " for input." << endl << usage << endl;
                return -1;
            }
            ...
            while (getline(cin, line) &&
                   (line.length() < end.length() || line.substr(line.length() - end.length()) != end))
                ;
