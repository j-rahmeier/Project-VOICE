////////////////////////////////////////////////////////////////////
// $Id: DylibTool.cpp 7658 2023-10-13 22:24:08Z mellinger $
...
int DylibToolMain(int argc, char **argv)
{
    std::string toolname = FileUtils::ExtractBase(FileUtils::ExecutablePath());
    bool help = false, listexports = false, cdecl_only = false, all_functions = false;
    int createglue = -1;
    ...
    if (help)
    {
        std::cout << toolname << " creates glue code for custom runtime-loading of dynamic libraries.\n"
                  ...
                  << "\nOptions:\n"
                  << "--cdecl-only: Assume the cdecl calling convention for symbols with unmangled names, and ignore "
                     "all others.\n"
                  << "--all-functions: Create imports for all functions, not only those listed in the header file.\n"
    ...
    if (sInteractive)
        std::cout << toolname << ": creates glue code for custom runtime-loading of dynamic libraries" << std::endl;
    ...
    if (!cdecl_only)
    {
        ...
        if (!all_functions)
        {
            for (auto i = fsymbols.begin(); i != fsymbols.end();)
            {
                ...
                {
                    std::cerr << "Warning: Function " << *i << " appears in library but not in header file, omitting"
                        << std::endl;
                    i = fsymbols.erase(i);
                }
                else
            ...
            for (auto i = dsymbols.begin(); i != dsymbols.end();)
            {
                if (!FindSymbolName(*i, headercontent))
                {
                    std::cerr << "Warning: Data symbol " << *i << " appears in library but not in header file, omitting"
                        << std::endl;
                    i = dsymbols.erase(i);
                }
                else
    ...
    for (const Template *tmpl = cdecl_only ? templates + 3 : templates; tmpl->r; ++tmpl)
    {
        ...
        while ((pos = tmplData.find(listtagopen, pos)) != std::string::npos)
        {
            ...
            if (end != std::string::npos)
            {
                ...
                tmplData =
                    tmplData.substr(0, pos) + "$" + listName.str() + "$" + tmplData.substr(end + listtagclose.length());
...
std::string Varexpand(const std::string &s, const std::map<std::string, std::string> &vars)
{
    std::string r = s;
    size_t pos = 0;
...
bool FindSymbolName(std::string &ioName, const std::string &header)
{
    if (ioName.empty())
        return false;
...
bool IsCdeclName(const std::string &name)
{
    if (name.empty())
        return false;
