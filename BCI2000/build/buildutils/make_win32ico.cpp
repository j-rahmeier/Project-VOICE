...
static bool ReadPng(std::istream &stream, file &f)
{
    stream.clear();
    stream.seekg(0);
...
static bool ReadBmp(std::istream &stream, file &f)
{
    stream.clear();
    stream.seekg(0);
    ...
    {
        int lineDataSize = w * info.biBitCount / 8, imageDataSize = lineDataSize * h,
        ...
        union {
            char *c;
            uint8_t *i;
        } pMask = {f.data.data() + imageDataSize};
        for (int x = 0; x < w; ++x)
        {
            for (int y = 0; y < h; ++y)
            {
