////////////////////////////////////////////////////////////////////
// $Id: SchedulingTool.cpp 7520 2023-08-04 17:40:46Z mellinger $
...
struct CollapsingGroup : QGroupBox, QtSignalConnector
{
    ...
    CollapsingGroup(QString s, QWidget *p = 0) : QGroupBox(s, p), mMinimumSize(0, 0)
    {
        setFlat(true);
#if 0
    setCheckable( true );
#endif
        Connect(this, SIGNAL(toggled(bool)), MemberCall<void(CollapsingGroup *)>(&CollapsingGroup::OnToggled, this));
    }
...
struct TimeSliceControl : CollapsingGroup
{
    ...
    TimeSliceControl(QWidget *p = 0)
        : CollapsingGroup("Time Slice", p), mpSelected(new QComboBox(this)), mpActual(new QLineEdit(this)),
          mpTimeSlice(0)
    {
        ...
        Connect(mpSelected, SIGNAL(currentTextChanged(QString)),
                MemberCall<void(TimeSliceControl *)>(&TimeSliceControl::OnChange, this));
    ...
    void OnChange()
    {
        StringUtils::String s = mpSelected->currentText().toLocal8Bit().constData();
        double d;
        if (s >> d && d > 0)
        {
            delete mpTimeSlice;
...
struct CycleConsumer : Thread
{
    ...
    CycleConsumer() : mMflop(0), mPriority(0), mSleep(0)
    {
    }
    ~CycleConsumer()
    {
    ...
    int OnExecute()
    {
        ...
        WithThreadPriority(mPriority)
        {
            while (!Terminating())
            {
                volatile float f = 0.0; // "volatile" is to avoid loop optimization
...
struct ThreadControl : CollapsingGroup
{
    ...
    ThreadControl(QWidget *pParent = 0)
        : CollapsingGroup("Threads", pParent), mpProcessPriority(new QComboBox(this)), mpSleep(new QComboBox(this)),
          mpCount(new QComboBox(this)), mpPriority(new QComboBox(this)), mpStep(new QComboBox(this)),
          mpThreadCount(new QLineEdit(this))
    {
        ...
        Connect(mpProcessPriority, SIGNAL(currentIndexChanged(int)),
                MemberCall<void(ThreadControl *)>(&ThreadControl::OnProcessPriority, this));
        ...
        Connect(pTimer, SIGNAL(timeout()),
                MemberCall<void(ThreadControl *)>(&ThreadControl::UpdateProcessPriority, this));
        pTimer->start(250);
    }
    ...
    void OnSpawn()
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            StringUtils::String(toString(mpSleep->currentText())) >> p->mSleep;
            StringUtils::String(toString(mpCount->currentText())) >> p->mMflop;
            p->Start();
            if (!p->Running())
                throw StringUtils::String() << "Could not start thread";
    ...
    void OnKill()
    {
        ...
        {
            int n = std::min<int>(mpStep->currentText().toInt(), mThreads.size()), newSize = mThreads.size() - n;
...
struct Window : QWidget, QtSignalConnector
{
    ...
    struct ToolWindow : QWidget{ToolWindow(QWidget * pAppWindow) :
                                    QWidget(pAppWindow){setWindowFlags(Qt::Tool | Qt::WindowStaysOnTopHint);
