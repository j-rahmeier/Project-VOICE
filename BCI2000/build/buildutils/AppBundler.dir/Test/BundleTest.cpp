//////////////////////////////////////////////////////////////////////////////
// $Id: BundleTest.cpp 7462 2023-06-30 14:27:11Z mellinger $
...
int main(int argc, char **argv)
{
    ...
    for (char **pArg = argv + 1; pArg < argv + argc; ++pArg)
    {
        if (**pArg == '-')
        {
            ...
            else if (arg == "--directory" && pArg < argv + argc - 1)
            {
                mode = directory;
            }
            else if (arg == "--debug")
    ...
    if (err == invalidArgs)
        std::cerr
            << "Usage:\nBundleTest ( --create <file size bytes> | --test <position count> <random seed> | --directory ) <file name>"
            << std::endl;
    else
...
static int Create(int size, const char *name)
{
    std::cerr << "Creating test file \"" << name << "\" of size " << size << " ..." << std::flush;
...
static int Test(int count, const char *seed, const char *name)
{
    ...
    if (!seed || !*seed || *seed == '-')
    {
        ...
        nseed = ::atoi(seed);
    std::cerr << "Testing bundle IO (" << count << " seek/read operations, random seed " << nseed << ") ..."
              << std::flush;
...
static int Directory(const char* pName)
{
    std::cerr << "Listing directory " << pName << " ..." << std::endl;
    WIN32_FIND_DATAA data = { 0 };
    HANDLE h = ::FindFirstFileA(pName, &data);
    if (h == INVALID_HANDLE_VALUE) {
        std::cerr << "FindFirstFile failed: " << ::GetLastError() << std::endl;
        return invalidData;
    }
    do {
        std::cerr << data.cFileName << std::endl;
