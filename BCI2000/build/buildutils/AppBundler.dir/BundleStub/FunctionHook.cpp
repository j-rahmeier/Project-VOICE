/////////////////////////////////////////////////////////////////////////////
// $Id: FunctionHook.cpp 7462 2023-06-30 14:27:11Z mellinger $
...
struct MemoryAddress
{
    ...
    MemoryAddress(void *p_)
    {
        p = p_;
    }
    MemoryAddress(size_t n_)
    {
        n = n_;
    }
    MemoryAddress(int n_)
    {
        n = n_;
    }
    bool operator<(const MemoryAddress &other)
    {
        return n < other.n;
    }
    MemoryAddress &operator+=(ptrdiff_t d)
    {
        n += d;
        return *this;
    }
    ...
    ptrdiff_t operator-(const MemoryAddress &other) const
    {
        return n - other.n;
    }
    MemoryAddress operator+(ptrdiff_t d) const
    {
        return MemoryAddress(*this) += d;
    }
    operator void *()
    {
        return p;
...
class Disassembler
{
  public:
    ...
    struct Operand : ud_operand_t
    {
        ...
        Operand() : encoding_bits(0), mutable_lval(nullptr) {}
    };
...
Disassembler::Disassembler(int bits) : mNumOperands(-1)
{
    ::ud_init(&mUd);
...
int Disassembler::ProcessInstruction(MemoryAddress &ip, int maxBytes)
{
...
ud_mnemonic_code Disassembler::Instruction() const
{
    return ::ud_insn_mnemonic(&mUd);
}

const struct Disassembler::Operand &Disassembler::Operand(size_t i) const
{
    if (i >= NumOperands())
        return sNullOperand;
    return mOperands[i];
...
size_t Disassembler::NumOperands() const
{
    if (mNumOperands < 0)
    {
        mNumOperands = 0;
        ...
        { // avoid assumptions about prefix and instruction
            ...
            union {
                int64_t n;
                ud_lval *p;
            } loc = {::ud_insn_off(&mUd) + ::ud_insn_len(&mUd) - d / 8};
...
{
  public:
    TemporaryProtection(void *addr, size_t length, int protection) : mAddr(addr), mLength(length), mProtect(protection)
    {
...
template <class T> T AlignTo(int alignment, T t)
{
...
MemoryAddress AllocateTrampoline(MemoryAddress inCloseTo, size_t inNumBytes)
{
    ...
    if (sEnd - sBegin < allocBytes || !CloseEnough(sBegin, inCloseTo))
    {
        ...
        while (ok && !sBase)
        {
            ...
            if (ok)
            {
                sBase = (char *)::VirtualAlloc(desired, sys.dwAllocationGranularity, MEM_COMMIT | MEM_RESERVE,
                                               PAGE_READWRITE);
                if (!sBase)
                    desired += info.RegionSize;
            }
...
static void SetAbsjmp64Target(uint8_t *jmpAddr, size_t targetAddr)
{
#ifdef _WIN64
    union {
        uint8_t *u8;
...
void Hotpatch(MemoryAddress &addr, const MemoryAddress &patch, const MemoryAddress &patchEnd)
{
...
bool Apply(void *inFunctionHooked, void *inHookFunction, void **outTrampolineToUnhookedFunction)
{
    ...
    if (!FindInstructionEnd(functionHooked, instructionsEnd))
        return false;
    size_t length_1 = instructionsEnd.n - functionHooked.n, length_2 = sizeof(sReljmp32),
...
bool Unapply(void *inFunctionHooked, void *inTrampolineToUnhookedFunction)
{
    MemoryAddress functionHooked(inFunctionHooked), trampoline(inTrampolineToUnhookedFunction);
