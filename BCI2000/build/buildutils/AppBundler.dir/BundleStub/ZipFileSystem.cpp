////////////////////////////////////////////////////////////////////
// $Id: ZipFileSystem.cpp 7462 2023-06-30 14:27:11Z mellinger $
...
struct ZipFileSystem::ZipFileStream : NtHookFs::Stream
{
    ...
    ZipFileStream(const wchar_t *archive, const mz_zip_archive_file_stat *p, uint64_t offset)
        : mpView(0), mpData(0), mInputSize(p->m_comp_size), mAvailIn(0), mOutputSize(p->m_uncomp_size), mOutputPos(0),
          mStatus(0), mCompressed(p->m_method == MZ_DEFLATED), mpBufEnd(mBuffer), mpBufPos(mBuffer),
          mName(new char[::strlen(p->m_filename) + 1]), mNameW(nullptr)
    {
        Stream::Name = &Name;
        Stream::Read = &Read;
        ...
        HANDLE hFile =
            ::CreateFileW(archive, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
        ...
        if (hFile != INVALID_HANDLE_VALUE)
        {
            ...
            if (hMapping)
            {
                mpView = ::MapViewOfFile(hMapping, FILE_MAP_READ, roundedOffset.HighPart, roundedOffset.LowPart,
                                         roundedSize.QuadPart);
...
void ZipFileSystem::TranslateStat(const mz_zip_archive_file_stat *stat, FILE_BOTH_DIR_INFORMATION *finfo, bool isDir)
{
...
bool ZipFileSystem::StatByIndex(int idx, wchar_t **pname, FILE_BOTH_DIR_INFORMATION *finfo)
{
...
bool ZipFileSystem::StatByName(const wchar_t *file, const wchar_t *end, FILE_BOTH_DIR_INFORMATION *finfo)
{
...
NtHookFs::Stream *ZipFileSystem::OpenByName(const wchar_t *file, const wchar_t *end)
{
...
NtHookFs::Stream *ZipFileSystem::OpenByIndex(int index)
{
    mz_zip_archive_file_stat stat;
    uint64_t offset = 0;
