//////////////////////////////////////////////////////////////////////////////
// $Id: AppBundler.cpp 7787 2024-01-05 21:45:38Z mellinger $
...
static std::ostream &operator<<(std::ostream &os, const PrintSize &p)
{
    return p.Print(os);
}
...
{
  public:
    Logstream(std::ostream &out, int verbosity) : mOutstream(out), mNullstream(0), mVerbosity(verbosity)
    {
    }
    ~Logstream()
    {
    ...
    {
        return level > mVerbosity ? mNullstream : mOutstream;
    }
...
int AppBundler(int argc, char **argv)
{
    ...
    {
        cerr << "Usage:\nAppBundler [--verbosity={0..10} --compression-level={0..10}] <bundle> <main executable> "
                "[<file 1>[=<bundle file 1>] ... <file n>[=<bundle file n>]]"
             << endl;
        return help ? 0 : -1;
    }
    ...
    for (size_t i = 0; i < sizeof(stubDirNullout) / sizeof(*stubDirNullout); ++i)
    {
        ...
        {
            cerr << "Warning: Stub's directory entry at offset " << stubDirNullout[i]
                 << " is not empty, will be cleared" << endl;
            ::memset(p, 0, sizeof(*p));
        }
    }
    if (pRsrcFrom)
    {
        ...
        ::memcpy(stubData.data() + pRsrcTo->PointerToRawData, exeFile.BaseAddress() + pRsrcFrom->PointerToRawData,
                 pRsrcFrom->SizeOfRawData);
