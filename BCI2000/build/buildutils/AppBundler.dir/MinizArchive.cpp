////////////////////////////////////////////////////////////////////
// $Id: MinizArchive.cpp 7462 2023-06-30 14:27:11Z mellinger $
...
MinizArchive::MinizArchive() : mFile(INVALID_HANDLE_VALUE), mMutex(::CreateMutexA(0, 0, 0)), mWriteCallback(0)
{
...
bool MinizArchive::Create(const wchar_t *name)
{
    MutexLock lock(mMutex);
    mFile =
        ::CreateFileW(name, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
    return mFile != INVALID_HANDLE_VALUE;
}

bool MinizArchive::Open(const wchar_t *name)
{
    MutexLock lock(mMutex);
    bool writable = true;
    mFile =
        ::CreateFileW(name, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    if (mFile == INVALID_HANDLE_VALUE)
    {
        writable = false;
        mFile = ::CreateFileW(name, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    }
    mWritable = writable;
    return mFile != INVALID_HANDLE_VALUE;
}

void MinizArchive::Close()
{
    MutexLock lock(mMutex);
...
bool MinizArchive::IsOpen() const
{
    return mFile != INVALID_HANDLE_VALUE;
}

mz_int64 MinizArchive::ArchiveSize() const
{
...
MinizArchive::Callback MinizArchive::SetWriteCallback(Callback c)
{
...
bool MinizArchive::Stat(const char *file, mz_zip_archive_file_stat *pStat, mz_uint64 *pOffset)
{
    MutexLock lock(mMutex);
    bool ok = false;
...
bool MinizArchive::Stat(int index, mz_zip_archive_file_stat *pStat, mz_uint64 *pOffset, mz_bool *pIsDir)
{
    MutexLock lock(mMutex);
    bool ok = false;
...
size_t MinizArchive::OnRead(mz_uint64 file_ofs, void *pBuf, size_t n)
{
    MutexLock lock(mMutex);
    DWORD read = 0;
    LARGE_INTEGER pos;
