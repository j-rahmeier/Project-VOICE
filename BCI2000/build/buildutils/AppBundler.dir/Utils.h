////////////////////////////////////////////////////////////////////
// $Id: Utils.h 7462 2023-06-30 14:27:11Z mellinger $
...
template <int N, class T> T Align(T a)
{
    return ((a + N - 1) / N) * N;
}

template <class T, int A = 64> class Vector
{
  public:
    explicit Vector(size_t s = 0) : mSize(s), mCapacity(Align<A>(s)), mp(mCapacity ? new T[mCapacity] : nullptr)
    {
    }
    Vector(const Vector &v) : mSize(0), mCapacity(0), mp(nullptr)
    {
        assign(v);
    }
...
template <class T> class BasicString
{
  public:
    ...
    BasicString(const T *t, int len = -1) : d(0)
    {
        append(t, len);
    }
    ...
    BasicString &operator+=(const T *t)
    {
        return append(t);
    }
    ...
    BasicString operator+(const T *t) const
    {
        return BasicString(*this).append(t);
    }
    BasicString operator+(const BasicString &s) const
    {
        return BasicString(*this).append(s.c_str(), s.length());
    }
    T &operator[](size_t i)
    {
        return d.data()[i];
