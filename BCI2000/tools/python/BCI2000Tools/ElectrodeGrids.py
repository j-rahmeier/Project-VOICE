...
# ///////////////////////////////////////////////////////////////////////////
# $Id: ElectrodeGrids.py 6642 2022-03-29 03:17:13Z jhill $
# Author: jeremy.hill@neurotechcenter.org
# Description: parsing of electrode grid layouts from plain text
#
...
Try this:
  ...
  print( g )
  print( g.ChannelNames() )         # BCI2000 parameter format
  print( g.BipolarSpatialFilter() ) # BCI2000 parameter format
...
from . import Electrodes # only used for the ChannelSet class, in the GridSet.ChannelSet() method
from . import Plotting   # only used for load_plt() in the PlotGrids() function (could be approximated by `import matplotlib.pyplot as plt` there)

if sys.version < '3': bytes = str
else: unicode = str; basestring = ( unicode, bytes )

def cmp(a, b): return (a > b) - (a < b)  # was a builtin in Python 2, gone in Python 3
...
def adddoc(func):
	for arg in inspect.getargs(func.__code__ if hasattr( func, '__code__' ) else func.func_code).args:
...
class ElectrodeCoordinate(object):
	...
	def name(self, prefix=None, sep='', precision=3, escape=False):
		if prefix is None: prefix = self.gridName
...
def ParseOneGrid(lines, gridname):
	...
	known = [[v,rows[i],cols[i]] for i,v in enumerate(vals) if v is not None]
	...
	def sorted_( seq, cmp ):
		try: return sorted( seq, cmp=cmp ) # Python 2
		except TypeError: return sorted( seq, key=functools.cmp_to_key( cmp ) ) # Python 3
	...
	if lastcol==startcol: # working up/down within the same column, i.e. in column-major order, which means row-index varies faster, which means column index is more significant in sorting
		...
		grid = sorted_(grid, cmp=lambda x,y: cmp([horizontalDirection*x[2],verticalDirection*x[1]],  [horizontalDirection*y[2],verticalDirection*y[1]]) )
		...
		if verticalDirection == 0: verticalDirection = 1
		grid = sorted_(grid, cmp=lambda x,y: cmp([verticalDirection*x[1],horizontalDirection*x[2]],  [verticalDirection*y[1],horizontalDirection*y[2]]) )
	else:
		raise ValueError("numbers %d and %d in %s do not share the same row or the same column" % (startval, lastval, gridname))
	
	prevval = None
	for entry in grid:
		if entry[0] is None and prevval is not None: entry[0] = prevval + 1
		prevval = entry[0]
	prevval = None
	for entry in grid[::-1]:
		if entry[0] is None and prevval is not None: entry[0] = prevval - 1
	...
	grid = sorted_(grid, cmp=lambda x,y: cmp([x[1],x[2]],  [y[1],y[2]]) )
...
def EachGrid(gs, grids=None, numsort=True):
	...
	if grids is None: grids = GetGridNames(gs)
...
def ReportGrids(gs, grids=None, sep='', precision=None):
	...
	"""
	s = ''
	if precision is None: precision = NumericalPrecision(gs)
	blank = ' ' * int(precision)
...
def PlotGrids(gs, grids=None, drawnow=True):
	...
	NB: requires installation of the matplotlib python package for plotting.
	"""
...
def WriteFile(content, f):
	...
	if hasattr(f, 'write'): f.write(txt+'\n'); return None
	else: return content
...
class GridSet(smartlist):
	...
	def ChannelSet(self, grids=None, precision=None, prefix=None, sep='', numsort=True):
		"""
		If the Electrodes module is available, create a ChannelSet object from
		the specified grid(s).  TODO: plotting non-EEG ChannelSets is still a work in progress.
		"""
		if precision is None: precision = NumericalPrecision(self)	
		names = []; coords = []
...
if __name__ == '__main__':
	...
	alphabetize = opts.pop('alphabetize', None) is not None
