...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% $Id: bci2000chain.m 7195 2023-02-07 18:31:03Z mellinger $ 
...
if nargin < 1, datfile = '--setup'; end  % so you can just say  `run C:\BING\BANG\BONG\tools\matlab\bci2000chain.m`  and that will bootstrap your paths for the duration of the session.
if ischar(datfile)
	if strcmp(lower(datfile), '-setup') | strcmp(lower(datfile), '--setup')
		oldwd = pwd;
		self = which(mfilename);
		cd(fileparts(self))
		fprintf('running `bci2000path -AddAll` from %s\n', fileparts(fileparts(fileparts(which('bci2000path')))))
		bci2000path -AddAll
		fprintf('Call `bci2000path -RemoveAll` to remove these paths.\n' )
		cd(oldwd)
		return
	end
end
...
if isempty(prm)
	cmd{end+1} = ['bci_dat2stream' offset duration input];
else
	...
	if dat2stream_has_p_flag
		cmd{end+1} = ['bci_dat2stream -p$PRMFILE_IN' offset duration input]; % new-style bci_dat2stream with -p option
		binaries{end+1} = 'bci_dat2stream';
	else
		cmd{end+1} =  '(';   % old-style bci_dat2stream with no -p option
		cmd{end+1} =  '   bci_prm2stream < $PRMFILE_IN';
		cmd{end+1} = ['&& bci_dat2stream --transmit-sd' input ];
		cmd{end+1} =  ')';
		binaries{end+1} = 'bci_dat2stream';
...
if isempty(err)
	...
	out.States = rmfield_safely(mat.Index, 'Signal');  % TODO: how do the command-line tools handle event states? this seems to be set up to deliver one value per block whatever kind of state we're dealing with
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function s = rmfield_safely(s, fieldname)
% Because it is possible for the struct to get loaded with "illegal"
% fieldnames __pad0 etc. in place, and then MathWorks' OWN implementation
% of rmfield crashes, even when trying to remove an UNRELATED field.
% So, as with nearly everything, if you want robust tools, sooner or
% later you have to do MathWorks' job for them.

fields = fieldnames(s);
values = struct2cell(s);
good = ~strcmp(fields, fieldname) & ~strncmp(fields, '_', 1 );
fields = fields( good );
values = values( good );
