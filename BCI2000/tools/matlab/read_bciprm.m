...
% 
% P = READ_BCIPRM(X, 'Value') [deprecated]
% 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% $Id: read_bciprm.m 7195 2023-02-07 18:31:03Z mellinger $ 
...
if ~isempty(subfield)
  fprintf(['NOTE: the read_bciprm(X, ''Value'') syntax is deprecated because '...
           'it fails when parameters have paths']);
	ff = fieldnames(p);
	for i = 1:numel(ff)
		if isstruct(p.(ff{i}))
...
if ischar(p)
	...
	elseif exist(p, 'file')
		...
		if strncmp(lower(fliplr(p)), fliplr('.dat'), 4) | strncmp(lower(fliplr(p)), fliplr('.bci2000'), 7) | strncmp(lower(fliplr(p)), fliplr('.bcidat'), 7)
			[ans ans p] = load_bcidat(p, [0 0]);
			return
		end
		if ~strncmp(lower(fliplr(p)), fliplr('.prm'), 4) && ~strncmp(lower(fliplr(p)), fliplr('.bciprm'), 7), error('can only deal with .dat and .prm files'), end
		fid = fopen(p, 'r');
