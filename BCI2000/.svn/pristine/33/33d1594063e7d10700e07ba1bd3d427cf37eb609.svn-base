...
SerialPort::SerialPort(const char *portName)
{
    ...
    if (this->handler == INVALID_HANDLE_VALUE)
    {
        ...
        {
          std::cerr << "ERROR: Handle was not attached. Reason : ERROR_ACCESS_DENIED\n";
        }
        else
        {
            std::cerr << "ERROR!!!\n";
    ...
    else
    {
        ...
        else
        {
            ...
            {
                this->connected = true;
                if (PurgeComm(this->handler, PURGE_RXCLEAR | PURGE_RXCLEAR | PURGE_TXCLEAR) == 0)
                  std::cerr << "Serial error: could not purge communication" << std::endl;
            ...
            if (!SetCommTimeouts(handler, &timeout)) {
                //handle error here
            }
        }
    }
