...
double ObserverNode::OnEvaluate()
{
    throw std_logic_error << "Function should never be called";
    return 0;
}

ExpressionParser::Node *ObserverNode::OnSimplify()
{
    throw bciexception << Name() << "Observers must be used with statistics functions";
    return this;
}

ObserverSource::ObserverSource(const std::string &inName, const std::string &inWhen, const std::string inReset,
                               StatisticalObserver::Number inWindow, bool inWeighted)
    : DataSource(inName), mConfig(StatisticalObserver::None), mWhen(inWhen), mReset(inReset), mWindow(inWindow),
      mObserveWeighted(inWeighted), mStreamingMax(0), mCurSample(0)
{
}
...
const StatisticalObserver::Observer &ObserverSource::SingleObserver(size_t inIdx) const
{
    Assert(inIdx < mObservers.size());
    return *mObservers[inIdx];
}

void ObserverSource::OnInitialize(const Context &inContext)
{
    DeleteObservers();
    ...
    mStreamingMax = 1;
    size_t numObservers = 1, totalCount = 1;
        ...
        for (size_t j = 0; j < p.Dimensions().size(); ++j)
        {
            ...
            if (dim.streaming)
            {
                ...
                if (!dynamic_cast<ExpressionSource *>(Sources()[i]))
                    mStreamingMax = std::max(dim.Size(), mStreamingMax);
            }
    ...
    if (mSampleDimensions.empty())
        bcierr << "No continuous dimension present when observing weighted data";
...
void ObserverSource::OnProcess(const Context &inContext)
{
    ...
    {
        int agingCount =
            ((mCurSample + 1) * mSampleBlockSize) / mStreamingMax - (mCurSample * mSampleBlockSize) / mStreamingMax;
...
DataSource::Value ObserverSource::OnData(int)
{
    throw std_logic_error << "Function should never be called";
    return 0;
}
...
void ObserverSource::Observe(size_t inSource, size_t inSourceDim, size_t inSourceIdx, size_t inBuffer,
                             size_t inObserverIdx, double inWeight)
{
    if (sAbortFlag)
        return;
    ...
    else
    {
        ...
        if (inSourceDim >= p.Dimensions().size())
        {
            if (mObserveWeighted)
            {
                StatisticalObserver::Number weight =
                    inWeight * Sources()[inSource]->Data(static_cast<int>(inSourceIdx));
        ...
        else
        {
            ...
            else if (mObserveWeighted && dim.continuous)
            {
                ...
                {
                    StatisticalObserver::Number value = (i - dim.unit.Offset()) * dim.unit.Gain();
                    mSampleBuffer[inBuffer] = value;
                    Observe(inSource, inSourceDim + 1, inSourceIdx * dim.Size() + i, inBuffer + 1, inObserverIdx,
                            inWeight);
                }
            }
            else
            {
                for (int i = 0; i < dim.Size(); ++i)
                    Observe(inSource, inSourceDim + 1, inSourceIdx * dim.Size() + i, inBuffer,
                            inObserverIdx * dim.Size() + i, inWeight);
            }
        }
    }
}

void ObserverSource::Reset()
{
...
void ObserverSource::DeleteObservers()
{
    for (size_t i = 0; !sAbortFlag && i < mObservers.size(); ++i)
        delete mObservers[i];
    mObservers.clear();
