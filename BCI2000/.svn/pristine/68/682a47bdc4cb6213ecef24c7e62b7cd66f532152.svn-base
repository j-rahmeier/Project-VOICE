...
#if defined(_WIN32)
#   if defined (VISAEXT_EXPORTS)
#      define VISAEXTDECLSPEC __declspec(dllexport)
#   else
#      define VISAEXTDECLSPEC __declspec(dllimport)
#   endif
#elif defined(__linux__)
#define VISAEXTDECLSPEC __attribute__((visibility("default")))
#define VISAEXTDECLSPEC_HIDDEN __attribute__((visibility("hidden")))
#else
//  do nothing and hope for the best?
#define VISAEXTDECLSPEC
#define VISAEXTDECLSPEC_HIDDEN
...
#define viMacroExtPeek8(addrOffset, pValue) \
   (whereMacroNow < MAX_MACRO_ELEMENTS) ? \
      ...
      macroArray[whereMacroNow].address = addrOffset, \
      macroArray[whereMacroNow].parm1   = (uintptr_t) pValue, \
      macroArray[whereMacroNow].parm2   = 0, \
      whereMacroNow++, \
      VI_SUCCESS \
   : \
...
#define viMacroExtSendNow() \
   viMacroExt((ViSession) macroArray[0].parm1, macroArray, whereMacroNow);
...
VISAEXTDECLSPEC ViStatus _VI_FUNC  viMacroExt(ViSession vi, ViMacroExtElem macroArray[], ViUInt32 size);
