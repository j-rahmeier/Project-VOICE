...
int AddEventVariable(int, char**, BCI2000Remote&);
int GetEventVariable(int, char**, BCI2000Remote&);
int SetEventVariable(int, char**, BCI2000Remote&);
int PulseEventVariable(int, char**, BCI2000Remote&);
...
sCommands[] =
{
  ...
  COMMAND( AddEventVariable, 3, "<variable name> <bit width> <initial value>", "Creates a new event variable" )
  COMMAND( GetEventVariable, 1, "<variable name>", "Returns the value of the named BCI2000 event variable" )
  COMMAND( SetEventVariable, 2, "<variable name> <variable value>", "Asynchronously sets the value of the named BCI2000 event variable" )
  COMMAND( PulseEventVariable, 2, "<variable name> <variable value>", "Asynchronously sets the value of the named BCI2000 event variable for the duration of a single sample" )
...
static std::vector<const char*> sProperties;
...
int main( int argc, char* argv[] )
{
  ...
  PROPERTY( TelnetAddress, std::string )
  PROPERTY( OperatorPath, std::string )
  PROPERTY( WindowVisible, atoi )
  PROPERTY( WindowTitle, std::string )
  PROPERTY( SubjectID, std::string )
  PROPERTY( SessionID, std::string )
  PROPERTY( DataDirectory, std::string )
  // Execute command.
  int i = 1;
  while( i < argc && *argv[i] == '-' )
  {
  ...
  while( j < sizeof( sCommands ) / sizeof( *sCommands ) )
  {
    if( !::stricmp( pCommand, sCommands[j].name ) )
    {
      ...
      {
        std::cerr << "Expected " << argsExpected << " argument(s) for " << sCommands[j].name
                  << " command, got " << argsReceived
                  << std::endl;
        return error;
      }
      std::string operatorPath = bci.OperatorPath();
      bci.OperatorPath( "" );
      ...
      if( !bci.Result().empty() )
      {
        std::ostream& os = ( result == ok ) ? std::cout : std::cerr;
        os << bci.Result() << std::endl;
      }
      return result;
    }
    ++j;
  }
  std::cerr << "Unknown command: " << pCommand << std::endl;
  Help( false );
  return error;
}
...
Help( bool inFull )
{
  std::cout << "Usage: " EXECUTABLE_NAME " [--property=value ...] command <arg1 arg2 ...>\n";
  if( !inFull )
  {
    std::cout << "Enter \"" EXECUTABLE_NAME " --help\" for a list of commands.";
  }
  else
  {
    std::cout << "Remotely controls BCI2000.\n\n"
...
Run( int inArgc, char** inArgs, BCI2000Remote& ioBCI )
{
  ...
  {
    std::ifstream file( inArgs[i] );
    if( !file.is_open() )
    {
      std::cerr << "Could not open script file \"" << inArgs[i] << "\" for reading" << std::endl;
      return error;
    }
...
int
AddEventVariable(int, char** inArgs, BCI2000Remote& ioBCI)
{
    return ioBCI.AddEventVariable(inArgs[0], ::atoi(inArgs[1]), ::atof(inArgs[2])) ? ok : error;
}

int
GetEventVariable(int, char** inArgs, BCI2000Remote& ioBCI)
{
    double d;
    return ioBCI.GetEventVariable(inArgs[0], d) ? ok : error;
}

int
SetEventVariable(int, char** inArgs, BCI2000Remote& ioBCI)
{
    return ioBCI.SetEventVariable(inArgs[0], ::atof(inArgs[1])) ? ok : error;
}

int
PulseEventVariable(int, char** inArgs, BCI2000Remote& ioBCI)
{
    return ioBCI.PulseEventVariable(inArgs[0], ::atof(inArgs[1])) ? ok : error;
}
