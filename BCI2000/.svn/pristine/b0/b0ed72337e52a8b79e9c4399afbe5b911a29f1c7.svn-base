...
DataSource *DataSource::SourceList::Find(const std::string &inName) const
{
    DataSource *p = NULL;
...
bool DataSource::SourceList::Contains(DataSource *p)
{
    return find(begin(), end(), p) != end();
}

DataSource::SourceList &DataSource::SourceList::Add(DataSource *p)
{
    Assert(p != NULL);
    push_back(p);
    return *this;
...
DataSource::SourceList &DataSource::SourceList::AddUnique(DataSource *p)
{
    if (!Contains(p))
        Add(p);
    return *this;
...
bool DataSource::Dimension::operator==(const Dimension &d) const
{
    return name == d.name && continuous == d.continuous && streaming == d.streaming && unit == d.unit &&
           labels == d.labels;
}

DataSource::DataProperties::DataProperties(const SignalProperties &s) : mDimensions(2), mUnit(s.ValueUnit())
{
    ...
    if (!s.IsStream() && s.UpdateRate() > 0)
    {
        ...
        st.labels.Resize(1);
        st.unit.SetGain(1.0 / s.UpdateRate()).SetSymbol("s");
        st.continuous = true;
        st.streaming = true;
    }
}
...
DataSource::DataProperties::operator SignalProperties() const
{
    ...
    if (!mDimensions.empty())
    {
        ...
        {
            result.SetChannels(1).SetElements(1);
        ...
        else
        {
            ...
            for (size_t i = 1; i < nonSingletonDims.size(); ++i)
            {
                ...
                const LabelIndex &labels = dim.labels.IsTrivial() ? LabelIndex(dim.unit) : dim.labels;
                if (i == 1)
...
int DataSource::DataProperties::DataSize() const
{
    int result = 1;
    for (size_t i = 0; i < mDimensions.size(); ++i)
...
int DataSource::DataProperties::ToSingleIndex(const IndexList &inIdx) const
{
...
DataSource::IndexList DataSource::DataProperties::ToIndexList(int inIdx) const
{
...
bool DataSource::DataProperties::Compatible(const DataSource::DataProperties &inP) const
{
    SignalProperties s1(*this), s2(inP);
    bool result = true;
...
DataSource::DataProperties &DataSource::DataProperties::OuterProduct(const DataProperties &inP)
{
...
void DataSource::Depends(const Context &inContext, SourceList &outList, SourceList inList)
{
    BCIStream::ContextFrame frame(Name());
    if (inList.Contains(this))
    {
        bcierr << "Circular dependency on \"" << mName << "\"";
    }
    else
    {
        OnDepends(inContext);
...
void DataSource::Initialize(const Context &inContext)
{
    BCIStream::ContextFrame frame(Name());
    OnInitialize(inContext);
}

void DataSource::Process(const Context &inContext)
{
    BCIStream::ContextFrame frame(Name());
    OnProcess(inContext);
}

DataSource::Value DataSource::Data(int inIdx)
{
    return sAbortFlag ? 0 : OnData(inIdx);
}

DataSource::Value DataSource::Data(const IndexList &inIdx)
{
