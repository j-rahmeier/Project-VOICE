...
void TDAS_ADC::Preflight(const SignalProperties &, SignalProperties &Output) const
{
    ...
    DASQueue::DASInfo boardInfo = {
        Parameter("BoardNumber"),     static_cast<int>(Parameter("SamplingRate").InHertz()),
        Parameter("SampleBlockSize"), Parameter("SourceCh"),
    ...
    Output = SignalProperties(boardInfo.numChannels, boardInfo.sampleBlockSize, SignalType::int16);
}

void TDAS_ADC::Initialize(const SignalProperties &, const SignalProperties &)
{
    DASQueue::DASInfo boardInfo = {
        Parameter("BoardNumber"),     static_cast<int>(Parameter("SamplingRate").InHertz()),
        Parameter("SampleBlockSize"), Parameter("SourceCh"),
    ...
    if (!mInputQueue.is_open())
        bcierr << "Could not initialize A/D board";
}

void TDAS_ADC::Process(const GenericSignal &, GenericSignal &Output)
{
    for (int sample = 0; sample < Output.Elements(); ++sample)
        for (int channel = 0; channel < Output.Channels(); ++channel)
        {
            if (!mInputQueue)
            {
                bcierr << "Lost connection to A/D board";
                return;
            }
