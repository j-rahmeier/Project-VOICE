...
ChannelSetSource::ChannelSetSource(const std::string &inName, const ChannelSet &inSet)
    : DataSource(inName), mChannelSet(inSet), mpSignal(NULL)
{
}

void ChannelSetSource::OnInitialize(const Context &inContext)
{
...
void ChannelSetSource::OnProcess(const Context &inContext)
{
    mpSignal = inContext.signal;
}

DataSource::Value ChannelSetSource::OnData(int inIdx)
{
    Assert(mpSignal != nullptr);
    int ch = inIdx / mpSignal->Elements(), el = inIdx % mpSignal->Elements();
