...
void CDataFile::SetFileName(t_Str szFileName)
{
    if (m_szFileName.size() != 0 && CompareNoCase(szFileName, m_szFileName) != 0)
    {
        ...
        Report(E_WARN, "[CDataFile::SetFileName] The filename has changed from <%s> to <%s>.", m_szFileName.c_str(),
               szFileName.c_str());
    }
...
bool CDataFile::Load(t_Str szFileName)
{
    ...
    //#elseif
    std::fstream File(szFileName.c_str(), std::ios::in);
    //#endif
    ...
    if (File.is_open())
    {
        ...
        for (s_pos = m_Sections.begin(); s_pos != m_Sections.end(); s_pos++)
        {
            ...
            for (k_pos = Section.Keys.begin(); k_pos != Section.Keys.end(); k_pos++)
            {
                ...
                {
                    WriteLn(File, "%s%s%s%s%c%s", Key.szComment.size() > 0 ? "\n" : "",
                            CommentStr(Key.szComment).c_str(), Key.szComment.size() > 0 ? "\n" : "", Key.szKey.c_str(),
                            EqualIndicators[0], Key.szValue.c_str());
                }
            }
        }
    }
...
bool CDataFile::GetBool(t_Str szKey, t_Str szSection)
{
    ...
    if (szValue.find("1") == 0 || CompareNoCase(szValue, "true") || CompareNoCase(szValue, "yes"))
    {
        bValue = true;
    }
...
void Trim(t_Str &szStr)
{
    ...
    if (rPos > nPos && rPos != std::string::npos)
