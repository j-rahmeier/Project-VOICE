...
#define UNICORN_SUPPORTED_DEVICE_VERSION "1."

//! The device name of the recommended (delivered) Bluetooth adapter.
#define UNICORN_RECOMMENDED_BLUETOOTH_DEVICE_NAME "CSR8510 A10"

//! The manufacturer of the recommended (delivered) Bluetooth adapter.
#define UNICORN_RECOMMENDED_BLUETOOTH_DEVICE_MANUFACTURER "Cambridge Silicon Radio Ltd."
...
//! Index of the first Accelerometer \ref UNICORN_AMPLIFIER_CHANNEL in the \ref UNICORN_AMPLIFIER_CONFIGURATION Channels
//! array.
#define UNICORN_ACCELEROMETER_CONFIG_INDEX 8

//! Index of the first gyroscope \ref UNICORN_AMPLIFIER_CHANNEL in the \ref UNICORN_AMPLIFIER_CONFIGURATION Channels
//! array.
#define UNICORN_GYROSCOPE_CONFIG_INDEX 11

//! Index of the battery level \ref UNICORN_AMPLIFIER_CHANNEL in the \ref UNICORN_AMPLIFIER_CONFIGURATION Channels
//! array.
...
//! Index of the validation indicator \ref UNICORN_AMPLIFIER_CHANNEL in the \ref UNICORN_AMPLIFIER_CONFIGURATION
//! Channels array.
...
#define UNICORN_NUMBER_OF_DIGITAL_OUTPUTS 8
    ...
    */
    ...
    /*!
        Evaluates which Bluetooth adapter is currently in use and whether it is the recommended (delivered) Bluetooth
       adapter.

        \param bluetoothAdapterInfo		A pointer to a \ref UNICORN_BLUETOOTH_ADAPTER_INFO structure that receives
    ...
    */
    ...
    /*!
        Discovers available paired or unpaired devices. Estimates the number of available paired or unpaired devices and
       returns information about discovered devices.
        ...
        \param availableDevicesCount	A pointer to a variable that receives the number of
                                        available devices.
        \param onlyPaired           	Defines whether only paired devices or only unpaired
                                        ...
                                        faster. If \ref TRUE, only paired devices are discovered. If
                                        \ref FALSE, only unpaired devices can be discovered.

        \return		An error code is returned as integer if scanning for available devices fails.
    */
    UNICORN_API int UNICORN_GetAvailableDevices(UNICORN_DEVICE_SERIAL *availableDevices,
                                                uint32_t *availableDevicesCount, BOOL onlyPaired);
    ...
    /*!
        ...
        \param hDevice		A pointer to a \ref UNICORN_HANDLE that receives the handle associated with the current
       session if the device could be opened successfully.
    ...
    */
    ...
    /*!
        ...
        \param hDevice				The \ref UNICORN_HANDLE associated with the session.
        \param configuration		A pointer to a \ref UNICORN_AMPLIFIER_CONFIGURATION which stores the configuration
       of the amplifier.
    ...
    */
    ...
    /*!
        ...
        \param destinationBuffer			A pointer to the destination buffer that receives the acquired data. The
       destination buffer must provide enough memory to hold the requested number of scans multiplied by the number of
       acquired channels. Call \ref UNICORN_GetNumberOfAcquiredChannels to determine the number of acquired channels.
                                            Call \ref UNICORN_GetChannelIndex to determine the index of a channel within
       a scan. Example: The sample of the battery level channel in the n-th scan is: n* \ref
       UNICORN_GetNumberOfAcquiredChannels()+ \ref UNICORN_GetChannelIndex(“Battery Level”) \param
       destinationBufferLength		Number of floats fitting into destination buffer.

        \return		An error code is returned as integer if data could not be read.
    */
    UNICORN_API int UNICORN_GetData(UNICORN_HANDLE hDevice, uint32_t numberOfScans, float *destinationBuffer,
                                    uint32_t destinationBufferLength);

    //! Determines the index of the requested channel in an acquired scan.
    /*!
        Uses the currently set \ref UNICORN_AMPLIFIER_CONFIGURATION to get the index of the requested channel within an
       acquired scan.
    ...
    */
    ...
    /*!
        ...
        \param hDevice	The \ref UNICORN_HANDLE associated with the session.
        \param deviceInformation	A pointer to a \ref UNICORN_DEVICE_INFORMATION that receives information about the
       device.
