...
class CAcqMessage
{
  public:
    ...
    {
        return 12;
    } // Header portion includes m_chId,m_wCode,m_wRequest,m_dwSize
    BOOL IsCtrlPacket()
    {
        return (!memcmp(m_chId, "CTRL", 4));
    }
    BOOL IsDataPacket()
    {
        return (!memcmp(m_chId, "DATA", 4));
    }
...
class AcqBasicInfo
{
  public:
    ...
    void Init()
    {
        memset((void *)&dwSize, 0, sizeof(AcqBasicInfo));
    }
    BOOL Validate()
    {
        return (nBlockPnts && nRate && nEegChan && nDataSize);
    }
};
