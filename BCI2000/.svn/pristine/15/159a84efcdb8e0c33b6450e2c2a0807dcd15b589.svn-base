...
void WebcamLogger::Preflight() const
{
  ...
  mControllers.clear(); // make sure there is no controller instance connected to a camera
...
void WebcamLogger::Initialize()
{
  ...
  if (Parameter("RecordAll"))
    mControllers = CreateControllers(
      Parameter("StartIndex"), Parameter("EndIndex"), true, false,
      Parameter("WebcamFormat"), 
      Parameter("WebcamURL"), Parameter("AudioURL")
    );
  else
    mControllers = CreateControllers(
      Parameter("CameraNumber"), Parameter("CameraNumber"), false, false,
      Parameter("WebcamFormat"), 
      Parameter("WebcamURL"), Parameter("AudioURL")
    );
...
WebcamLogger::ControllerContainer WebcamLogger::CreateControllers(
  int startIndex, int endIndex, bool ignoreMissing, bool preflight,
...
)
{
  ...
  for (int camIndex = startIndex; camIndex <= endIndex; ++camIndex)
  {
    ...
    FindFormat(camIndex, formats, params.width, params.height, params.fps, params.videoprofile, params.compressionFactor);
    ...
    if (preflight)
      pController->Preflight(camIndex, params);
    else
      pController->Initialize(camIndex, params);
    if (pController->Error().empty())
      controllers.push_back(std::move(pController));
    else if (!ignoreMissing || pController->Error().find("No camera available") != 0)
      bcierr << "Camera " << camIndex << ": " << pController->Error();
  }
  return controllers;
}

void WebcamLogger::FindFormat(int index, const ParamRef& formats, int& width, int& height, int& fps, int& videoprofile, int& compressionFactor)
{
  for (int i = 0; i < formats->NumRows(); ++i) {
    if (formats(i, 0) == index) {
      ...
      if (formats->ColumnLabels().Exists("compression factor"))
          compressionFactor = formats(i, "compression factor");
      else
          compressionFactor = 8;
      return;
    }
