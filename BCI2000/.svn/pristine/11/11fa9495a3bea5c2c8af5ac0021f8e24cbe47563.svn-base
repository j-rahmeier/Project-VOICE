...
void PrintErrorAddressHelper(int errAddress)
{
	if (!(errAddress < 0))
		...
		//bciwarn << "LabJack error/warning address: " << errAddress;
}
	...
	const char * description, va_list args)
{
	...
	if (err >= LJME_WARNINGS_BEGIN && err <= LJME_WARNINGS_END) {
		...
		//bciwarn << "Warning code :" << errName << " with value " << err;
	...
	else if (err != LJME_NOERROR)
	{
		...
		PrintErrorAddressHelper(errAddress);
		//bcierr << "Error code :" << errName << " with value " << err << ". Closing all devices and exiting.";
