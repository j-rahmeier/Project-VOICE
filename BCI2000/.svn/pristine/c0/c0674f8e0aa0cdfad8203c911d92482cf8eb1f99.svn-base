...
#undef EVENTCALLBACK
    ...
    int EXTCCONV cbACalibrateData(int BoardNum, long NumPoints, int Gain, USHORT *ADData);
    ...
    int EXTCCONV cbAConvertData(int BoardNum, long NumPoints, USHORT *ADData, USHORT *ChanTags);
    int EXTCCONV cbAConvertPretrigData(int BoardNum, long PreTrigCount, long TotalCount, USHORT *ADData,
                                       USHORT *ChanTags);
    int EXTCCONV cbAIn(int BoardNum, int Chan, int Gain, USHORT *DataValue);
    int EXTCCONV cbAInScan(int BoardNum, int LowChan, int HighChan, long Count, long *Rate, int Gain, HGLOBAL MemHandle,
                           int Options);
    int EXTCCONV cbALoadQueue(int BoardNum, short *ChanArray, short *GainArray, int NumChans);
    int EXTCCONV cbAOut(int BoardNum, int Chan, int Gain, USHORT DataValue);
    int EXTCCONV cbAOutScan(int BoardNum, int LowChan, int HighChan, long Count, long *Rate, int Gain,
                            HGLOBAL MemHandle, int Options);
    int EXTCCONV cbAPretrig(int BoardNum, int LowChan, int HighChan, long *PreTrigCount, long *TotalCount, long *Rate,
                            int Gain, HGLOBAL MemHandle, int Options);
    int EXTCCONV cbATrig(int BoardNum, int Chan, int TrigType, USHORT TrigValue, int Gain, USHORT *DataValue);
    int EXTCCONV cbC7266Config(int BoardNum, int CounterNum, int Quadrature, int CountingMode, int DataEncoding,
    ...
    int EXTCCONV cbC8536Config(int BoardNum, int CounterNum, int OutputControl, int RecycleMode, int Retrigger);
    int EXTCCONV cbC9513Config(int BoardNum, int CounterNum, int GateControl, int CounterEdge, int CountSource,
                               int SpecialGate, int Reload, int RecycleMode, int BCDMode, int CountDirection,
                               int OutputControl);
    int EXTCCONV cbC8536Init(int BoardNum, int ChipNum, int Ctr1Output);
    int EXTCCONV cbC9513Init(int BoardNum, int ChipNum, int FOutDivider, int FOutSource, int Compare1, int Compare2,
                             int TimeOfDay);
    int EXTCCONV cbCFreqIn(int BoardNum, int SigSource, int GateInterval, USHORT *Count, long *Freq);
    ...
    int EXTCCONV cbCStoreOnInt(int BoardNum, int IntCount, short *CntrControl, HGLOBAL MemHandle);
    ...
    int EXTCCONV cbDInScan(int BoardNum, int PortNum, long Count, long *Rate, HGLOBAL MemHandle, int Options);
    int EXTCCONV cbDOut(int BoardNum, int PortNum, USHORT DataValue);
    int EXTCCONV cbDOutScan(int BoardNum, int PortNum, long Count, long *Rate, HGLOBAL MemHandle, int Options);
    int EXTCCONV cbErrHandling(int ErrReporting, int ErrHandling);
    int EXTCCONV cbFileAInScan(int BoardNum, int LowChan, int HighChan, long Count, long *Rate, int Gain,
                               char *FileName, int Options);
    int EXTCCONV cbFileGetInfo(char *FileName, short *LowChan, short *HighChan, long *PreTrigCount, long *TotalCount,
                               long *Rate, int *Gain);
    int EXTCCONV cbFilePretrig(int BoardNum, int LowChan, int HighChan, long *PreTrigCount, long *TotalCount,
                               long *Rate, int Gain, char *FileName, int Options);
    int EXTCCONV cbFileRead(char *FileName, long FirstPoint, long *NumPoints, USHORT *DataBuffer);
    int EXTCCONV cbFlashLED(int BoardNum);
    int EXTCCONV cbGetErrMsg(int ErrCode, char *ErrMsg);
    int EXTCCONV cbGetIOStatus(int BoardNum, short *Status, long *CurCount, long *CurIndex, int FunctionType);
    int EXTCCONV cbRS485(int BoardNum, int Transmit, int Receive);
    int EXTCCONV cbStopIOBackground(int BoardNum, int FunctionType);
    int EXTCCONV cbTIn(int BoardNum, int Chan, int Scale, float *TempValue, int Options);
    int EXTCCONV cbTInScan(int BoardNum, int LowChan, int HighChan, int Scale, float *DataBuffer, int Options);
    int EXTCCONV cbMemSetDTMode(int BoardNum, int Mode);
    int EXTCCONV cbMemReset(int BoardNum);
    int EXTCCONV cbMemRead(int BoardNum, USHORT *DataBuffer, long FirstPoint, long Count);
    int EXTCCONV cbMemWrite(int BoardNum, USHORT *DataBuffer, long FirstPoint, long Count);
    int EXTCCONV cbMemReadPretrig(int BoardNum, USHORT *DataBuffer, long FirstPoint, long Count);
    int EXTCCONV cbWinBufToArray(HGLOBAL MemHandle, USHORT *DataArray, long StartPt, long Count);
    int EXTCCONV cbWinArrayToBuf(USHORT *DataArray, HGLOBAL MemHandle, long StartPt, long Count);
    ...
    int EXTCCONV cbGetConfig(int InfoType, int BoardNum, int DevNum, int ConfigItem, int *ConfigVal);
    int EXTCCONV cbSetConfig(int InfoType, int BoardNum, int DevNum, int ConfigItem, int ConfigVal);
    int EXTCCONV cbToEngUnits(int BoardNum, int Range, USHORT DataVal, float *EngUnits);
    int EXTCCONV cbFromEngUnits(int BoardNum, int Range, float EngUnits, USHORT *DataVal);
    int EXTCCONV cbGetBoardName(int BoardNum, char *BoardName);
    int EXTCCONV cbDeclareRevision(float *RevNum);
    int EXTCCONV cbSetTrigger(int BoardNum, int TrigType, USHORT LowThreshold, USHORT HighThreshold);

    int EXTCCONV cbEnableEvent(int BoardNum, unsigned EventType, unsigned Count, EVENTCALLBACK CallbackFunc,
                               void *UserData);
...
int WINAPI cbAConvertPretrigData(int BoardNum, long PreTrigCount, long TotalCount, unsigned FAR *ADData,
                                 int FAR *ChanTags);
int WINAPI cbAIn(int BoardNum, int Chan, int Gain, unsigned FAR *DataValue);
int WINAPI cbAInScan(int BoardNum, int LowChan, int HighChan, long Count, long FAR *Rate, int Gain, HGLOBAL MemHandle,
                     int Options);
...
int WINAPI cbAOutScan(int BoardNum, int LowChan, int HighChan, long Count, long FAR *Rate, int Gain, HGLOBAL MemHandle,
                      int Options);
int WINAPI cbAPretrig(int BoardNum, int LowChan, int HighChan, long FAR *PreTrigCount, long FAR *TotalCount,
                      long FAR *Rate, int Gain, HGLOBAL MemHandle, int Options);
int WINAPI cbATrig(int BoardNum, int Chan, int TrigType, unsigned TrigValue, int Gain, unsigned FAR *DataValue);
int WINAPI cbC8254Config(int BoardNum, int CounterNum, int Config);
int WINAPI cbC8536Config(int BoardNum, int CounterNum, int OutputControl, int RecycleMode, int Retrigger);
int WINAPI cbC9513Config(int BoardNum, int CounterNum, int GateControl, int CounterEdge, int CountSource,
                         int SpecialGate, int Reload, int RecycleMode, int BCDMode, int CountDirection,
                         int OutputControl);
int WINAPI cbC8536Init(int BoardNum, int ChipNum, int Ctr1Output);
int WINAPI cbC9513Init(int BoardNum, int ChipNum, int FOutDivider, int FOutSource, int Compare1, int Compare2,
                       int TimeOfDay);
int WINAPI cbCFreqIn(int BoardNum, int SigSource, int GateInterval, unsigned FAR *Count, long FAR *Freq);
int WINAPI cbCIn(int BoardNum, int CounterNum, unsigned FAR *Count);
int WINAPI cbCStoreOnInt(int BoardNum, int IntCount, int FAR *CntrControl, HGLOBAL MemHandle);
...
int WINAPI cbDInScan(int BoardNum, int PortNum, long Count, long FAR *Rate, HGLOBAL MemHandle, int Options);
int WINAPI cbDOut(int BoardNum, int PortNum, unsigned DataValue);
int WINAPI cbDOutScan(int BoardNum, int PortNum, long Count, long FAR *Rate, HGLOBAL MemHandle, int Options);
int WINAPI cbErrHandling(int ErrReporting, int ErrHandling);
int WINAPI cbFileAInScan(int BoardNum, int LowChan, int HighChan, long Count, long FAR *Rate, int Gain,
                         char FAR *FileName, int Options);
int WINAPI cbFileGetInfo(char FAR *FileName, int FAR *LowChan, int FAR *HighChan, long FAR *PreTrigCount,
                         long FAR *TotalCount, long FAR *Rate, int FAR *Gain);
int WINAPI cbFilePretrig(int BoardNum, int LowChan, int HighChan, long FAR *PreTrigCount, long FAR *TotalCount,
                         long FAR *Rate, int Gain, char FAR *FileName, int Options);
int WINAPI cbFileRead(char FAR *FileName, long FirstPoint, long FAR *NumPoints, unsigned FAR *DataBuffer);
int WINAPI cbGetErrMsg(int ErrCode, char FAR *ErrMsg);
int WINAPI cbGetRevision(float FAR *DLLRevNum, float FAR *VXDRevNum);
int WINAPI cbGetStatus(int BoardNum, int FAR *Status, long FAR *CurCount, long FAR *CurIndex);
int WINAPI cbRS485(int BoardNum, int Transmit, int Receive);
int WINAPI cbStopBackground(int BoardNum);
int WINAPI cbTIn(int BoardNum, int Chan, int Scale, float FAR *TempValue, int Options);
int WINAPI cbTInScan(int BoardNum, int LowChan, int HighChan, int Scale, float FAR *DataBuffer, int Options);
int WINAPI cbMemSetDTMode(int BoardNum, int Mode);
int WINAPI cbMemReset(int BoardNum);
int WINAPI cbMemRead(int BoardNum, unsigned FAR *DataBuffer, long FirstPoint, long Count);
int WINAPI cbMemWrite(int BoardNum, unsigned FAR *DataBuffer, long FirstPoint, long Count);
int WINAPI cbMemReadPretrig(int BoardNum, unsigned FAR *DataBuffer, long FirstPoint, long Count);
int WINAPI cbWinBufToArray(HGLOBAL MemHandle, unsigned FAR *DataArray, long StartPt, long Count);
int WINAPI cbWinArrayToBuf(unsigned FAR *DataArray, HGLOBAL MemHandle, long StartPt, long Count);
...
short _pascal cbAConvertData(short BoardNum, long int NumPoints, unsigned short *ADData, short *ChanTags);
short _pascal cbACalibrateData(short BoardNum, long int NumPoints, short Gain, unsigned int *ADData);
short _pascal cbAConvertPretrigData(short BoardNum, long int PreTrigCount, long int TotalCount, unsigned short *ADData,
                                    short *ChanTags);
short _pascal cbAIn(short BoardNum, short Chan, short Gain, unsigned short *DataValue);
short _pascal cbAInScan(short BoardNum, short LowChan, short HighChan, long int Count, long int *Rate, short Gain,
                        unsigned short MemHandle, short Options);
short _pascal cbALoadQueue(short BoardNum, short *ChanArray, short *GainArray, unsigned short NumChans);
short _pascal cbAOut(short BoardNum, short Chan, short Gain, unsigned short DataValue);
short _pascal cbAOutScan(short BoardNum, short LowChan, short HighChan, long int Count, long int *Rate, short Gain,
                         unsigned short MemHandle, short Options);
short _pascal cbAPretrig(short BoardNum, short LowChan, short HighChan, long int *PreTrigCount, long int *TotalCount,
                         long int *Rate, short Gain, unsigned short MemHandle, short Options);
short _pascal cbATrig(short BoardNum, short Chan, short TrigType, unsigned short TrigValue, short Gain,
                      unsigned short *DataValue);
short _pascal cbC8254Config(short BoardNum, short CounterNum, short Config);
short _pascal cbC8536Config(short BoardNum, short CounterNum, short OutputControl, short RecycleMode, short Retrigger);
short _pascal cbC9513Config(short BoardNum, short CounterNum, short GateControl, short CounterEdge, short CountSource,
                            short SpecialGate, short Reload, short RecycleMode, short BCDMode, short CountDirection,
                            short OutputControl);
short _pascal cbC8536Init(short BoardNum, short ChipNum, short Ctr1Output);
short _pascal cbC9513Init(short BoardNum, short ChipNum, short FOutDivider, short FOutSource, short Compare1,
                          short Compare2, short TimeOfDay);
short _pascal cbCFreqIn(short BoardNum, short SigSource, short GateInterval, unsigned short *Count, long int *Freq);
short _pascal cbCIn(short BoardNum, short CounterNum, unsigned short *Count);
short _pascal cbCStoreOnInt(short BoardNum, short IntCount, short *CntrControl, unsigned short MemHandle);
short _pascal cbCLoad(short BoardNum, short RegNum, unsigned short LoadValue);
short _pascal cbDBitIn(short BoardNum, short PortType, short BitNum, short *BitValue);
short _pascal cbDBitOut(short BoardNum, short PortType, short BitNum, short BitValue);
short _pascal cbDConfigPort(short BoardNum, short PortNum, short Direction);
short _pascal cbDeclareRevision(float *RevNum);
short _pascal cbDIn(short BoardNum, short PortNum, unsigned short *DataValue);
short _pascal cbDInScan(short BoardNum, short PortNum, long int Count, long int *Rate, unsigned short MemHandle,
                        short Options);
short _pascal cbDOut(short BoardNum, short PortNum, unsigned short DataValue);
short _pascal cbDOutScan(short BoardNum, short PortNum, long int Count, long int *Rate, unsigned short MemHandle,
                         short Options);
short _pascal cbErrHandling(short ErrReporting, short ErrHandling);
short _pascal cbFileAInScan(short BoardNum, short LowChan, short HighChan, long int Count, long int *Rate, short Gain,
                            char *FileName, short Options);
short _pascal cbFileGetInfo(char *FileName, short *LowChan, short *HighChan, long int *PreTrigCount,
                            long int *TotalCount, long int *Rate, short *Gain);
short _pascal cbFilePretrig(short BoardNum, short LowChan, short HighChan, long int *PreTrigCount, long int *TotalCount,
                            long int *Rate, short Gain, char *FileName, short Options);
short _pascal cbFileRead(char *FileName, long int FirstPoint, long int *NumPoints, unsigned short *DataBuffer);
short _pascal cbGetErrMsg(short ErrCode, char *ErrMsg);
short _pascal cbGetRevision(float *DLLRevNum, float *VXDRevNum);
short _pascal cbGetStatus(short BoardNum, short *Status, long int *CurCount, long int *CurIndex);
short _pascal cbRS485(short BoardNum, short Transmit, short Receive);
short _pascal cbStopBackground(short BoardNum);
short _pascal cbTIn(short BoardNum, short Chan, short Scale, float *TempValue, short Options);
short _pascal cbTInScan(short BoardNum, short LowChan, short HighChan, short Scale, float *DataBuffer, short Options);
short _pascal cbMemSetDTMode(short BoardNum, short Mode);
short _pascal cbMemReset(short BoardNum);
short _pascal cbMemRead(short BoardNum, unsigned short *DataBuffer, long int FirstPoint, long int Count);
short _pascal cbMemWrite(short BoardNum, unsigned short *DataBuffer, long int FirstPoint, long int Count);
short _pascal cbMemReadPretrig(short BoardNum, unsigned short *DataBuffer, long int FirstPoint, long int Count);
short _pascal cbWinBufToArray(unsigned short MemHandle, unsigned short *DataArray, long int StartPt, long int Count);
short _pascal cbWinArrayToBuf(unsigned short *DataArray, unsigned short MemHandle, long int StartPt, long int Count);
...
short _pascal cbGetConfig(short InfoType, short BoardNum, short DevNum, short ConfigItem, short *ConfigVal);
short _pascal cbSetConfig(short InfoType, short BoardNum, short DevNum, short ConfigItem, short ConfigVal);
short _pascal cbToEngUnits(short BoardNum, short Range, unsigned short DataVal, float *EngUnits);
short _pascal cbFromEngUnits(short BoardNum, short Range, float EngUnits, unsigned short *DataVal);
