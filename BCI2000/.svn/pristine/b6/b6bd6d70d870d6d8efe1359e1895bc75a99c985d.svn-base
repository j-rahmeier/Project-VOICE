...
void mainUI::timerUpdate()
{
  ...
  int row = 0;
  for( std::map<std::string, std::string>::const_iterator i = states.begin(); i != states.end(); ++i )
  {
...
//---------------------------------------------------------------
void mainUI::writeValue(const std::string& name, double value)
{
  //this function writes a value to a state in BCI2000
  //Isn't this easy?!
  sendConnection << name << ' ' << value << std::endl;
}
...
void mainUI::changeStateValue(int row, int column)
{
  ...
  std::string name = stateTable->item(row,0)->text().toStdString();
  ...
  if (ok1 && ok2)
  {
    ...
    std::stringstream str;
    str << "Wrote " << name << ": "<< newValue << std::endl;
    statusBox->setText(str.str().c_str());
  }
  else
