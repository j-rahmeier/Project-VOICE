////////////////////////////////////////////////////////////////////////////////
// $Id: ChoiceCombination.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
ChoiceCombination::ChoiceCombination(const std::string &inParamName) : mChoice(0), mParamName(inParamName)
{
}
...
void ChoiceCombination::Publish()
{
...
void ChoiceCombination::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    bool configOK = true;
    if (mFilters.empty())
    {
...
void ChoiceCombination::Initialize(const SignalProperties &Input, const SignalProperties &Output)
{
    mChoice = Parameter(mParamName);
    mFilters[mChoice]->CallInitialize(Input, Output);
}

void ChoiceCombination::Process(const GenericSignal &Input, GenericSignal &Output)
{
    mFilters[mChoice]->CallProcess(Input, Output);
}

void ChoiceCombination::StartRun()
{
    mFilters[mChoice]->CallStartRun();
}

void ChoiceCombination::StopRun()
{
    mFilters[mChoice]->CallStopRun();
}

void ChoiceCombination::Resting()
{
    mFilters[mChoice]->CallResting();
}

void ChoiceCombination::Halt()
{
