////////////////////////////////////////////////////////////////////////////////
// $Id: Color.cpp 7857 2024-02-23 16:24:35Z mellinger $
...
RGBColor RGBColor::operator*(double f) const
{
    int r = ToInt(f * R()), g = ToInt(f * G()), b = ToInt(f * B());
    return RGBColor(std::min(r, 0xff), std::min(g, 0xff), std::min(b, 0xff));
}

std::ostream &RGBColor::InsertInto(std::ostream &os) const
{
...
std::istream &RGBColor::ExtractFrom(std::istream &is)
{
...
std::ostream &RGBColor::Serialize(std::ostream &os) const
{
    os.put((mValue >> 24) & 0xff)
      .put((mValue >> 16) & 0xff)
      .put((mValue >> 8) & 0xff)
      .put(mValue & 0xff);
    return os;
...
std::istream &RGBColor::Unserialize(std::istream &is)
{
    mValue = 0;
    mValue |= is.get() << 24;
    mValue |= is.get() << 16;
...
int RGBColor::ToWinColor() const
{
    return R() + (G() << 8) + (B() << 16);
}

RGBColor RGBColor::FromWinColor(int c)
{
...
RGBColor RGBColor::ToGray() const
{
    int gray = (R() + G() + B()) / 3;
    return RGBColor(gray, gray, gray);
}

RGBColor RGBColor::FromHSV(double H, double S, double V)
{
    ...
    if (!(i % 2))
        f = 1.0 - f;
    int m = ToInt((V * (1.0 - S)) * 0x100), n = ToInt((V * (1.0 - S * f)) * 0x100), v = ToInt(V * 0x100);
    if (m > 0xff)
        m = 0xff;
    if (n > 0xff)
        n = 0xff;
...
RGBColor RGBColor::FromHsv360(double H, double S, double V)
{
    return FromHSV((240 - H) / 360, S, V);
}

void RGBColor::ToHsv360(double &H, double &S, double &V)
{
