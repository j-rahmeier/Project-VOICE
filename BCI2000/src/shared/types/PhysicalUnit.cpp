////////////////////////////////////////////////////////////////////////////////
// $Id: PhysicalUnit.cpp 7820 2024-02-02 20:46:50Z mellinger $
...
const struct Prefix
{
    ...
    {
        {"f", 1e-15, -15}, {"p", 1e-12, -12},      {"n", 1e-9, -9}, {"mu", 1e-6, -6},
        {"u", 1e-6, -6},   {"\xc2\xb5", 1e-6, -6}, {"m", 1e-3, -3}, {"", 1.0, 0},
        {"k", 1e3, 3},     {"M", 1e6, 6},          {"G", 1e9, 9},   {"T", 1e12, 12},
},
  *PrefixesEnd = Prefixes + sizeof(Prefixes) / sizeof(*Prefixes);

double NumberFromString(const std::string& s)
{
    const char* begin = s.c_str();
    char* end = nullptr;
    double value = std::strtod(begin, &end);
    if (end == begin)
        throw Exception("Could not convert string into number");
    else if (end - begin != s.length())
        throw Exception("String representation contains extra characters");
    return value;
}

} // namespace

bool PhysicalUnit::TokenizePhysical(const std::string &inPhysical, size_t &outPrefixPos, size_t &outSymbolPos) const
{
...
bool PhysicalUnit::SexagesimalAllowed() const
{
...
bool PhysicalUnit::ParseNumber(const std::string &inNumber, PhysicalUnit::ValueType &outValue) const
{
    outValue = 0;
    bool valid = true;
    int count = 0;
...
bool PhysicalUnit::ApplyPrefix(const std::string &inPrefix, PhysicalUnit::ValueType &ioValue) const
{
...
void PhysicalUnit::ExtractPrefix(PhysicalUnit::ValueType inRange, PhysicalUnit::ValueType &ioValue,
                                 std::string &outPrefix) const
{
...
void PhysicalUnit::ToEngNotation(PhysicalUnit::ValueType inRange, PhysicalUnit::ValueType &ioValue,
                                 std::string &outPrefix) const
{
    int n = 0;
...
PhysicalUnit::ValueType PhysicalUnit::RawToPhysicalValue(ValueType inRawValue) const
{
    return (inRawValue - mOffset) * mGain;
}

PhysicalUnit::ValueType PhysicalUnit::PhysicalToRawValue(ValueType inPhysicalValue) const
{
    return (inPhysicalValue / mGain) + mOffset;
}

PhysicalUnit &PhysicalUnit::SetOffset(ValueType inOffset)
{
    mOffset = inOffset;
    return *this;
}

PhysicalUnit &PhysicalUnit::SetGain(ValueType inGain)
{
    mGain = inGain;
    return *this;
}

PhysicalUnit &PhysicalUnit::SetGainWithSymbol(const std::string &inGain)
{
    if (inGain.empty())
...
PhysicalUnit &PhysicalUnit::SetSymbol(const std::string &inSymbol, double inPower)
{
    mSymbolPowers.clear();
...
PhysicalUnit &PhysicalUnit::SetRawMin(ValueType v)
{
    mRawMin = v;
    mIsRangeSet = true;
    return *this;
...
PhysicalUnit &PhysicalUnit::SetRawMax(ValueType v)
{
    mRawMax = v;
    mIsRangeSet = true;
    return *this;
...
int PhysicalUnit::RawRange() const
{
    return Floor(::fabs(mRawMax - mRawMin) + 1);
}

bool PhysicalUnit::IsPhysical(const std::string &inValue) const
{
    if (Symbol().empty())
        return false;
...
PhysicalUnit::ValueType PhysicalUnit::PhysicalToRaw(const std::string &s) const
{
    ValueType value = 0;
    size_t beginPos = 0, prefixPos = 0, symbolPos = 0;
    ...
    if (value != 0) // zero times whatever is identical to zero
    {
        ...
        if (!unitOK)
        {
            std::string symbol = s.substr(symbolPos), message;
            if (symbol.empty())
...
PhysicalUnit::Pair PhysicalUnit::RawToPhysical(ValueType inRawValue, ValueType inRangeMax) const
{
...
bool PhysicalUnit::operator==(const PhysicalUnit &inUnit) const
{
    return mSymbol == inUnit.mSymbol && mOffset == inUnit.mOffset && mGain == inUnit.mGain;
}

PhysicalUnit &PhysicalUnit::operator*=(const PhysicalUnit &inUnit)
{
    ...
    mSymbol = mSymbolPowers.SingleSymbol();
    mGain *= inUnit.mGain;
    ValueType max1max2 = mRawMax * inUnit.mRawMax, max1min2 = mRawMax * inUnit.mRawMin,
              min1max2 = mRawMin * inUnit.mRawMax, min1min2 = mRawMin * inUnit.mRawMin;
...
PhysicalUnit PhysicalUnit::operator*(const PhysicalUnit &inOther) const
{
    return PhysicalUnit(*this) *= inOther;
}

PhysicalUnit &PhysicalUnit::Combine(const PhysicalUnit &inUnit)
{
    if (*this != inUnit)
    {
        mGain = 1;
...
std::ostream &PhysicalUnit::InsertInto(std::ostream &os) const
{
    return os << std::setprecision(10) << mOffset << ' ' << mGain << ' ' << mSymbol << ' ' << mRawMin << ' ' << mRawMax;
}

std::istream &PhysicalUnit::ExtractFrom(std::istream &is)
{
    EncodedString symbol;
    std::string offset, gain, rawMin, rawMax;
    is >> offset >> gain >> symbol >> rawMin >> rawMax;
    try {
        SetOffset(NumberFromString(offset));
        SetGain(NumberFromString(gain));
        SetSymbol(symbol);
        SetRawMin(NumberFromString(rawMin));
        SetRawMax(NumberFromString(rawMax));
    }
...
PhysicalUnit::SymbolPowers &PhysicalUnit::SymbolPowers::operator*=(const SymbolPowers &inP)
{
...
std::string PhysicalUnit::SymbolPowers::SingleSymbol() const
{
    std::ostringstream oss;
...
{
    return EncodedString(operator std::string());
}
...
PhysicalUnit::Pair PhysicalUnit::Pair::Utf8() const
{
    if (unit.length() < 2)
        return *this;
