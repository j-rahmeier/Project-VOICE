////////////////////////////////////////////////////////////////////////////////
// $Id: Param.h 7922 2024-03-12 17:12:33Z mellinger $
...
class Param
{
  ...
  public:
    class ParamValue
    {
      ...
      public:
        ...
        ParamValue() : mNative(string), mpString(new EncodedString), mpParam(NULL)
        {
        }
        ParamValue(const ParamValue &p) : mNative(none), mpString(NULL), mpParam(NULL)
        {
            Assign(p);
        }
        ParamValue(const char *s) : mNative(string), mpString(new EncodedString(s)), mpParam(NULL)
        {
        }
        ParamValue(const std::string &s) : mNative(string), mpString(new EncodedString(s)), mpParam(NULL)
        {
        }
        ParamValue(const Param &p) : mNative(parameter), mpString(NULL), mpParam(new Param(p))
        {
        }
        ~ParamValue()
        {
        ...
        operator const StringUtils::String &() const
        {
            return AsString();
        }
        ...
        const StringUtils::String &AsString() const;
  ...
  public:
    ...
    Param();
    Param(const std::string &path, const std::string &section, const std::string &type = "int",
          const std::string &value = "0", const std::string &defaultvalue = "0", const std::string &lowrange = "0",
          const std::string &highrange = "0", const std::string &comment = "");
    // deprecated, use Param::fromDefinition() instead
    explicit Param(const std::string &parameterDefinition);
    ~Param()
    {
    }
    // copy the entire parameter will all fields
    Param &operator=(const Param &);
    // only copy parameter value(s)
    Param &AssignValues(const Param &, bool setChanged = false);

    bool Readonly() const // corresponds to the (readonly) flag in comment
    {
        return mReadonly;
    }
    bool AllowOverride() const // corresponds to the (allow_override) flag in comment
    {
        return mAllowOverride;
    }
    bool ParentScope() const // corresponds to the (parent_scope) flag in comment
    {
        return mParentScope;
    }
    // Sections
    ...
    // ParentPath
    Param &SetParentPath(const std::string &s);
  ...
  private:
    Param &SetDimensionsInternal(size_t rows, size_t cols);
  ...
  private:
    ...
    bool IsTypeCompatible(const std::string &) const;
  ...
  private:
    ...
    EncodedString mName, mParentPath, mType, mDefaultValue, mLowRange, mHighRange;
    std::string mComment;
    LabelIndex mDim1Index, mDim2Index;
  ...
  private:
    ...
    static void tolower(StringUtils::String &);
  ...
  public:
    class NameCmp
    {
      public:
        ...
        {
            return std::lexicographical_compare(a.begin(), a.end(), b.begin(), b.end(), ciless);
        }
    };
};
