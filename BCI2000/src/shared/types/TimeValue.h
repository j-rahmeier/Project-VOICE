////////////////////////////////////////////////////////////////////////////////
// $Id: TimeValue.h 7195 2023-02-07 18:31:03Z mellinger $
...
class TimeValue
{
  public:
    ...
    {
        int hours = mValue / 3600, mins = (mValue - 3600 * hours) / 60, secs = mValue % 60;
        return os << std::setw(2) << std::setfill('0') << hours << ':' << std::setw(2) << mins << ':' << std::setw(2)
                  << secs;
    }
...
inline std::ostream &operator<<(std::ostream &os, const TimeValue &t)
{
    return t.InsertInto(os);
}

inline std::istream &operator>>(std::istream &is, TimeValue &t)
{
