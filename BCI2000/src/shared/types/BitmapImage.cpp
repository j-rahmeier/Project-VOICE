////////////////////////////////////////////////////////////////////////////////
// $Id: BitmapImage.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
BitmapImage::BitmapImage(int inWidth, int inHeight)
    : mWidth(inWidth), mHeight(inHeight), mpData(new uint16_t[inWidth * inHeight])
{
    SetTransparent();
}

BitmapImage::BitmapImage(const BitmapImage &b)
    : mWidth(b.mWidth), mHeight(b.mHeight), mpData(new uint16_t[mWidth * mHeight])
{
...
BitmapImage &BitmapImage::SetBlack()
{
    std::memset(mpData, 0, mWidth * mHeight * sizeof(*mpData));
    return *this;
}

BitmapImage &BitmapImage::SetTransparent()
{
    std::memset(mpData, uint16_t(-1), mWidth * mHeight * sizeof(*mpData));
    return *this;
}

std::ostream &BitmapImage::Serialize(std::ostream &os) const
{
    os.put(mWidth & 0xff).put(mWidth >> 8).put(mHeight & 0xff).put(mHeight >> 8);
    // RLE compression
    uint16_t *pData = mpData, *pEnd = mpData + mWidth * mHeight;
    while (pData < pEnd)
    {
        int value = *pData, length = 0;
...
std::istream &BitmapImage::Unserialize(std::istream &is)
{
