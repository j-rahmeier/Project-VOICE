////////////////////////////////////////////////////////////////////////////////
// $Id: LengthField.h 7195 2023-02-07 18:31:03Z mellinger $
...
template <int NumBytes> class LengthField
{
  public:
    ...
    template <typename T> LengthField &operator=(T value)
    {
        mValue = size_t(value);
        return *this;
    }
...
template <int NumBytes> std::ostream &operator<<(std::ostream &os, const LengthField<NumBytes> &l)
{
    return l.InsertInto(os);
}

template <int NumBytes> std::istream &operator>>(std::istream &is, LengthField<NumBytes> &l)
{
    return l.ExtractFrom(is);
}

template <int NumBytes> std::ostream &LengthField<NumBytes>::Serialize(std::ostream &os) const
{
    size_t value = mValue;
...
template <int NumBytes> std::istream &LengthField<NumBytes>::Unserialize(std::istream &is)
{
    mValue = 0;
