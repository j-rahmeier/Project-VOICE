////////////////////////////////////////////////////////////////////////////////
// $Id: ValueList.h 7695 2023-11-06 17:13:42Z mellinger $
...
template <typename T> class ValueList : public std::vector<T>
{
  public:
    ...
    void Resize(size_t inSize, T inValue = 0)
    {
        this->resize(inSize, inValue);
    }
    int Size() const
    {
...
template <typename T> std::istream &ValueList<T>::ExtractFrom(std::istream &is)
{
    this->clear();
    is >> std::ws;
...
template <typename T> std::ostream &ValueList<T>::InsertInto(std::ostream &os) const
{
...
template <typename T> inline std::ostream &operator<<(std::ostream &s, const ValueList<T> &v)
{
    return v.InsertInto(s);
}

template <typename T> inline std::istream &operator>>(std::istream &s, ValueList<T> &v)
{
