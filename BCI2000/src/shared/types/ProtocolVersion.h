////////////////////////////////////////////////////////////////////////////////
// $Id: ProtocolVersion.h 7985 2024-04-03 11:07:24Z mellinger $
...
class ProtocolVersion
{
    ...
    static const Version *History()
    {
        static const Version v[] = {
            {2, 5, "State2 messages containing kind information"},
            {2, 4, "VisVideoFrame messages"},
  ...
  public:
    enum
    {
        ...
        VisVideoFrameMessages,
        State2Messages, // containing kind information
    };

    ProtocolVersion() : mMajor(0), mMinor(0)
    {
    }
    ~ProtocolVersion()
    {
...
inline bool ProtocolVersion::Provides(int feature) const
{
    switch (feature)
    {
    ...
    case VisVideoFrameMessages:
        return AtLeast(ProtocolVersion(2, 4));
    case State2Messages:
        return AtLeast(ProtocolVersion(2, 5));
    }
    return false;
}

inline std::ostream &ProtocolVersion::Serialize(std::ostream &os) const
{
    os << mMajor;
    if (mMinor)
        os << "." << mMinor;
...
inline std::istream &ProtocolVersion::ExtractFrom(std::istream &is)
{
    mMinor = 0;
    is >> mMajor;
    if (is.peek() == '.')
...
inline std::ostream &operator<<(std::ostream &os, const ProtocolVersion &s)
{
    return s.InsertInto(os);
}

inline std::istream &operator>>(std::istream &is, ProtocolVersion &s)
{
