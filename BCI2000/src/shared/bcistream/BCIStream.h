////////////////////////////////////////////////////////////////////////////////
// $Id: BCIStream.h 8066 2024-04-23 22:47:26Z mellinger $
...
#define bcinote bcinote__.Locked()(0)
...
#define BCISTREAM_DBGLEVEL_                                                                                            \
    BCIStream::IsSet<bcidbgOff>::Value                                                                                 \
        ? BCIStream::OutStream::NeverDisplayMessage                                                                    \
...
void NoteMessage(const std::string &);
...
class OutStream : public Lockable<>, public std::ostream
{
  ...
  public:
    ...
    {
      public:
        Ref(OutStream &os) : p(&os)
        {
            p->Lock();
        }
        Ref(const Ref &other) : p(other.p)
        {
            const_cast<Ref &>(other).p = 0;
        }
        ~Ref()
        {
            if (p)
        ...
        bool RefersTo(const OutStream &os) const
        {
            return p == &os;
        }
        ...
        {
            if (p)
                static_cast<std::ostream &>(*p) << t;
            return *this;
        }
        Ref &operator<<(std::ostream &f(std::ostream &))
        {
            if (p)
                static_cast<std::ostream &>(*p) << f;
            return *this;
        }
        Ref &operator()(int level)
        {
            if (p)
                p->MessageVerbosity(level);
            return *this;
        }
  ...
  private:
    ...
    static thread_local std::list<std::string> stContext;
};

class ContextFrame
{
  public:
    ContextFrame(const std::string &s) : mCopied(false)
    {
        OutStream::stContext.push_back(s);
    }
    ContextFrame(ContextFrame &c) : mCopied(false)
    {
        c.mCopied = true;
    }
    ~ContextFrame()
...
{
  public:
    Dispatcher(BCIStream::Action a = LogicError) : mAction(a)
    {
    }
    virtual ~Dispatcher()
    {
...
#define bcierr__ bcierr___()
#define bciout__ bciout___()
#define bciwarn__ bciwarn___()
#define bcidbg__ bcidbg___()
#define bcinote__ bcinote___()

BCIStream::OutStream &bcierr___();
void bcierr___(BCIStream::OutStream &);
BCIStream::OutStream &bciout___();
void bciout___(BCIStream::OutStream &);
BCIStream::OutStream &bciwarn___();
void bciwarn___(BCIStream::OutStream &);
BCIStream::OutStream &bcidbg___();
void bcidbg___(BCIStream::OutStream &);
BCIStream::OutStream &bcinote___();
void bcinote___(BCIStream::OutStream&);
