////////////////////////////////////////////////////////////////////////////////
// $Id: BCIRegistry.cpp 7711 2023-11-10 18:39:15Z mellinger $
...
#define RegisterFilter_(name, pos, priority) extern "C" std::ptrdiff_t FilterObjectName_(name, pos, priority);

#undef RegisterExtension_
#define RegisterExtension_(x) extern "C" std::ptrdiff_t ExtensionObjectName_(x);

#undef bcitest
#define bcitest(x) extern "C" std::ptrdiff_t TestObjectName_(x);

#include INCLUDE_FILE(REGISTRY_NAME)

// Re-define second-level macros to create code referencing global variables.
#undef RegisterFilter_
#define RegisterFilter_(name, pos, priority) result += std::ptrdiff_t(FilterObjectName_(name, pos, priority));

#undef RegisterExtension_
#define RegisterExtension_(x) result += std::ptrdiff_t(ExtensionObjectName_(x));

#undef bcitest
#define bcitest(x) result += std::ptrdiff_t(TestObjectName_(x));
