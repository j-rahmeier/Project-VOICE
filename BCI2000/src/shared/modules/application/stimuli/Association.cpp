...
////////////////////////////////////////////////////////////////////////////////
// $Id: Association.cpp 7990 2024-04-03 20:43:04Z mellinger $
...
Association::Association()
    : mStimulusDuration(-1), mISIMinDuration(-1), mISIMaxDuration(-1), mHasEarlyOffsetExpression(false),
      mEarlyOffsetExpression()
{
}

Association &Association::SetStimulusDuration(int i)
{
    mStimulusDuration = i;
    return *this;
}

int Association::StimulusDuration() const
{
    return mStimulusDuration;
}

Association &Association::SetISIMinDuration(int i)
{
    mISIMinDuration = i;
    return *this;
}

int Association::ISIMinDuration() const
{
    return mISIMinDuration;
}

Association &Association::SetISIMaxDuration(int i)
{
    mISIMaxDuration = i;
    return *this;
}

int Association::ISIMaxDuration() const
{
    return mISIMaxDuration;
}

Association &Association::SetEarlyOffsetExpression(const std::string &inExpr)
{
...
const Expression &Association::EarlyOffsetExpression() const
{
    return mEarlyOffsetExpression;
}

bool Association::HasEarlyOffsetExpression() const
{
...
Association &Association::DeleteObjects()
{
...
Association &Association::Add(Stimulus *s)
{
    mStimuli.Add(s);
    return *this;
}

Association &Association::Remove(Stimulus *s)
{
    mStimuli.Remove(s);
    return *this;
}

bool Association::Contains(Stimulus *s) const
{
    return mStimuli.Contains(s);
}

bool Association::Intersects(const SetOfStimuli &s) const
{
    return mStimuli.Intersects(s);
}

Association &Association::Add(Target *t)
{
    mTargets.Add(t);
    return *this;
}

Association &Association::Remove(Target *t)
{
    mTargets.Remove(t);
    return *this;
}

bool Association::Contains(Target *t) const
{
    return mTargets.Contains(t);
}

bool Association::Intersects(const SetOfTargets &s) const
{
    return mTargets.Intersects(s);
}

Association &Association::Present()
{
    mStimuli.Present();
    return *this;
}

Association &Association::Conceal()
{
    mStimuli.Conceal();
    return *this;
}

Association &Association::Stop()
{
    mStimuli.Stop();
    return *this;
}
...
SetOfStimuli &Association::Stimuli()
{
    return mStimuli;
}

const SetOfStimuli &Association::Stimuli() const
{
    return mStimuli;
}

SetOfTargets &Association::Targets()
{
    return mTargets;
}

const SetOfTargets &Association::Targets() const
{
    return mTargets;
}

// TargetClassification
Target *TargetClassification::MostLikelyTarget(double &outEvidence) const
{
    Target *result = NULL;
    double maxScore = std::numeric_limits<double>::quiet_NaN(), secondMaxScore = maxScore;
    for (const_iterator i = begin(); i != end(); ++i)
    {
        if (!(maxScore > i->second)) // NaN comparison is always false
        {
            result = i->first;
            secondMaxScore = maxScore;
            maxScore = i->second;
        }
        else if (!(secondMaxScore > i->second))
            secondMaxScore = i->second;
    }
    outEvidence = maxScore;
    if (secondMaxScore <= maxScore) // could be NaN
        outEvidence -= secondMaxScore;
    return result;
}
...
TargetClassification AssociationMap::ClassifyTargets(const ClassResult &inResult)
{
...
SetOfStimuli AssociationMap::TargetToStimuli(Target *in_pTarget) const
{
    SetOfStimuli result;
...
bool AssociationMap::StimuliIntersect(int s1, int s2)
{
    return s1 == s2 || (*this)[s1].Intersects((*this)[s2].Stimuli());
}

bool AssociationMap::TargetsIntersect(int s1, int s2)
{
