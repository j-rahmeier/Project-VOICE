////////////////////////////////////////////////////////////////////////////////
// $Id: TextWindow.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
TextWindow::TextWindow() : mpForm(new QWidget()), mpEditField(new QTextEdit(mpForm))
{
...
TextWindow &TextWindow::Show()
{
    mpForm->show();
    return *this;
}

TextWindow &TextWindow::Hide()
{
    mpForm->hide();
    return *this;
}

TextWindow &TextWindow::SetLeft(int inLeft)
{
    mpForm->move(inLeft, mpForm->y());
    return *this;
}

int TextWindow::Left() const
{
    return mpForm->x();
}

TextWindow &TextWindow::SetTop(int inTop)
{
    mpForm->move(mpForm->x(), inTop);
    return *this;
}

int TextWindow::Top() const
{
    return mpForm->y();
}

TextWindow &TextWindow::SetWidth(int inWidth)
{
...
TextWindow &TextWindow::SetHeight(int inHeight)
{
...
TextWindow &TextWindow::SetText(const std::string &inText)
{
...
const std::string &TextWindow::Text() const
{
    mTextBuf = mpEditField->toPlainText().toLocal8Bit().constData();
    return mTextBuf;
}

TextWindow &TextWindow::SetFontName(const std::string &inFontName)
{
    mpEditField->setFontFamily(QString::fromLocal8Bit(inFontName.c_str()));
    return *this;
}

const std::string &TextWindow::FontName() const
{
    mTextBuf = mpEditField->font().family().toLocal8Bit().constData();
    return mTextBuf;
}

TextWindow &TextWindow::SetFontSize(int inSize)
{
    mpEditField->setFontPointSize(inSize);
    return *this;
}

float TextWindow::FontSize() const
{
