////////////////////////////////////////////////////////////////////////////////
// $Id: SpellerCommand.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
// Formatted IO
std::istream &SpellerCommand::ExtractFrom(std::istream &is)
{
    mValue = "";
    mCode = "";
    ...
    if (is && mValue.empty())
    {
        ...
        std::string::iterator i = mCode.begin();
        while (i != mCode.end() && ::isalpha(*i))
        {
            *i = ::toupper(*i);
            ++i;
        }
        mValue = std::string(i, mCode.end());
        mCode = std::string(mCode.begin(), i);
    }
    return is;
}

std::ostream &SpellerCommand::InsertInto(std::ostream &os) const
{
    if (mCode.empty())
        os << mValue;
    else
