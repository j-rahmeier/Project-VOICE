////////////////////////////////////////////////////////////////////////////////
// $Id: Speller.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
// Properties
Speller &Speller::Add(SpellerTarget *s)
{
    mTargets.insert(s);
    return *this;
}

Speller &Speller::Remove(SpellerTarget *s)
{
    mTargets.erase(s);
    return *this;
}

Speller &Speller::DeleteObjects()
{
...
const SetOfSpellerTargets &Speller::Targets() const
{
    return mTargets;
}

// Target suggestion
SpellerTarget *Speller::SuggestTarget(const std::string &inFrom, const std::string &inTo) const
{
    ...
    struct
    {
        int operator()(const std::string &from, const std::string &to)
        {
            int result = static_cast<int>(from.length() + to.length());
            std::string::const_iterator p1 = from.begin(), p2 = to.begin();
    ...
    std::map<int, SpellerTarget *> sortedTargets;
...
bool Speller::TrySpelling(const std::string &inText, SequenceOfSpellerTargets *pOutSequence)
{
    ...
    std::string remainingText = inText;
    ...
    do
    {
        ...
        {
            const std::string &entryText = pSuggestedTarget->EntryText();
...
// Event triggering
Speller &Speller::Enter(const std::string &s)
{
    OnEnter(s);
    return *this;
}
...
SpellerTarget::SpellerTarget(Speller &s) : mSpeller(s)
{
    mSpeller.Add(this);
}
...
// Properties
SpellerTarget &SpellerTarget::SetEntryText(const std::string &s)
{
    mEntryText = s;
    return *this;
}

const std::string &SpellerTarget::EntryText() const
{
    return mEntryText;
}
