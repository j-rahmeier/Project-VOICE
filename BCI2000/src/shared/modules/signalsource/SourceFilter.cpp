////////////////////////////////////////////////////////////////////////////////
// $Id: SourceFilter.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
void SourceFilter::DesignFilter(SignalProperties &ioSignal, Real &outGain, ComplexVector &outZeros,
                                ComplexVector &outPoles) const
{
    ...
    { // Configure notch filter
        ...
        default:
            bcierr << "Unknown value of NotchFilter parameter";
        }
        ...
        if (notchFilter != disabled)
        {
            ...
            else
            {
                TransferFunction notch =
                    FilterDesign::Chebyshev().Ripple_dB(-0.1).Order(3).Bandstop(corner1, corner2).TransferFunction();
                tf *= notch;
    ...
    { // Configure HP filter
        ...
        default:
            bcierr << "Unknown value of HighPassFilter parameter";
        }
        if (highPassFilter != disabled)
        {
            ...
            {
                TransferFunction hp = FilterDesign::Butterworth().Order(1).Highpass(corner).TransferFunction();
                tf *= hp;
    ...
    { // Configure LP filter
        ...
        default:
            bcierr << "Unknown value of LowPassFilter parameter";
        }
        if (lowPassFilter != disabled)
        {
            ...
            {
                TransferFunction lp = FilterDesign::Butterworth().Order(2).Lowpass(corner).TransferFunction();
                tf *= lp;
...
void SourceFilter::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    std::set<int> dummy;
    Omit(Input, dummy);
...
void SourceFilter::Initialize(const SignalProperties &Input, const SignalProperties &Output)
{
    Omit(Input, mUnfilteredChannels);
    IIRFilterBase::Initialize(Input, Output);
}

void SourceFilter::Omit(const SignalProperties &Input, std::set<int> &channelsToOmit) const
{
    std::string paramName = "UnfilteredChannels";
        ...
        for (int i = 0; i < omit->NumValues(); i++)
        {
            ...
            if (channelIndex < 0)
            {
                ...
                for (int li = 0; li < channelNames->NumValues(); li++)
                    if (channelNames(li) == channelSpec)
                    {
                        channelIndex = li;
                        break;
                    }
            ...
            if (channelIndex < 0)
                bcierr << "invalid or unrecognized channel \"" << channelSpec << "\" in " << paramName << " parameter";
            channelsToOmit.insert(channelIndex);
        }
    }
}

void SourceFilter::Process(const GenericSignal &Input, GenericSignal &Output)
{
