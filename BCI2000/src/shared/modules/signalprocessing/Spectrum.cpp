////////////////////////////////////////////////////////////////////////////////
// $Id: Spectrum.cpp 7827 2024-02-08 15:13:29Z mellinger $
...
int Spectrum::NumberOfBins(const SignalProperties &Input) const
{
    ...
    if (firstBinCenter > 0.5 || firstBinCenter < 0 || lastBinCenter > 0.5 || lastBinCenter < 0)
    ...
    if (firstBinCenter > lastBinCenter)
        bcierr << "FirstBinCenter cannot be greater than LastBinCenter";
    int numBins = 0;
    if (binWidth <= 0)
        bcierr << "BinWidth must be greater zero";
    else
        numBins = Floor((lastBinCenter - firstBinCenter + Epsilon(firstBinCenter)) / binWidth + 1);
    return std::max(0, numBins);
}

void Spectrum::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    Output = Input;
    if (Input.SamplingRate() < Epsilon(Input.SamplingRate()))
    {
        bcierr << "Zero input sampling rate";
    }
    else
    {
        Output.SetElements(NumberOfBins(Input));
        Output.ElementUnit()
            .SetOffset(-Parameter("FirstBinCenter").InHertz() / Parameter("BinWidth").InHertz())
        ...
        double inputAmplitude = (Input.ValueUnit().RawMax() - Input.ValueUnit().RawMin()) / 2,
               expectedPower = inputAmplitude * inputAmplitude /
                               3; // Constant spectral power of uniform white noise with input amplitude.
        expectedPower *= 10;      // Account for a peaked spectrum.
        switch (int(Parameter("OutputType")))
        {
        case SpectralAmplitude:
            Output.ValueUnit().SetOffset(0).SetGain(1).SetSymbol("muV").SetRawMax(::sqrt(expectedPower));
            Output.SetName("Amplitude Spectrum");
            break;

        case SpectralPower:
            Output.ValueUnit().SetOffset(0).SetGain(1).SetSymbol("muV^2").SetRawMax(expectedPower);
