////////////////////////////////////////////////////////////////////////////////
// $Id: LinearClassifier.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
void LinearClassifier::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    // Determine the classifier matrix format:
    std::map<int, double> outputMax, outputMin;
    double totalOutputMax = 0.0, totalOutputMin = 0.0;
    ...
    else
    {
        ...
        {
            if (bcierr__.Flushes() > 10) // avoid hogging the log
                break;
            ...
            if (std::min(::fmod(ch, 1.0), 1 - ::fmod(ch, 1.0)) > 1e-2)
                bciout << "Channel specification in physical units:\n\t" << DescribeEntry(row, 0)
                       << "\nis not an exact match, using input channel " << Input.ChannelLabels()[Round(ch)] << " ("
            ...
            if (std::min(::fmod(el, 1.0), 1 - ::fmod(el, 1.0)) > 1e-2)
                bciout << "Specification in physical units:\n\t" << DescribeEntry(row, 1)
                       << "\nis not an exact match, using input element " << Input.ElementLabels()[Round(el)] << " ("
            ...
            if (weight > 0)
            {
                outputMax[outputChannel] += std::max(weight * Input.ValueUnit().RawMax(), 0.0);
                outputMin[outputChannel] += std::min(weight * Input.ValueUnit().RawMin(), 0.0);
            }
            else
            {
                outputMax[outputChannel] += std::max(weight * Input.ValueUnit().RawMin(), 0.0);
                outputMin[outputChannel] += std::min(weight * Input.ValueUnit().RawMax(), 0.0);
            }
    ...
    Output.ValueUnit().SetRawMin(totalOutputMin).SetRawMax(totalOutputMax);
...
void LinearClassifier::Initialize(const SignalProperties &Input, const SignalProperties & /*Output*/)
{
    const ParamRef &Classifier = Parameter("Classifier");
...
void LinearClassifier::Process(const GenericSignal &Input, GenericSignal &Output)
{
    ...
    for (size_t i = 0; i < mWeights.size(); ++i)
        Output(mOutputChannels[i], 0) += Input(mInputChannels[i], mInputElements[i]) * mWeights[i];
}

std::string LinearClassifier::DescribeEntry(int inRow, int inCol) const
{
    ParamRef Classifier = Parameter("Classifier");
    std::ostringstream oss;
    oss << "Classifier(" << Classifier->RowLabels()[inRow] << "," << Classifier->ColumnLabels()[inCol]
        << ")=" << Classifier(inRow, inCol);
    return oss.str();
}

int LinearClassifier::Round(double inValue)
{
