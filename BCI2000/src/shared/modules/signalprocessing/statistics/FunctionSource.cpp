...
FunctionSource *FunctionSource::NewFunctionSource(const std::string &inName, const NodeList &inArgs,
                                                  const int &inrDataIndex)
{
    const struct
    {
        const char *name;
        ...
        {"RSquared", StatisticalObserver::RSquared_},
        {"ZScore", StatisticalObserver::ZScore_},
        {"Count", StatisticalObserver::Count},
        {"Mean", StatisticalObserver::Mean},
        {"Variance", StatisticalObserver::Variance},
        {"Covariance", StatisticalObserver::Covariance},
        {"Correlation", StatisticalObserver::Correlation},
        {"CentralMoment", StatisticalObserver::CentralMoment},
        {"Skewness", StatisticalObserver::Skewness},
        {"Kurtosis", StatisticalObserver::Kurtosis},
        {"Histogram", StatisticalObserver::Histogram},
        {"Quantile", StatisticalObserver::Quantile},
        {"QQuantiles", StatisticalObserver::QQuantiles},
    };
    const size_t count = sizeof(names) / sizeof(*names);
    int functionID = StatisticalObserver::None;
    for (size_t i = 0; i < count && functionID == StatisticalObserver::None; ++i)
        if (inName == names[i].name)
            functionID = names[i].functionID;
    return functionID == StatisticalObserver::None ? NULL
                                                   : new FunctionSource(inName, functionID, inArgs, inrDataIndex);
}

FunctionSource::FunctionSource(const std::string &inName, int inFunctionID, const NodeList &inArgs,
                               const int &inrDataIndex)
    : DataSource(inName), mFunctionID(inFunctionID), mrDataIndex(inrDataIndex), mReturnType(TypeNone), mResultSize(0),
      mIndexArgs(0), mArgsChanged(false), mPreviousIndex(0), mNumberResult(0)
{
    const struct
    {
        int functionID, numObservers, numArgs, returnType;
    } functions[] = {
        {StatisticalObserver::RSquared_, 2, 0, TypeMatrix},      {StatisticalObserver::ZScore_, 2, 0, TypeMatrix},
        {StatisticalObserver::Count, 1, 0, TypeNumber},          {StatisticalObserver::Mean, 1, 0, TypeVector},
        {StatisticalObserver::Variance, 1, 0, TypeVector},       {StatisticalObserver::Covariance, 1, 0, TypeSymMatrix},
        {StatisticalObserver::Correlation, 1, 0, TypeSymMatrix}, {StatisticalObserver::CentralMoment, 1, 1, TypeVector},
        {StatisticalObserver::Skewness, 1, 0, TypeVector},       {StatisticalObserver::Kurtosis, 1, 0, TypeVector},
        {StatisticalObserver::Histogram, 1, 3, TypeMatrix},      {StatisticalObserver::Quantile, 1, 1, TypeVector},
        {StatisticalObserver::QQuantiles, 1, 1, TypeMatrix},
    };
    ...
    std::vector<ObserverNode *> observerNodes(functions[functionIdx].numObservers);
    for (size_t i = 0; i < std::min(inArgs.size(), observerNodes.size()); ++i)
    ...
    switch (mFunctionID)
    {
    case StatisticalObserver::RSquared_:
    case StatisticalObserver::ZScore_:
    ...
    int minArgs = functions[functionIdx].numArgs, maxArgs = minArgs + resultDimensions;
    int remainingArgs = static_cast<int>(inArgs.size() - mObservers.size());
    if (remainingArgs < minArgs || remainingArgs > maxArgs)
        throw std_invalid_argument << inName << "(): Expected between " << minArgs << " and " << maxArgs
                                   << " non-observer arguments, got " << remainingArgs;
    ...
    if (mReturnType == TypeSymMatrix && mObservers[0]->Sources().size() == 2 && mIndexArgs == 0)
    {
        for (int i = 0; i < 2; ++i)
...
void FunctionSource::OnInitialize(const Context &inContext)
{
    std::vector<DataProperties> observationProperties(mObservers.size());
    for (size_t i = 0; i < mObservers.size(); ++i)
    {
    ...
    mResultProperties.Unit().SetOffset(0).SetGain(1).SetRawMin(0).SetRawMax(1);
    switch (mReturnType)
    {
    case TypeNumber:
        break;
    ...
    case TypeMatrix:
        ...
        case StatisticalObserver::Histogram: {
            for (size_t i = 0; i < 3; ++i)
                if (!mChildren[i]->IsConst())
                    throw bciexception << "First 3 arguments to " << Name() << " function must be constant";
            double center = mChildren[0]->Evaluate(), resolution = mChildren[1]->Evaluate();
        ...
        case StatisticalObserver::QQuantiles: {
            ...
            Dimension d;
            d.labels.Resize(q + 1);
            d.unit.SetGain(1.0 / q).SetOffset(0).SetRawMin(0).SetRawMax(q);
            for (int i = 0; i <= q; ++i)
            {
    ...
    size_t resultDims = mResultProperties.Dimensions().size(), observerDims = dims.size();
    ...
    mResultSize = std::max(1, mResultSize);
    ...
    case StatisticalObserver::ZScore_:
        ...
        break;
    case StatisticalObserver::Count:
    case StatisticalObserver::Histogram:
        u.SetRawMin(0).SetRawMax(mObservers[0]->Window() / mChildren[2]->Evaluate() * 10);
        break;
    case StatisticalObserver::Mean:
    case StatisticalObserver::Quantile:
    case StatisticalObserver::QQuantiles:
        u = observationProperties[0].Unit();
        break;
    case StatisticalObserver::Variance:
    case StatisticalObserver::Covariance:
        u = observationProperties[0].Unit();
        u *= u;
        break;
    case StatisticalObserver::Correlation:
        u.SetRawMin(-1).SetRawMax(1);
        break;
    case StatisticalObserver::CentralMoment:
        break;
    case StatisticalObserver::Skewness:
        u.SetRawMin(-10).SetRawMax(10);
        break;
    case StatisticalObserver::Kurtosis:
...
void FunctionSource::OnProcess(const Context &inContext)
{
    mArgsChanged = true;
    mPreviousIndex = std::numeric_limits<int>::max();
}

DataSource::Value FunctionSource::OnData(int inIdx)
{
    int observerIdx = inIdx / mResultSize, remIdx = inIdx % mResultSize;
...
double FunctionSource::OnEvaluate()
{
...
void FunctionSource::Compute(int inObserverIdx)
{
    Assert(mObservers.size() <= 2);
    const StatisticalObserver::Observer *pObserver1 = NULL, *pObserver2 = NULL;
    ...
    case StatisticalObserver::Kurtosis:
        ...
        break;
    case StatisticalObserver::Histogram: {
        StatisticalObserver::Vector edges =
            StatisticalObserver::BinEdges(mArgBuffer[0], mArgBuffer[1], static_cast<int>(mArgBuffer[2]));
...
void FunctionSource::CheckSizeArg(int inPos, int inValue, int inSize) const
{
    if (inValue < 0 || inSize >= 0 && inValue >= inSize)
    {
        std::ostringstream oss;
        oss << "Argument #" << inPos + 1 << " out of bounds in function " << Name() << ": is " << inValue + 1
            << ", should be >= 1";
        if (inSize >= 0)
            oss << " and " << inSize;
