////////////////////////////////////////////////////////////////////////////////
//  $Id: ObserverSource.h 7195 2023-02-07 18:31:03Z mellinger $
...
class ObserverNode : public ExpressionParser::Node
{
  public:
    ...
    class ObserverSource *ObserverSource() const
    {
        return mpObserverSource;
    }
...
{
  public:
    ObserverSource(const std::string &name, const std::string &when, const std::string reset, double window,
                   bool weighted);
    ~ObserverSource();
    ...
    void Request(int inConfig)
    {
        mConfig |= inConfig;
    }

    const DimensionList &SampleDimensions() const
    {
        return mSampleDimensions;
    }
    const DimensionList &OuterDimensions() const
    {
        return mOuterDimensions;
    }
  ...
  private:
    // Configuration
    Expression mWhen, mReset;
    double mWindow;
    bool mObserveWeighted;
    int mConfig;

    DimensionList mSampleDimensions, mOuterDimensions;
    std::vector<StatisticalObserver::Observer *> mObservers;
    int mSampleBlockSize, mStreamingMax;
