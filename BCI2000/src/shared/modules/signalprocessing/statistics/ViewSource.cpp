////////////////////////////////////////////////////////////////////////////////
//  $Id: ViewSource.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
ViewSource::ViewSource(const std::string &inName) : DataSource(inName)
{
}

void ViewSource::OnInitialize(const Context &)
{
    mElements.clear();
    ...
    for (size_t i = 1; i < Sources().size(); ++i)
    {
        ...
        if (s.Channels() != output.Channels())
        {
            bcierr << Name() << " View: Cannot concatenate data with differing number of channels"
                   << " when adding \"" << Sources()[i]->Name() << "\", which has " << s.Channels() << " channels";
        }
        else
        {
...
void ViewSource::OnProcess(const Context &)
{
...
DataSource::Value ViewSource::OnData(int inIdx)
{
    int ch = inIdx / mSignal.Elements(), el = inIdx % mSignal.Elements();
    return mSignal(ch, el);
}

void ViewSource::AppendElements(SignalProperties &ioSp, const SignalProperties &inSp)
{
    ...
    int numEl1 = ioSp.Elements(), numEl2 = inSp.Elements();
