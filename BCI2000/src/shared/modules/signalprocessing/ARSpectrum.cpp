////////////////////////////////////////////////////////////////////////////////
// $Id: ARSpectrum.cpp 7268 2023-03-22 19:12:22Z mellinger $
...
void ARSpectrum::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    ...
    if (Parameter("OutputType") == Coefficients)
    {
        ...
        Output.ElementUnit().SetSymbol("").SetOffset(0).SetGain(1).SetRawMin(0).SetRawMax(Output.Elements() - 1);
    }
    else
    {
...
void ARSpectrum::Initialize(const SignalProperties &Input, const SignalProperties &Output)
{
    mInputs.clear();
...
void ARSpectrum::Process(const GenericSignal &Input, GenericSignal &Output)
{
...
#pragma omp parallel for
    for (int ch = 0; ch < Input.Channels(); ++ch)
    {
        ...
        case SpectralPower:
            ...
            for (size_t bin = 0; bin < mSpectra[ch].size(); ++bin)
                pOutput[Output.LinearIndex(ch, bin)] = mSpectra[ch][bin];
            break;

        case ARCoefficients: {
            const Polynomial<Real>::Vector &coeff = tf.Denominator().Coefficients();
            for (size_t i = 1; i < coeff.size(); ++i)
                pOutput[Output.LinearIndex(ch, i - 1)] = coeff[i];
        }
        break;
