////////////////////////////////////////////////////////////////////////////////
// $Id: FFTSpectrum.cpp 7268 2023-03-22 19:12:22Z mellinger $
...
void FFTSpectrum::Publish()
{
    Super::Publish();
}

void FFTSpectrum::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
...
void FFTSpectrum::Initialize(const SignalProperties &Input, const SignalProperties &Output)
{
    mOutputType = Parameter("OutputType");
    double binWidthHz = Parameter("BinWidth").InHertz(), firstBinCenterHz = Parameter("FirstBinCenter").InHertz();
    mFFTs.clear();
    ...
    for (int t = 0; t < Input.Elements(); ++t)
        mShiftCarrier[t] = std::polar<Real>(1, t * carrierOmega);
    // Normalization is applied to energy, and chosen such that input energy per second matches output energy in all
    // bins.
...
void FFTSpectrum::Process(const GenericSignal &Input, GenericSignal &Output)
{
...
#pragma omp parallel for
    for (int ch = 0; ch < Input.Channels(); ++ch)
    {
        for (size_t i = 0; i < Output.Elements(); ++i)
            pOutput[Output.LinearIndex(ch, i)] = 0;
        ...
        while (sample < Input.Elements())
        {
            ...
            case SpectralPower:
                for (size_t i = 0; i < Output.Elements(); ++i)
                    pOutput[Output.LinearIndex(ch, i)] += norm(mFFTs[ch].Output(i));
                break;
            case Coefficients:
                for (size_t i = 0; i < Output.Elements() / 2; ++i)
                    pOutput[Output.LinearIndex(ch, i)] += mFFTs[ch].Output(i).real();
                for (size_t i = 0; i < Output.Elements() / 2; ++i)
                    pOutput[Output.LinearIndex(ch, i + Output.Elements() / 2)] += mFFTs[ch].Output(i).imag();
                break;
            }
        }
        switch (mOutputType)
        ...
        case SpectralAmplitude:
            for (int i = 0; i < Output.Elements(); ++i)
                pOutput[Output.LinearIndex(ch, i)] = ::sqrt(pOutput[Output.LinearIndex(ch, i)] * mNormalizationFactor);
            break;
        case SpectralPower:
            for (int i = 0; i < Output.Elements(); ++i)
                pOutput[Output.LinearIndex(ch, i)] *= mNormalizationFactor;
            break;
        case Coefficients:
            for (int i = 0; i < Output.Elements(); ++i)
                pOutput[Output.LinearIndex(ch, i)] *= ::sqrt(mNormalizationFactor);
            break;
        }
    }
}
