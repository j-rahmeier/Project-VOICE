////////////////////////////////////////////////////////////////////////////////
// $Id: WindowingFilter.cpp 7268 2023-03-22 19:12:22Z mellinger $
...
WindowingFilter::WindowingFilter() : mDetrend(None), mWindowFunction(Rectangular)
{
}

void WindowingFilter::Publish()
{
...
void WindowingFilter::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    Output = Input;
    ...
    if (numSamples < 0)
    {
        bcierr << "WindowLength parameter must be >= 0";
        numSamples = 0;
    }
    Output.SetElements(numSamples).SetIsStream(false).ElementUnit().SetRawMin(0).SetRawMax(Output.Elements() - 1);
}

void WindowingFilter::Initialize(const SignalProperties &Input, const SignalProperties &Output)
{
...
void WindowingFilter::Process(const GenericSignal &Input, GenericSignal &Output)
{
...
#pragma omp parallel for
    for (int ch = 0; ch < Input.Channels(); ++ch)
    {
        ...
        i = std::max<ptrdiff_t>(0, mBuffers[ch].size() - Input.Elements());
        j = 0;
        ...
        case Mean:
            ::Detrend::MeanDetrend(mBuffers[ch], mDetrendBuffers[ch]);
            pDetrendedData = &mDetrendBuffers[ch];
            break;

        case Linear:
            ::Detrend::LinearDetrend(mBuffers[ch], mDetrendBuffers[ch]);
            pDetrendedData = &mDetrendBuffers[ch];
            break;
        ...
        if (mWindowFunction == Rectangular)
            for (size_t i = 0; i < pDetrendedData->size(); ++i)
                pOutput[Output.LinearIndex(ch, i)] = (*pDetrendedData)[i];
        else
            for (size_t i = 0; i < pDetrendedData->size(); ++i)
                pOutput[Output.LinearIndex(ch, i)] = mWindow[i] * (*pDetrendedData)[i];
    }
}

void WindowingFilter::StartRun()
{
