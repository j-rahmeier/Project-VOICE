////////////////////////////////////////////////////////////////////////////////
//  $Id: LPFilter.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
LPFilter::LPFilter()
    : mDecayFactor(0), mPreviousOutput(0)
{
...
void LPFilter::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    ...
    if (LPTimeConstant < 0)
        bcierr << "The LPTimeConstant parameter must be 0 or greater";

    // Request output signal properties:
    Output = Input;
}

void LPFilter::Initialize(const SignalProperties &Input, const SignalProperties & /*Output*/)
{
    ...
    if (timeConstant < std::numeric_limits<double>::epsilon())
        mDecayFactor = 0.0;
    else
...
void LPFilter::Process(const GenericSignal &Input, GenericSignal &Output)
{
    ...
    for (int channel = 0; channel < Input.Channels(); ++channel)
        ...
        {
            if (IsNaN(Input(channel, sample))) // if input is nan, do not update LP filter but just pass nan to output
