////////////////////////////////////////////////////////////////////////////////
// $Id: Normalizer.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
Normalizer::Normalizer() : mpUpdateTrigger(NULL), mPreviousTrigger(true), mDoAdapt(false)
{
    ...
    mOutputPrmFile = std::string(OptionalParameter("NormalizerOutputPrm", ""));
    if (mOutputPrmFile.size())
    {
        std::ofstream prmFile;
        prmFile.open(mOutputPrmFile, std::ios::app);
        if (prmFile.is_open())
            prmFile.close();
        else
            bciout << "failed to open NormalizerOutputPrm file \"" << mOutputPrmFile << "\" for appending";
    }
}
...
void Normalizer::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    ...
    for (int channel = 0; channel < Input.Channels() && channel < Adaptation->NumValues(); ++channel)
    ...
    if (adaptation)
    {
        ...
        std::string UpdateTrigger = Parameter("UpdateTrigger");
    ...
    // Describe output:
    if (adaptation)
        Output.ValueUnit().SetOffset(0).SetGain(1).SetSymbol("").SetRawMin(-2).SetRawMax(2);
}

void Normalizer::Initialize(const SignalProperties &Input, const SignalProperties & /*Output*/)
{
    mOffsets.clear();
    mGains.clear();
    ...
    ParamRef Adaptation = Parameter("Adaptation"), NormalizerOffsets = Parameter("NormalizerOffsets"),
    ...
    if (mDoAdapt)
    {
        ...
        mDataBuffers.resize(BufferConditions->NumColumns(),
                            std::vector<RingBuffer>(BufferConditions->NumRows(), RingBuffer(bufferSize)));
        bcidbg << "Allocated " << mDataBuffers.size() << "x" << (mDataBuffers.empty() ? 0 : mDataBuffers[0].size())
               << " data buffers of size " << bufferSize << ".";
    }
}

void Normalizer::Process(const GenericSignal &Input, GenericSignal &Output)
{
    if (mDoAdapt)
    {
    ...
    for (int channel = 0; channel < Input.Channels(); ++channel)
        for (int sample = 0; sample < Input.Elements(); ++sample)
            Output(channel, sample) = (Input(channel, sample) - mOffsets[channel]) * mGains[channel];
}

void Normalizer::StartRun()
{
    mPreviousTrigger = true;
}

void Normalizer::StopRun()
{
    if (mDoAdapt)
    ...
    {
        std::ofstream prmFile;
        prmFile.open(mOutputPrmFile.c_str(), std::ios::app);
        if (prmFile.is_open())
        {
            ...
            for (int i = 0; i < p->NumValues(); i++)
                prmFile << (std::string)p(i) << "  ";
            prmFile << " // written " << timeBuffer << std::endl;
            ParamRef q = Parameter("NormalizerGains");
            prmFile << q->Section() << " " << q->Type() << " " << q->Name() << "= " << q->NumValues() << "      ";
            for (int i = 0; i < q->NumValues(); i++)
                prmFile << (std::string)q(i) << "  ";
            prmFile << " // written " << timeBuffer << std::endl;
            prmFile.close();
        }
        else
        {
            bciout << "failed to write NormalizerOffsets and NormalizerGains to file " << mOutputPrmFile;
        }
    }
}
...
void Normalizer::Update()
{
    for (size_t channel = 0; channel < mDataBuffers.size(); ++channel)
        ...
        { // Compute raw moments for all buffers.
            size_t numValues = 0;
            std::vector<double> bufferMeans;
            std::vector<double> bufferSqMeans;
            for (size_t i = 0; i < mDataBuffers[channel].size(); ++i)
            {
                ...
                double bufferSum = 0, bufferSqSum = 0;
            ...
            // Compute total mean and variance from raw moments.
            double dataMean = 0;
            if (!bufferMeans.empty())
            { // Use the mean of means to avoid bias.
                dataMean = accumulate(bufferMeans.begin(), bufferMeans.end(), 0.0) / bufferMeans.size();

                mOffsets[channel] = static_cast<float>(dataMean);
                bcidbg << "Channel " << channel << ": Set offset to " << mOffsets[channel] << " using information"
            ...
            { // Normalize to unit variance.
                ...
                { // Again, use the mean of means to avoid bias.
                    dataSqMean = accumulate(bufferSqMeans.begin(), bufferSqMeans.end(), 0.0) / bufferSqMeans.size();
                }
                ...
                if (dataVar > eps)
                {
                    ...
                    bcidbg << "Set gain to " << mGains[channel] << ", using data variance";
                }
                bcidbg(2) << "\n"
                          << "\tmean:    \t" << dataMean << "\n"
                          << "\tvariance:\t" << dataVar << "\n"
