////////////////////////////////////////////////////////////////////////////////
// $Id: IIRFilterBase.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
void IIRFilterBase::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    Real preflightGain;
    ComplexVector preflightZeros, preflightPoles;
    Output = Input;
    DesignFilter(Output, preflightGain, preflightZeros, preflightPoles);
    if (preflightZeros.size() != preflightPoles.size())
        bcierr << "The numbers of zeros and poles must agree";
}

void IIRFilterBase::Initialize(const SignalProperties &Input, const SignalProperties &Output)
{
    ...
    mFilter.SetGain(gain).SetZeros(zeros).SetPoles(poles).Initialize(Input.Channels());
}

void IIRFilterBase::StartRun()
{
    mFilter.Initialize();
}

void IIRFilterBase::Process(const GenericSignal &Input, GenericSignal &Output)
{
