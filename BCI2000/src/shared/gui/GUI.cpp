////////////////////////////////////////////////////////////////////////////////
// $Id: GUI.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
{
    Rect r = {std::max(r1.left, r2.left), std::max(r1.top, r2.top), std::min(r1.right, r2.right),
              std::min(r1.bottom, r2.bottom)};
    return r;
}

Region& Region::Add(const Rect& r)
{
    if (EmptyRect(r))
        return *this;
    for (auto& rect : *this) {
      if (rect == r)
        return *this;
      if (PointInRect(r.TopLeft(), rect) && PointInRect(r.BottomRight(), rect))
        return *this;
      if (PointInRect(rect.TopLeft(), r) && PointInRect(rect.BottomRight(), r)) {
        rect = r;
        return *this;
      }
    }
...
Region &Region::Clip(const Rect &r)
{
