////////////////////////////////////////////////////////////////////////////////
// $Id: DisplayStatisticsField.cpp 7536 2023-08-17 17:46:25Z mellinger $
...
DisplayStatisticsField::DisplayStatisticsField(GUI::GraphDisplay &display, int zOrder)
    : TextField(display, zOrder), mpWindow(dynamic_cast<GUI::DisplayWindow *>(&display))
{
    if (!mpWindow)
    {
...
void DisplayStatisticsField::OnChange(GUI::DrawContext &dc)
{
    if (mpWindow)
    {
        ...
        if (!info.Empty())
        {
            StringUtils::String vendor = info["GLVendor"], renderer = info["GLRenderer"];
...
void DisplayStatisticsField::OnTick(const ClockTick &t)
{
    if (mpWindow)
    {
        auto s = mpWindow->FrameClockStatistics();
        double nominal = t.rate.Period().Seconds() * 1e3, interval = s.interval.Seconds() * 1e3,
               jitter = s.jitter.Seconds() * 1e3;
        std::ostringstream oss;
        if (!mInfo.empty())
            oss << mInfo << "\n";
        oss.precision(2);
        oss << std::fixed << "frame " << nominal << "ms / " << interval << " \xc2\xb1 " << jitter << "ms, " << s.missed
            << " missed, ";
        ...
        oss << "decim " << r.decimation << " render " << r.render.mean.Seconds() * 1e3 << " \xc2\xb1 "
