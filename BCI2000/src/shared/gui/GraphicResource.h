////////////////////////////////////////////////////////////////////////////////
// $Id: GraphicResource.h 7464 2023-06-30 15:04:08Z mellinger $
...
{
template <class T> int Width(const T &t)
{
    return t[0].count;
}
template <class T> int Height(const T &t)
{
    return t[1].count;
}
template <int Mode, class T> void Render(const T &, const DrawContext &);
}; // namespace GraphicResource

} // namespace GUI

template <int Mode, class T> void GUI::GraphicResource::Render(const T &inGraphic, const GUI::DrawContext &inDC)
{
#if USE_QT
    if (inDC.handle.dc)
    {
        ...
        int width = Width(inGraphic), height = Height(inGraphic);
        ...
        QRect targetRect(inDC.rect.left, inDC.rect.top, inDC.rect.right - inDC.rect.left,
