////////////////////////////////////////////////////////////////////////////////
// $Id: Shapes.h 7195 2023-02-07 18:31:03Z mellinger $
...
class Shape : public GUI::GraphObject
{
  public:
    ...
    void OnMove(GUI::DrawContext &ioDC) override;
  ...
  protected:
    ...
    {
        std::atomic<int> kind;
        std::atomic<RGBColor> color, fillColor;
        std::atomic<float> lineWidth, startAngle, endAngle;

        ShapeDef(int inKind = None)
            : kind(inKind), color(RGBColor::White), fillColor(RGBColor::NullColor), lineWidth(0), startAngle(0),
              endAngle(360)
        {
        }
    };
  ...
  private:
    SynchronizedObject<GUI::Rect> mRect;
};

class RectangularShape : public Shape
{
  public:
    RectangularShape(GUI::GraphDisplay &display, int zOrder = ShapeZOrder) : Shape(display, Rectangle, zOrder)
    {
    }
    ~RectangularShape()
    {
...
{
  public:
    EllipticShape(GUI::GraphDisplay &display, int zOrder = ShapeZOrder) : Shape(display, Ellipse, zOrder)
    {
    }
    ~EllipticShape()
    {
...
{
  public:
    PieShape(GUI::GraphDisplay &display, int zOrder = ShapeZOrder) : EllipticShape(display, zOrder)
    {
        mDef.kind = Pie;
    }
    ~PieShape()
