//////////////////////////////////////////////////////////////////////////////////////
// $Id: ExpressionNodes.h 7195 2023-02-07 18:31:03Z mellinger $
...
{
  public:
    ObjPtr(RefObj *p = 0) : mp(p)
    {
        Inc();
    }
    ObjPtr(const ObjPtr &p) : mp(p.mp)
    {
        Inc();
    }
...
class Node : public RefObj
{
  public:
    ...
    const struct ExpressionParser::Location &Location() const
    {
        return mLocation;
    }
...
inline NodePtr::NodePtr(Node *p) : mPtr(p), mp(p)
{
}
...
{
  public:
    AssignmentNode(double &valueRef, Node *rhs) : mrValue(valueRef)
    {
        AddChild(rhs);
    }
  ...
  protected:
    double OnEvaluate() override
    {
        return (mrValue = mChildren[0]->Evaluate());
    }
...
template <int N> class FunctionNode
{
};

template <> class FunctionNode<0> : public ConstPropagatingNode
{
  public:
...
template <> class FunctionNode<1> : public ConstPropagatingNode
{
  public:
    typedef double (*Pointer)(double);
    FunctionNode(bool c, Pointer f, Node *arg1) : ConstPropagatingNode(c), p(f)
    {
        AddChild(arg1);
    }
...
template <> class FunctionNode<2> : public ConstPropagatingNode
{
  public:
    typedef double (*Pointer)(double, double);
    FunctionNode(bool c, Pointer f, Node *arg1, Node *arg2) : ConstPropagatingNode(c), p(f)
    {
        AddChild(arg1);
        AddChild(arg2);
    }
  ...
  protected:
    double OnEvaluate()
    {
        return p(mChildren[0]->Evaluate(), mChildren[1]->Evaluate());
    }
...
template <> class FunctionNode<3> : public ConstPropagatingNode
{
  public:
    typedef double (*Pointer)(double, double, double);
    FunctionNode(bool c, Pointer f, Node *arg1, Node *arg2, Node *arg3) : ConstPropagatingNode(c), p(f)
    {
        AddChild(arg1);
        AddChild(arg2);
        AddChild(arg3);
  ...
  protected:
    double OnEvaluate() override
    {
        return p(mChildren[0]->Evaluate(), mChildren[1]->Evaluate(), mChildren[2]->Evaluate());
    }
...
{
  public:
    StringNode(const std::string &s) : mString(s)
    {
        mIsConst = true;
    }
