//////////////////////////////////////////////////////////////////////////////////////
// $Id: ExpressionNodes.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
{
    if (mRefs)
        SuggestDebugging << "Deleting object with " << mRefs << " references";
}
...
NodeList &NodeList::Add(Node *node, size_t idx)
{
    if (node)
...
NodeList &NodeList::Append(const NodeList &other)
{
    mList.insert(mList.end(), other.mList.begin(), other.mList.end());
    return *this;
}

Node *NodeList::operator[](size_t idx) const
{
...
// Node
Node::Node(bool isConst) : mIsConst(isConst)
{
    mLocation.end = 0;
    mLocation.begin = mLocation.end;
}

Node::Node(const NodeList &inList) : mIsConst(false)
{
    mLocation.end = 0;
    mLocation.begin = mLocation.end;
    AddChildren(inList);
...
void Node::AddChildren(const NodeList &inList)
{
    for (int i = 0; i < inList.Size(); ++i)
        AddChild(inList[i]);
}

void Node::SetLocation(size_t begin, size_t end)
{
...
Node *ConstPropagatingNode::OnSimplify()
{
    if (mPropagate)
    {
...
double StringNode::OnEvaluate()
{
    return NaN<double>();
}

// ExpressionNodes::AddressNode
AddressNode::AddressNode(Node *n1, Node *n2) : StringNode("")
{
    AddChild(n1);
    AddChild(n2);
}

double AddressNode::OnEvaluate()
{
    if (!mChildren.empty())
    {
        mString.clear();
