////////////////////////////////////////////////////////////////////////////////
// $Id: IndexList.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
bool IndexList::Parse(const std::string &inList, const LabelIndex &inLabels, const PhysicalUnit &inUnit)
{
    this->clear();
    mErrors.clear();

    std::istringstream iss(inList);
    std::string entry;
    while (iss >> std::ws >> entry)
    {
        bool negated = (entry[0] == '!');
        size_t pos = entry.find_first_of(":-");
        if (negated || pos == std::string::npos)
        ...
        { // range of addresses, no pattern matching
            std::string addr1 = entry.substr(0, pos), addr2 = entry.substr(pos + 1);
