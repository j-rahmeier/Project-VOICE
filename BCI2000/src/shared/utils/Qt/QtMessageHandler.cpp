////////////////////////////////////////////////////////////////////////////////
// $Id: QtMessageHandler.cpp 7913 2024-03-08 20:31:29Z mellinger $
...
{
    const char *file = inContext.file ? inContext.file : "n/a",
               *function = inContext.function ? inContext.function : "n/a";
    ...
    case QtDebugMsg:
        ...
        break;
    case QtWarningMsg:
        if (what.find("QWidget::paintEngine:") == 0)
            break;
        if (what.find("QTextCursor::setPosition:") == 0)
            break;
        if (what.find("QAbstractItemView::closeEditor ") == 0)
            break;
        if (what.find("accessibility") != std::string::npos)
            break;
        if (what.find("Unable to set geometry") != std::string::npos)
            break;
        NOTIFY(std::clog, "Qt Warning Message", what + where);
        break;
    case QtCriticalMsg:
        if (what.find("QThread::start") != std::string::npos)
            break;
