////////////////////////////////////////////////////////////////////////////////
// $Id: SerialStream.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
////////////////////////////////////////////////////////////////////////////////
serialbuf::serialbuf() : m_handle(INVALID_HANDLE_VALUE), m_timeout(serialbuf::defaultTimeout)
{
}
...
void serialbuf::set_timeout(int t)
{
    m_timeout = t;
    if (m_timeout == serialbuf::infiniteTimeout)
        m_timeout = MAXDWORD;
    COMMTIMEOUTS c = {MAXDWORD, MAXDWORD, m_timeout, MAXDWORD, m_timeout};
    ::SetCommTimeouts(m_handle, &c);
}

serialbuf *serialbuf::open(const char *mode)
{
...
bool serialbuf::is_open() const
{
    return m_handle != INVALID_HANDLE_VALUE;
}

serialbuf *serialbuf::close()
{
    if (m_handle != INVALID_HANDLE_VALUE)
        ::CloseHandle(m_handle);
    m_handle = INVALID_HANDLE_VALUE;
...
std::streamsize serialbuf::showmanyc()
{
    // Are there any data available in the streambuffer?
    std::streamsize result = egptr() - gptr();
    if (result < 1)
    {
        // Are there data waiting in the serial device buffer?
        COMMTIMEOUTS dont_block = {MAXDWORD, 0, 0, 0, 0};
...
std::ios::int_type serialbuf::underflow()
{
    ...
    std::ios::int_type result = traits_type::eof();
...
std::ios::int_type serialbuf::overflow(int c)
{
    if (sync() == traits_type::eof())
        return traits_type::eof();
...
////////////////////////////////////////////////////////////////////////////////
serialstream::serialstream() : std::iostream(0), buf()
{
    init(&buf);
}

serialstream::serialstream(const char *device) : std::iostream(0), buf()
{
    init(&buf);
    open(device);
}

void serialstream::open(const char *device)
{
    if (!buf.open(device))
