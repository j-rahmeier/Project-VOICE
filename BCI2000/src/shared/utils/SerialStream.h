////////////////////////////////////////////////////////////////////////////////
// $Id: SerialStream.h 7195 2023-02-07 18:31:03Z mellinger $
...
class serialbuf : public std::streambuf
{
  ...
  protected:
    std::streamsize showmanyc() override; // Called from streambuf::in_avail().

  protected:
    std::ios::int_type underflow() override;     // Called from read operations if empty.
    std::ios::int_type overflow(int c) override; // Called if write buffer is filled.
    int sync() override;                         // Called from iostream::flush().
