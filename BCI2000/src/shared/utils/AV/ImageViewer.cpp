////////////////////////////////////////////////////////////////////////////////
// $Id: ImageViewer.cpp 7536 2023-08-17 17:46:25Z mellinger $
// Author: juergen.mellinger@uni-tuebingen.de
// Description: A GraphObject for image rendering.
//
...
struct ImageViewer::Data : Lockable<std::mutex>
{
    StringUtils::String mFile, mError;
    AVDecoder mDecoder;
    int mNativeWidth, mNativeHeight;
};

// ImageViewer
ImageViewer::ImageViewer(GUI::GraphDisplay &display, int zOrder) : GraphObject(display, zOrder), d(new Data)
{
    WithLock(d)
    {
        d->mNativeWidth = 0;
...
int ImageViewer::NativeWidth() const
{
    SyncMemory<BeforeRead>();
    return d->mNativeWidth;
}

int ImageViewer::NativeHeight() const
{
    SyncMemory<BeforeRead>();
    return d->mNativeHeight;
}

const StringUtils::String &ImageViewer::Error() const
{
    SyncMemory<BeforeRead>();
...
const StringUtils::String &ImageViewer::File() const
{
    SyncMemory<BeforeRead>();
    return d->mFile;
}

StringUtils::String ImageViewer::Codec() const
{
    SyncMemory<BeforeRead>();
    return d->mDecoder.Codec();
}

StringUtils::String ImageViewer::ContainerFormat() const
{
    SyncMemory<BeforeRead>();
...
ImageViewer &ImageViewer::SetFile(const std::string &s)
{
    WithLock(d)
    {
        ...
        if (d->mDecoder.Open(s, AVDecoder::Image, 0).Error().Empty())
        {
...
// GUI::GraphObject
void ImageViewer::OnResize(GUI::DrawContext &c)
{
    SyncMemory<BeforeRead>();
    ...
    WithLock(d) d->mDecoder.BeginConfiguration()
        .SetResamplingQuality(AVDecoder::QualityGood)
        .SetFrameDimension(0, c.rect.Width())
        .SetFrameDimension(1, c.rect.Height())
        .EndConfiguration();
}

void ImageViewer::OnPaint(const GUI::DrawContext &c)
{
    GUI::Rect r = c.rect;
    AVDecoder::Chunk frame;
    WithLock(d) frame = d->mDecoder.GetFrames(1);
