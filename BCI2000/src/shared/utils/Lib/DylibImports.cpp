////////////////////////////////////////////////////////////////////////////////
// $Id: DylibImports.cpp 7654 2023-10-11 18:30:14Z mellinger $
...
static void *LoadLibrary_(const char *s)
{
    ...
    try
    {
        ...
        if (!h)
        {
            std::string path = FileUtils::CanonicalPath(s);
            if (path.empty())
                path = FileUtils::CanonicalPath(std::string(s) + ".dll");
            if (!path.empty())
            {
...
static void *LoadDylib(std::string &ioName)
{
    void *handle = 0;
#if _WIN32
...
static void UnloadDylib(void *inHandle)
{
#if _WIN32
    if (inHandle)
...
struct Unprotect
{
    Unprotect(void *addr, size_t length) : mLength(length), mOriginal(0)
    {
        mAddr.p = addr;
#if _WIN32
...
static const struct Absjmp
{
    ...
    {                           // amd64
     ...
     5 + 8 + 1, 1, 9},
    {                      // arm64 (AArch64)
      "\x50\x00\x00\x58"   // ldr x16,#8
      "\x00\x02\x1F\xD6"   // br x16
      "\0\0\0\0\0\0\0\0",
     4 + 4 + 8, 4 + 4, 4 + 4 + 4},
};

void WriteJumpInstruction(void *inStub, uint64_t inTargetAddress, int inMachine)
{
...
static void *ResolveExport(void *inHandle, const std::string &inName)
{
#if _WIN32
...
static int ListExports(void *inHandle, std::vector<std::string> &outExports)
{
    int result = 0;
    outExports.clear();
#if _WIN32
...
Library::Library(const std::string &lib, const Dylib::Names &names) : mNames(names)
{
...
#if _WIN32
    if (mHandle)
        switch (PEHeader(mHandle).CoffFileHeader()->Machine)
        {
...
int Library::Find(const char *inName, int inOptions, void **outResult, int *outStdcallStackBytes)
{
...
bool Library::Resolve(const Import *inImports, int inCount)
{
    if (mState != found)
        return false;
...
// StartupLoader class
StartupLoader::StartupLoader(const char *inLib, const Import *inImports) : Library(inLib)
{
    if (Library::Error().empty())
        Library::Resolve(inImports, 0);
}

StartupLoader::StartupLoader(const char *inLib, const char *inAliases, const Import *inImports, const char *inMsg,
                             const char *inUrl, ThrowFunc inF)
    : Library(inLib, ParseAliases(inAliases))
{
    if (!Library::Error().empty())
    {
        std::string exe = FileUtils::ExtractBase(FileUtils::ExecutablePath()), msg = inMsg ? inMsg : "",
                    url = inUrl ? inUrl : "";
        if (msg.empty())
        {
            std::ostringstream oss;
            oss << sArchBits << "-bit Library " << Library::Name() << " could not be loaded, but is necessary for "
                << exe << " to run.";
            bool isWow = false;
#if _WIN32
    ...
    {
        std::string msg = "Could not load functions from dynamic library " + Library::Name() +
...
Dylib::Names StartupLoader::ParseAliases(const char *inAliases)
{
    Dylib::Names names;
...
void StartupLoader::BuildMessage(const std::string &inMsg, const std::string &inUrl)
{
    mMessage = inMsg;
...
void StartupLoader::UninitializedFunctionExport(const char *inFunctionName) const
{ // This function will normally not be executed.
    ...
    if (State() == none)
        throw std_runtime_error << "Function " << inFunctionName << " from dynamic library " << Library::Name()
                                << " was called at initialization time, "
                                << "before the library was loaded.";
    if (mMessage.empty())
        throw std_runtime_error << "The current program requires function \"" << inFunctionName
...
void *StartupLoader::ResolveDataExport(const char *inName, int inFlags)
{
    void *addr = 0;
    if (Library::Error().empty())
    {
        ...
        if (!found && !(inFlags & Import::optional))
            throw std_runtime_error << "Could not resolve data symbol " << inName << " from dynamic library"
                                    << Library::Name();
    }
    return addr;
}

void StartupLoader::ThrowError() const
{
    throw std_runtime_error << mMessage;
}
