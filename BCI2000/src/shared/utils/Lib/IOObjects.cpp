////////////////////////////////////////////////////////////////////////////////
// $Id: IOObjects.cpp 7464 2023-06-30 15:04:08Z mellinger $
...
Streambuf *BasicIOObject::AttachTo(Streambuf *sb)
{
...
void BasicIOObject::OnAttachTo(Streambuf *)
{
    throw std_logic_error << "This function should never be called";
}

const char *BasicIOObject::OnDescribeIOState() const
{
    if (Good())
        return "Good";
    if (Bad())
...
InputObject &InputObject::Default()
{
    static struct InputFile_ : File
    {
        InputFile_() { AttachToFd(0); }
    } file;
    return file;
}

int64_t InputObject::Read(void *p, int64_t n)
{
    ScopedLock(this);
    if (Bad())
        return -1;
...
OutputObject &OutputObject::Default()
{
    static struct OutputFile_ : File
    {
        OutputFile_() { AttachToFd(1); }
    } file;
    return file;
}

void InputObject::OnAttachTo(Streambuf *sb)
{
...
void OutputObject::OnAttachTo(Streambuf *sb)
{
    Streambuf *prev = AttachedTo();
    if (prev)
    {
...
int64_t OutputObject::Write(const void *p, int64_t n)
{
    ScopedLock(this);
    if (Bad())
        return -1;
