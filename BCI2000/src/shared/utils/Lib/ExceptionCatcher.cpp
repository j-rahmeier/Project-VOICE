//////////////////////////////////////////////////////////////////////
// $Id: ExceptionCatcher.cpp 7536 2023-08-17 17:46:25Z mellinger $
...
ExceptionCatcher::ExceptionCatcher(int flags) : mFlags(flags)
{
}

bool ExceptionCatcher::Run(Runnable &inRunnable)
{
...
bool ExceptionCatcher::RunWithWin32ExceptionsHandled(Runnable &inRunnable)
{
...
bool ExceptionCatcher::RunWithSignalsHandled(Runnable &inRunnable)
{
    bool result = false;
    static std::atomic<int> signalHandlingInstalled = 0;
    if ((mFlags & CatchSignals) && !signalHandlingInstalled.exchange(1))
    {
...
bool ExceptionCatcher::RunWithCppExceptionsHandled(Runnable &inRunnable)
{
    bool result = false;
    if (mFlags & CatchCppExceptions)
    {
        ...
        {
            message = "Unhandled exception of type " + ClassName(typeid(e)) + ": " + e.what();
        }
        catch (...)
        {
...
#if CATCH_WIN32_EXCEPTIONS
inline const Win32Exception::ExceptionInfo *Win32Exception::Info() const
{
...
#define BAD_EXCEPTION(x) EXCEPTION_(x, true)
    static const ExceptionInfo info[] = {
        EXCEPTION(ACCESS_VIOLATION) EXCEPTION(ARRAY_BOUNDS_EXCEEDED) EXCEPTION(BREAKPOINT)
        EXCEPTION(DATATYPE_MISALIGNMENT) EXCEPTION(FLT_DENORMAL_OPERAND) 
        EXCEPTION(FLT_DIVIDE_BY_ZERO) EXCEPTION(FLT_INEXACT_RESULT)
        EXCEPTION(FLT_INVALID_OPERATION) EXCEPTION(FLT_OVERFLOW) EXCEPTION(FLT_STACK_CHECK)
        EXCEPTION(FLT_UNDERFLOW) EXCEPTION(GUARD_PAGE) EXCEPTION(ILLEGAL_INSTRUCTION)
        EXCEPTION(IN_PAGE_ERROR) EXCEPTION(INT_DIVIDE_BY_ZERO) EXCEPTION(INT_OVERFLOW)
        EXCEPTION(INVALID_DISPOSITION) EXCEPTION(INVALID_HANDLE)
        BAD_EXCEPTION(NONCONTINUABLE_EXCEPTION) EXCEPTION(PRIV_INSTRUCTION)
        EXCEPTION(SINGLE_STEP) BAD_EXCEPTION(STACK_OVERFLOW)
...
std::string Win32Exception::Describe() const
{
    std::ostringstream oss;
    oss << Info()->description << " (0x" << std::hex << this->code << ")";
    return oss.str();
}
...
void ExceptionCatcher::ReportWin32Exception(const Win32Exception &inException)
{
    OnReportException(inException.Describe());
}
#endif // CATCH_WIN32_EXCEPTIONS

void ExceptionCatcher::ReportSignal(int inSignal)
{
    size_t numSignals = sizeof(sSignals) / sizeof(*sSignals), i = 0;
    while (sSignals[i].code != inSignal)
        ++i;
    const char *pDescription = i < numSignals ? sSignals[i].name : "Unknown Signal";
    std::ostringstream oss;
    oss << "Signal caught: " << pDescription;
    OnReportException(oss.str());
}

void ExceptionCatcher::OnReportException(const std::string &inMessage)
{
    std::cerr << inMessage + (mMessage.empty() ? "" : "\n") + mMessage << std::endl;
}
