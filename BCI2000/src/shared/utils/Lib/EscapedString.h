//////////////////////////////////////////////////////////////////////
// $Id: EscapedString.h 7195 2023-02-07 18:31:03Z mellinger $
...
namespace std
{
inline istream &getline(istream &is, Tiny::EscapedString &e, char delim = '\n')
{
    struct Eq
    {
        bool operator()(int b)
...
inline std::ostream &operator<<(std::ostream &s, const Tiny::EscapedString &e)
{
    return e.InsertInto(s);
}

inline std::istream &operator>>(std::istream &s, Tiny::EscapedString &e)
{
    return e.ExtractFrom(s);
}

template <typename T> std::istream &Tiny::EscapedString::ReadUntil(std::istream &is, T predicate)
{
    const struct
    {
        char code;
        ...
        {'0', '\0'}, {'a', '\a'}, {'t', '\t'}, {'v', '\v'}, {'f', '\f'}, {'r', '\r'}, {'n', '\n'},
    };
    const size_t count = sizeof(escapeCodes) / sizeof(*escapeCodes);

    clear();
    bool done = false, withinQuotes = false;
    while (!done)
    {
        int c = is.peek();
        switch (c)
