//////////////////////////////////////////////////////////////////////
// $Id: TinySemaphore.cpp 7604 2023-09-21 17:02:45Z jhill $
...
struct Semaphore::Private
{
    ...
    Private() : mpEventCondition(new Waitable::EventCondition(false)), mConditionRef(mpEventCondition)
    {
    }
};

Semaphore::Semaphore(int initialCount) : p(new Private)
{
...
{
    WithLocked(pCount = p->mpCount.Mutable()) if (++*pCount == 1) p->mpEventCondition->Set();
    return true;
}

bool Semaphore::Decrease(const Time::Interval &inTimeout)
{
