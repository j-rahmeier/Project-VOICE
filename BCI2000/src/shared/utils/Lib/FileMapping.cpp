////////////////////////////////////////////////////////////////////////////////
// $Id: FileMapping.cpp 7570 2023-09-05 16:39:58Z mellinger $
...
#if _WIN32
FileMapping::Private::Private() : mpBaseAddress(nullptr), mFile(INVALID_HANDLE_VALUE), mLength(0)
{
}
...
bool FileMapping::Private::Open(const char *inName, int inFlags)
{
    Close();
    int openMode = GENERIC_READ, shareMode = FILE_SHARE_READ, disposition = OPEN_EXISTING, protection = PAGE_READONLY,
...
bool FileMapping::Private::SetLength(int64_t newLength)
{
    if (newLength < 0)
        return false;
    if (mFile == INVALID_HANDLE_VALUE)
...
void FileMapping::Private::Close()
{
    if (mpBaseAddress)
    {
...
FileMapping::Private::Private() : mpBaseAddress(nullptr), mLength(0), mFile(-1)
{
}
...
bool FileMapping::Private::Open(const char *inName, int inFlags)
{
    ...
    {
        SysError err;
        mError = "mmap(" + std::to_string(mLength) + ", " + std::string(inName) + "): ";
        if (err.Code())
            mError += err.Message();
        else
            mError += "Unknown error";
...
FileMapping::FileMapping() : p(new Private)
{
}

FileMapping::FileMapping(const std::string &file, int flags) : p(new Private)
{
...
FileMapping &FileMapping::Open(const std::string &file, int flags)
{
    p->Open(FileUtils::AbsolutePath(file).c_str(), flags);
    return *this;
}
...
FileMapping &FileMapping::SaveAs(const std::string &file)
{
    Private *next = new Private;
    if (!(next->Open(FileUtils::AbsolutePath(file).c_str(), FileMapping::write) && next->SetLength(p->mLength)))
    {
    ...
    {
        if (next->mpBaseAddress && p->mpBaseAddress)
...
char *FileMapping::BaseAddress() const
{
    return static_cast<char *>(p->mpBaseAddress);
}

int64_t FileMapping::Length() const
{
    return p->mLength;
}

FileMapping &FileMapping::SetLength(int64_t inLength)
{
    p->SetLength(inLength);
    return *this;
}

const std::string &FileMapping::Error() const
{
    return p->mError;
}
