////////////////////////////////////////////////////////////////////////////////
// $Id: ProcessUtils.h 7627 2023-09-27 13:59:38Z mellinger $
...
std::string ProcessWD();
void SetProcessWD(const std::string &);
class TemporaryProcessWD
{
  public:
    TemporaryProcessWD(const std::string &);
    ~TemporaryProcessWD() noexcept(false);
...
bool ExecuteSynchronously(const std::string &executable, const std::string &arguments, std::ostream &stdout_,
                          std::ostream &stderr_, int &);
...
bool InitializeShellIntegration(const std::string &registrar);
bool AssertSingleInstance(int argc, char **argv, const std::string &id = "", int timeout = 0);
bool RequestPrecisionTiming();

#ifndef _WIN32
char** CommandLineToArgvA(const char *lpCmdline, int *pArgc);
#endif
...
static const int Minimum = -(1 << 16), Normal = 0, Maximum = 1 << 16;
} // namespace Priority
