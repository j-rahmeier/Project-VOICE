//////////////////////////////////////////////////////////////////////
// $Id: Lockable.h 7195 2023-02-07 18:31:03Z mellinger $
...
template <class T = std::recursive_mutex> class Lockable;
...
template <typename T> class Lock_
{
  ...
  public:
    Lock_(T &t) : mpT(&t), mcpT(0)
    {
        t.Lock();
    }
    Lock_(const T &t) : mpT(0), mcpT(&t)
    {
        t.Lock();
    }
    Lock_(T *pT) : mpT(pT), mcpT(0)
    {
        pT->Lock();
    }
    Lock_(const T *cpT) : mpT(0), mcpT(cpT)
    {
        cpT->Lock();
    }
    ...
    Lock_(const Lock_ &other) : mpT(other.mpT), mcpT(other.mcpT)
    {
        if (mpT)
            mpT->Lock();
        else if (mcpT)
...
template <typename T> class Lock_<const T> : public Lock_<T>
{
  public:
...
template <typename T> class Locked : private Lock_<T>
{
  public:
    ...
    T *operator->()
    {
        return &Lock_<T>::operator()();
    }
};

template <typename T> Lock_<T> TemporaryLock(T &t)
...
inline LockableObject::LockObject::LockObject(const LockableObject &obj, bool tryLock) : mp(&obj), mFlag(false)
{
    Construct(tryLock);
}

inline LockableObject::LockObject::LockObject(const LockableObject *pObj, bool tryLock) : mp(pObj), mFlag(false)
{
    Construct(tryLock);
}

inline LockableObject::LockObject::~LockObject()
{
    if (mp)
        mp->Unlock();
}

inline void LockableObject::LockObject::Construct(bool tryLock)
{
    if (!tryLock)
    {
        mp->Lock();
...
inline bool LockableObject::LockObject::ToggleFlag()
{
    return mFlag = !mFlag;
}
