//////////////////////////////////////////////////////////////////////
// $Id: SharedMemory.cpp 7570 2023-09-05 16:39:58Z mellinger $
...
const struct ProtocolEntry
{
    const char *name;
    int id;
} Protocols[] = {{"file://", file}, {"shm://", shm}, {0, shm}};
...
SharedMemory::SharedMemory(const std::string &inName, size_t inSize)
    : mName(inName), mServer(inSize != 0), mSize(inSize), mpMemory(0), mProtocol(none)
{
    Initialize();
}

SharedMemory::SharedMemory(size_t inSize) : mServer(true), mSize(inSize), mpMemory(0), mProtocol(none)
{
    Initialize();
}
...
void SharedMemory::ParseProtocol()
{
    size_t nameOffset = 0;
...
void SharedMemory::NormalizeName()
{
    if (mProtocol == shm)
    {
        if (mName.empty())
        ...
        if (mName[0] != '/')
            mName = '/' + mName;
        size_t pos = 1;
        while ((pos = mName.find_first_of(":/\\<>|", pos)) != std::string::npos)
            mName[pos++] = '_';
        if (mName.length() > 255)
    ...
    else if (mProtocol == file)
    {
        ...
        {
            char ch = mName[i];
            if (i == 0 && (ch == '/' || ch == '\\') && mName.length() >= 3 && (mName[2] == ':' || mName[2] == '|'))
                continue;
            if (ch == '/')
                ch = '\\';
            if (ch == '|')
...
void SharedMemory::Create()
{
    if (mProtocol == shm)
    {
        ...
        while (mSize != 0 && mHandle.h == 0)
        {
            LARGE_INTEGER size;
            size.QuadPart = mSize;
            mHandle.h = ::CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, size.HighPart, size.LowPart,
                                             mName.c_str() + 1);
    ...
    {
        std::fstream f(mName, std::ios::binary | std::ios::out | std::ios::trunc);
        f.fill('%');
        f << std::setw(mSize) << f.fill();
        f.close();
        Open();
    }
}
...
void SharedMemory::Open()
{
    ...
    {
        HANDLE hFile = ::CreateFileA(mName.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
                                     OPEN_EXISTING, 0, 0);
        if (hFile)
...
void SharedMemory::MapMemory()
{
    ...
    if (mHandle.h)
    {
        ...
        if (mpMemory == nullptr)
            throw std_runtime_error << "Could not map shared memory: " << SysError().Message();
    }
#else  // _WIN32
    if (mSize == 0)
        return;
    ...
    if (mHandle.fd >= 0)
    {
        mpMemory = ::mmap(0, mSize, PROT_READ | PROT_WRITE, MAP_SHARED, mHandle.fd, 0);
        if (mpMemory == (void*)-1)
            throw std_runtime_error << "Could not map shared memory of size " << mSize << ": " << SysError().Message();
    }
#endif // _WIN32
}
