////////////////////////////////////////////////////////////////////////////////
// $Id: LCRandomGenerator.cpp 7464 2023-06-30 15:04:08Z mellinger $
...
template <typename T> T inverf(T x)
{ // Inverse Error Function
    ...
    T signx = (x > 0) - (x < 0), b = log(1 - x * x), c = 2 / Pi(x) / a + b / 2, r = 0;
    r = c * c - b / a;
    r = ::sqrt(r);
    r -= c;
    r = signx * ::sqrt(r);
...
double LCRandomGenerator::Normal::InverseCPDF(double x)
{
    return inverf(2 * x - 1);
}

LCRandomGenerator::SeedType LCRandomGenerator::DefaultSeed()
{
...
LCRandomGenerator::NumberType LCRandomGenerator::RandMax()
{
    return NumberFromSeed(~SeedType(0));
}

LCRandomGenerator::NumberType LCRandomGenerator::Random()
{ // x_i = ( x_{i-1} * a + c ) mod m
    ...
    const uint32_t a = 2891336453UL, c = 1;
    mSeed *= a;
    mSeed += c;
    return NumberFromSeed(mSeed);
}

LCRandomGenerator::NumberType LCRandomGenerator::NumberFromSeed(LCRandomGenerator::SeedType inSeed)
{
    return inSeed >> 16; // Ignore lower bits which have bad periodicity.
}

std::string LCRandomGenerator::RandomName(size_t inLength)
{
    std::string s;
    if (inLength)
...
char LCRandomGenerator::RandomCharacter(int (*inClass)(int))
{
    char c = 0;
    do
        c = Random() & 0x7f;
