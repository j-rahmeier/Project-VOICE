////////////////////////////////////////////////////////////////////////////////
// $Id: EnvVariable.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
// Class methods
bool EnvVariable::Set(const std::string &inName, const std::string &inValue)
{
    bool success = false;
#if _WIN32
...
bool EnvVariable::Get(const std::string &inName, std::string &outValue)
{
#if _WIN32
    bool success = false;
...
bool EnvVariable::Clear(const std::string &inName)
{
    bool success = false;
#if _WIN32
...
// Instance methods
EnvVariable::EnvVariable(const std::string &inName) : mName(inName), mExisted(false)
{
...
bool EnvVariable::Exists() const
{
    std::string ignored;
    return Get(ignored);
}

bool EnvVariable::Set(const std::string &inValue)
{
    return Set(mName, inValue);
}

bool EnvVariable::Get(std::string &outValue) const
{
    return Get(mName, outValue);
}

std::string EnvVariable::Get() const
{
    std::string value;
    Get(mName, value);
    return value;
