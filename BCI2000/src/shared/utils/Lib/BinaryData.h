//////////////////////////////////////////////////////////////////////
// $Id: BinaryData.h 7195 2023-02-07 18:31:03Z mellinger $
...
{
    template <typename T> static std::istream &Get(std::istream &is, T *t, size_t n = 1)
    {
        union {
            T *t;
            char *c;
    ...
    template <typename T> static std::ostream &Put(std::ostream &os, const T *t, size_t n = 1)
    {
        union {
            const T *t;
            const char *c;
...
template <> struct BinaryIO<false>
{
    ...
    template <typename T> static std::ostream &Put(std::ostream &os, const T *t, size_t n = 1)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            const char *c = p.c + i * sizeof(T), *q = c + sizeof(T);
            while (q > c)
                os.put(*--q);
        }
        return os;
...
template <> struct ErrorIf<true>
{
  private:
    ErrorIf(); /* Intentional compiler error, results from unsuited type argument */
};
...
template <typename T, int DataByteOrder, int Count_ = ArrayNo> class BinaryData
{
  public:
    ...
    {
        return BinaryIO<DataByteOrder == HostOrder>::Get(is, mData, Count_);
    }
    std::ostream &Put(std::ostream &os) const
    {
        return BinaryIO<DataByteOrder == HostOrder>::Put(os, mData, Count_);
    }
...
template <typename T, int DataByteOrder> class BinaryData<T, DataByteOrder, ArrayNo>
{
  public:
    ...
    explicit BinaryData(std::istream &is) : mData(0)
    {
        Get(is);
    }
    explicit BinaryData(std::ifstream &is) : mData(0)
    {
        Get(is);
    }
    operator T() const
    ...
    std::istream &Get(std::istream &is)
    {
        return BinaryIO<DataByteOrder == HostOrder>::Get(is, &mData);
    }
    std::ostream &Put(std::ostream &os) const
    {
        return BinaryIO<DataByteOrder == HostOrder>::Put(os, &mData);
    }
  ...
  private:
    ...
    // Cannot use BinaryData<> for structs because it would mix data across struct elements.
    ErrorIf<!IsElementaryNumericType> mError1;
    // Cannot use BinaryData<> for nonstandard floats because endianness is only defined for IEC559 floats.
...
template <typename T, int DataByteOrder> class BinaryData<T, DataByteOrder, ArrayYes>
{
  public:
    ...
    BinaryData(const BinaryData &other) : mpData(0), mCount(0)
    {
        AssignFrom(other);
    }
    ~BinaryData()
    ...
    std::istream &Get(std::istream &is)
    {
        return BinaryIO<DataByteOrder == HostOrder>::Get(is, mpData, mCount);
    }
    std::ostream &Put(std::ostream &os) const
    {
        return BinaryIO<DataByteOrder == HostOrder>::Put(os, mpData, mCount);
    }
...
struct BinaryField : BinaryData<T, DataByteOrder, Count_>, FieldBase
{
    ...
    {
        BinaryData<T, DataByteOrder, Count_>::operator=(other);
    }
    template <class U> BinaryField &operator=(U u)
    {
        BinaryData<T, DataByteOrder, Count_>::operator=(u);
        return *this;
    }
    int ByteSize() const override
    {
        return BinaryData<T, DataByteOrder, Count_>::ByteSize();
    }
    std::ostream &Put(std::ostream &os) const override
    {
        return BinaryData<T, DataByteOrder, Count_>::Put(os);
    }
    std::istream &Get(std::istream &is) override
    {
        return BinaryData<T, DataByteOrder, Count_>::Get(is);
    }
};
...
template <class T> struct BinaryRecord : BinaryRecord_::Base, T
{
    BinaryRecord()
    {
        BinaryRecord_::Base::ConstructionDone();
    }
    BinaryRecord(const BinaryRecord &other) : T(other)
    {
        BinaryRecord_::Base::ConstructionDone();
    }
};
