////////////////////////////////////////////////////////////////////////////////
// $Id: PEHeader.cpp 7464 2023-06-30 15:04:08Z mellinger $
...
PEHeader::PEHeader(void *inHeaderAddress)
    : mpAddress(nullptr), mpCoffFileHeader(nullptr), mBits(0), mpSections(0), mSectionCount(0), mpDirectory(0),
      mDirEntryCount(0)
{
...
void PEHeader::Init(void *inHeaderAddress)
{
...
IMAGE_SECTION_HEADER *PEHeader::SectionFromRVA(ptrdiff_t rva)
{
...
ptrdiff_t PEHeader::RVAToFileOffset(ptrdiff_t rva)
{
    IMAGE_SECTION_HEADER *pSection = SectionFromRVA(rva);
    if (pSection)
    {
        ptrdiff_t raw = pSection->PointerToRawData, virt = pSection->VirtualAddress;
        return rva + raw - virt;
    }
    return 0;
}
...
bool PEHeader::RelocateResources(ptrdiff_t delta)
{
...
// PEFile
PEFile::PEFile(PEHeader &h, bool isImageSection) : mHeader(h), mIsImageSection(isImageSection)
{
}

template <class T> T PEFile::DataAs(ptrdiff_t rva)
{
    if (mIsImageSection)
...
const char *PEFile::ExportLibraryName()
{
...
bool PEFile::NextExport(PEFile::SymbolInfo &ioInfo)
{
    ioInfo.name = nullptr;
    ioInfo.data = false;
    ioInfo.rva = 0;
...
const char *PEFile::ImportLibraryName(size_t idx)
{
    if (mHeader.CoffFileHeader())
    {
        ...
        if (pImportsLocation)
        {
            const IMAGE_IMPORT_DESCRIPTOR *pImports =
                DataAs<IMAGE_IMPORT_DESCRIPTOR *>(pImportsLocation->VirtualAddress);
