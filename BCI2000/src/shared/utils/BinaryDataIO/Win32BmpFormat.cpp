////////////////////////////////////////////////////////////////////////////////
// $Id: Win32BmpFormat.cpp 7464 2023-06-30 15:04:08Z mellinger $
...
std::string Decode(std::istream &f, int &outWidth, int &outHeight, std::vector<uint8_t> &outImg, int inFormat)
{
    ...
    bool isBGRA = info.bV4Size >= info.ByteSize() && info.bV4V4Compression == Win32::BI_BITFIELDS &&
                  info.bV4BitCount == 32 && info.bV4RedMask == 0xff << rShift && info.bV4GreenMask == 0xff << gShift &&
                  info.bV4BlueMask == 0xff << bShift && info.bV4AlphaMask == 0xff << aShift;
    if (info.bV4Width < 0 || info.bV4Planes > 1 || (!isBGRA && (info.bV4V4Compression != Win32::BI_RGB)))
...
std::string Encode(std::ostream &f, int inWidth, int inHeight, const std::vector<uint8_t> &inImg)
{
