////////////////////////////////////////////////////////////////////////////////
// $Id: BCI2000ActiveDataPort.cpp 7314 2023-04-13 17:05:03Z mellinger $
...
{
  COM_METHOD_START
  std::string eventType = com::DualString( inEventType );
  for( std::string::iterator i = eventType.begin(); i != eventType.end(); ++i )
    if( ::isspace( *i ) )
      *i = '_';
  ...
  {
    std::string eventCode = com::DualString( inEventCode );
    std::istringstream iss( eventCode );
    std::string subject, session, directory;
    if( iss >> subject )
      mBCI2000.SubjectID( subject );
    if( iss >> session )
      mBCI2000.SessionID( session );
    if( std::getline( iss >> std::ws, directory ) )
...
CBCI2000ActiveDataPort::Initialize( const DataPortSettings& inSettings )
{
  ...
  else if( inSettings[DataPortSettings::OperatorPath].empty() )
  {
    ...
    if( ::LoadStringW( com::Module::GetHInstance(), IDS_OperatorName, reinterpret_cast<wchar_t*>( &pOperator ), 0 ) )
      ...
      path += L"Operator.exe";
    mBCI2000.OperatorPath( std::string( path ) );
    if( !mBCI2000.Connect() )
      throw bciexception <<
  ...
  mEventTypes.clear();
  std::istringstream iss( inSettings[DataPortSettings::EventTypes] );
