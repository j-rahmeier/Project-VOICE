//   $Id: PyffFilter.cpp 6866 2022-07-26 18:45:37Z mellinger $
//
  ...
  mpThreadState( NULL )
{
  try {
    ...
    std::string dllname  = OptionalParameter(FILTER_PREFIX "DLL", "python27");
    std::string logFile  = OptionalParameter(FILTER_PREFIX "Log", "");
    std::string frameworkDir = OptionalParameter(FILTER_PREFIX "Dir", FileUtils::InstallationDirectory() + FILTER_PREFIX);
    std::string workingDir = OptionalParameter(FILTER_PREFIX "WD", ".");
    ...
    std::string module = PYTHON_MODULE;
    ...
    for( size_t i = 0; i < numParamDefs; ++i ) {
      std::string paramDef = PyString_AsString(PyList_GetItem(py_params, i));
    ...
    for( size_t i = 0; i < numStateDefs; ++i ) {
      std::string stateDef = PyString_AsString(PyList_GetItem(py_states, i));
...
FILTER_NAME::SendParametersToPython() const
{
  ...
  for( int i = 0; i < Parameters->Size(); ++i ) {
    ...
    if(p.Type().size() >= 6 && strcmp(p.Type().c_str() + p.Type().size() - 6, "matrix") == 0) {
      ...
      int entries = std::max(p.NumRows(),p.NumColumns());
  ...
  for( int ip = 0; ip < Parameters->Size(); ++ip ) {
    ...
    if(strcmp(p.Type().c_str(), "matrix") == 0) {
      ...
      if(!PyList_Check(py_value)) {
        bcierr << "failed to propagate self.params['" << name << "'] because of wrong type (python list was expected)";
        mStayOpen = 1; return;
      }
      else if (PyList_Size(py_value) != nrows) {
        bcierr << "failed to propagate self.params['" << name << "'] because of wrong length (" << nrows << " entries were expected)";
        mStayOpen = 1; return;
      }
      else {
        for(int i = 0; i < nrows; i++) {
          ...
          if(!PyList_Check(py_row)) {
            bcierr << "failed to propagate self.params['" << name << "'][" << i << "] because of wrong type (python list was expected)";
            mStayOpen = 1; return;
          }
          else if (PyList_Size(py_row) != ncols) {
            bcierr << "failed to propagate self.params['" << name << "'][" << i << "] because of wrong length (" << ncols << " entries were expected)";
            mStayOpen = 1; return;
          }
          else {
            for(int j = 0; j < ncols; j++) {
              ...
              if(!PyString_Check(py_item)) {
                bcierr << "failed to propagate self.params['" << name << "'][" << i << "][" << j << "] because of wrong type (python string was expected)";
                mStayOpen = 1; return;
              }
              else {
      ...
      if(!PyList_Check(py_value)) {
        ...
        bcierr << "failed to propagate self.params['" << name << "'] because of wrong length (" << entries << " entries were expected)";
        mStayOpen = 1; return;
      }
      else {
        int ind = 0;
        for(int i = 0; i < p.NumRows(); i++) {
          for(int j = 0; j < p.NumColumns(); j++, ind++) {
            ...
            if(!PyString_Check(py_item)) {
              bcierr << "failed to propagate self.params['" << name << "'][" << ind << "] because of wrong type (python string was expected)";
              mStayOpen = 1; return;
            }
            else {
              std::string tmp = PyString_AsString(py_item);
      ...
      if(!PyString_Check(py_value)) {
        bcierr << "failed to propagate self.params['" << name << "'] because of wrong type (python string was expected)";
        mStayOpen = 1; return;
      }
      else {
...
FILTER_NAME::ConvertPyObjectToProperties(PyObject* pyOutSignalProperties, SignalProperties& outSignalProperties) const
{
  ...
  outSignalProperties.SetName(std::string(name));
  ...
  ConvertPyDictToPhysicalUnit(PyDict_GetItemString(pyOutSignalProperties, "ValueUnit"), outSignalProperties.ValueUnit(0));
}
...
FILTER_NAME::ConvertPyListToLabelIndex(PyObject* from, LabelIndex& to) const
{
  ...
  for(int i = 0; i < to.Size(); i++){
    std::string channelName = PyString_AsString(PyList_GetItem(from, i));
...
{
  if(qualifier && strlen(qualifier)) std::cerr << "[" << qualifier << "] ";
  std::cerr   << (notify_restart ? "BCI2000 must be restarted: " : "")  << msg << std::endl;
  bcierr << (notify_restart ? "BCI2000 must be restarted: " : "")  << msg;
}

void
FILTER_NAME::HandleEndUserError(const EndUserError& e, std::string qualifier) const
{
  UnblockThreads();
  bcierr << e.What();
}
...
void
FILTER_NAME::HandleException(const std::exception& e, std::string qualifier) const
{
  UnblockThreads();
...
void
FILTER_NAME::ChangeDir(const std::string& d)
{
