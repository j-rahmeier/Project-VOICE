////////////////////////////////////////////////////////////////////////////////
// $Id: CoherenceFilter.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
CoherenceFilter::Preflight( const SignalProperties& Input, SignalProperties& Output ) const
{
  ...
  {
    std::string label1 = Input.ChannelLabels()[ch1];
    for( int ch2 = 0; ch2 < ch1; ++ch2 )
    {
      std::string label2 = Input.ChannelLabels()[ch2],
                  outLabel = std::string( "Coh-" ) + label1 + "-" + label2;
      Output.ChannelLabels()[outChannel++] = outLabel;
    }
  }
  Output.SetElements( CohFrequencies->NumValues() );
  for( int bin = 0; bin < CohFrequencies->NumValues(); ++bin )
    Output.ElementLabels()[bin] = std::string( CohFrequencies( bin ) );
...
CoherenceFilter::Initialize( const SignalProperties& Input, const SignalProperties& Output )
{
  ...
  mInputBuffer.clear();
  int bufferLength = static_cast<int>( Parameter( "CohBufferLength" ).InSampleBlocks() * Input.Elements() );
  mInputBuffer.resize( Input.Channels(), std::valarray<real>( bufferLength ) );
  ...
  mFIRCoefficients.resize( numBins, std::valarray<complex>( windowLength ) );
  for( size_t bin = 0; bin < mFIRCoefficients.size(); ++bin )
  {
    ...
    {
      // e^(iwt)
      mFIRCoefficients[bin][time] = std::polar( 1.0, 2.0 * M_PI * frequency * time );
      // Hamming window
  ...
  mFIRConvolution.resize( Input.Channels(), std::vector< std::valarray<complex> >( numBins ) );
...
CoherenceFilter::Process( const GenericSignal& Input, GenericSignal& Output )
{
  ...
  for( int ch = 0; ch < Input.Channels(); ++ch )
  {
    ...
    for( int sample = std::max( 0, Input.Elements() - static_cast<int>( mInputBuffer[ch].size() ) ); sample < Input.Elements(); ++sample )
      mInputBuffer[ch][sample + mInputBuffer[ch].size() - Input.Elements()] = Input( ch, sample );
    // Remove mean to avoid artifacts.
    std::valarray<real> zeroMeanBuffer = mInputBuffer[ch];
    zeroMeanBuffer -= std::accumulate( &zeroMeanBuffer[0], &zeroMeanBuffer[zeroMeanBuffer.size()], 0.0 ) / zeroMeanBuffer.size();
...
CoherenceFilter::InnerProduct( const std::valarray<complex>& inV1, const std::valarray<complex>& inV2 )
{
  return ( inV1 * inV2.apply( std::conj ) ).sum();
}
