////////////////////////////////////////////////////////////////////////////////
// $Id: FieldTripBufferFilter.cpp 7195 2023-02-07 18:31:03Z mellinger $
                                   ...
                                   const SignalProperties& Output )
{
  ...
  {
    bufferLength = std::max( bufferLength, i->length() );
    bcidbg( 2 ) << "State " << *i << " will be written to buffer";
  }
  for( StateSet::const_iterator i = mFTStatesFromBuffer.begin(); i != mFTStatesFromBuffer.end(); ++i )
  {
    bufferLength = std::max( bufferLength, i->length() );
    bcidbg( 2 ) << "State " << *i << " will be read from buffer";
  }
  bufferLength += sizeof( INT32_T );
  mpEventBuffer = new UINT8_T[ bufferLength ];

  mSignalBuffer = GenericSignal( 0, 0 );
  mFTOutputEventType = std::string( Parameter( "FTOutputEventType" ) );
...
FieldTripBufferFilter::ConfigureStateSets( StateSet& ioToBufferSet, StateSet& ioFromBufferSet ) const
{
  ...
  {
    std::string state = Parameter( "FTStatesToBuffer" )( i );
    if( state == "*" )
      for( const auto& state : *States )
        ioToBufferSet.insert( state.Name() );
    else if( !state.empty() && std::string( "!~-" ).find( state[ 0 ] ) != std::string::npos )
  ...
  {
    std::string state = Parameter( "FTStatesFromBuffer" )( i );
...
FieldTripBufferFilter::ProcessEvent( const event_t& inEvent )
{
  ...
  {
    std::string type = std::string( reinterpret_cast<const char*>( inEvent.buf ), inEvent.def->type_numel );
    ...
    if( i != mFTStatesFromBuffer.end() )
    {
      ...
      switch( inEvent.def->value_type )
      {
        ...
        case DATATYPE_FLOAT64:
          ...
          break;
        default:
          bcierr << "Unexpected value_type in " << type << " event";
      }
      State( *i ) = value;
      bcidbg( 2 ) << "Set state " << *i << "=" << value << " from buffer";
    }
    ...
    bcierr << "Unexpected type_type in event";
}
...
FieldTripBufferFilter::PutStateToBuffer( const std::string& inName, int inSample )
{
  ...
  bcidbg( 2 ) << "Put state " << inName << "==" << value << " into buffer";
...
FieldTripBufferFilter::SendRequest( int inCommand, T* inDef, void* inData ) const
{
  ...
  if( 0 != dmarequest( &request, &response ) )
    bcierr << "Could not send request of type " << ClassName( typeid( *inDef ) );
...
void
FieldTripBufferFilter::ParseHostAddress( const std::string& inString, host_t& outHostAddress ) const
{
  const char cSeparator = ':';
  std::string address = inString;
