  ...
  acquireEventRead( NULL )
{
    ...
    BEGIN_PARAMETER_DEFINITIONS
				"Source:TDT int DeviceType= 9 9 0 10"
					"//Device Type: 0 RP2, 1 RL2, 2 RA16, "
												 "3 RV8, 4 RX5, 5 RX6, 6 RX7, "
												 "7 RX8, 8 RZ2, 9 RZ5, 10 RZ6 (enumeration) ",
...
void TDTADC::Preflight(const SignalProperties&,	SignalProperties& outputProperties)	const
{
    ...
    // connect to the ZBus
    ZBus->ConnectZBUS("GB"); 
		int deviceType = Parameter("DeviceType").ToNumber();
		devAddrTemp = ZBus->GetDeviceAddr(mDeviceIDs.find(deviceType)->second, 1);
    /*devAddrTemp[0] = ZBus->GetDeviceAddr(45,1);
    devAddrTemp[1] = ZBus->GetDeviceAddr(50,1);
		devAddrTemp[2] = ZBus->GetDeviceAddr(53, 1);*/
    short tConnectType;
    if (devAddrTemp == 0)
    ...
    switch (deviceType){
				case 0:
					if (!RPcoX1->ConnectRP2("GB", 1))
						errMes = "RP2";
						break;
				case 1:
					if (!RPcoX1->ConnectRL2("GB", 1))
						errMes = "RL2";
					break;
				case 2:
					if (!RPcoX1->ConnectRA16("GB", 1))
						errMes = "RA16";
					break;
				case 3:
					if (!RPcoX1->ConnectRV8("GB", 1))
						errMes = "RV8";
					break;
				case 4:
					...
					break;
				case 5:
					if (!RPcoX1->ConnectRX6("GB", 1))
						errMes = "RX6";
					break;
				case 6:
					if (!RPcoX1->ConnectRX7("GB", 1))
						errMes = "RX7";
					break;
				case 7:
					if (!RPcoX1->ConnectRX8("GB", 1))
						errMes = "RX8";
					break;
				case 8:
					...
					break;
				case 9:
					if (!RPcoX1->ConnectRZ5("GB", 1))
						errMes = "RZ5";
					break;
				case 10:
					if (!RPcoX1->ConnectRZ6("GB", 1))
						errMes = "RZ6";
					break;
    }
		if (errMes != "")
			bcierr << "Error connecting	to the " << errMes << ".Use	the	zBuzMon	to ensure you are connected, and that you are using an " << errMes << ".";
...
void TDTADC::Initialize(const SignalProperties&, const SignalProperties&)
{
  ...
  blockSize =	mSampleBlockSize;
    ...
    ZBus->ConnectZBUS("GB");
		int deviceType = Parameter("DeviceType").ToNumber();
		devAddr = ZBus->GetDeviceAddr(mDeviceIDs.find(deviceType)->second, 1);
		short tConnectType;
		if (devAddr == 0)
			bcierr << "There does not seem to be a compatible TDT device on the rack. Quitting." << endl;
