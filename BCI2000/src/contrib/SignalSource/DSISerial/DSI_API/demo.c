...
running The default demo mode is "signals" but the following are recognized:
    ...
    version, info, buffering, montages, naming, impedances, signals, accel
...
#   include <stdlib.h>  //
    ...
    int     AccelDemo( int argc, const char * argv[] );
...
DSI_API_Loader.c has also been compiled and linked into your project:
    ...
    int main( int argc, const char * argv[] )
    {
        ...
        else if(  mode &&  *mode && strncmp( mode, "accel",      strlen( mode ) ) == 0 ) return AccelDemo( argc, argv );
...
to measure impedances:
    ...
    int ImpedanceDemo( int argc, const char * argv[] )
    {
        ...
        return 0;
                                                                              ...
                                                                              /*
A thrid demo illustrates accessing the optional 3D Accelerometer sensors by
setting the Accelerometer Rate to a positive value up to 30 Hz.  The registered
SampleCallback function checks for an update to the values of the accelerometer
channels and prints them if an updated sample is availalbe.  Because the
accelerator channels are updated at lower frequency than the EEG channels, the
API pushes the same sample to their queues for several subsequent calls to the
SampleCallback function, only provideing the next available accelerometer
sample data when the EEG packet-sent timestamps have exceeded the packet-sent
timestamp of the next available accelerometer sample.  Use of these 3D accelerometer
channels is illustrated in the `AccelDemo` example, as follows:
                                                                              */
    int AccelDemo( int argc, const char * argv[] )
    {
        DSI_Headset h;
        ...
        double      custom = GetFloatingPointOpt( argc, argv, "custom",       "c",  0.0 );
        double durationSec = GetFloatingPointOpt( argc, argv, "duration-sec", NULL, 5.0 );
        int accelRateHz = GetIntegerOpt( argc, argv, "rate", NULL, 30 );
        ...
        {
            fprintf( stderr,
                "\nUsage: %s accel [--options...] \n"
                "\n"
                "e.g.:  %s --port=COM4 --rate=30 > accel.csv\n"
                "\n"
                "       Connects to COM4 and activates the accelerometer at 30 Hz\n"
                "       then records accelerometer samples for a few seconds,\n"
                "       dumping output to accel.csv\n"
                "\n",
                argv[ 0 ], argv[ 0 ] );
            return 0;
        }
        ...
        if ( ! DSI_Headset_GetFeatureAvailability(h, "Accelerometer") )
        {
            fprintf( stderr, "\nThis headset does not offer the 3D Accelerometer feature.");
            return 0;
        // Check for active 3D accelerometer feature and exit if not available
        }

        double currentAccelRateHz = DSI_Headset_GetAccelerometerRate(h);
        if( (int)currentAccelRateHz != accelRateHz )
        {
            DSI_Headset_SetAccelerometerRate(h, accelRateHz);
            currentAccelRateHz = DSI_Headset_GetAccelerometerRate(h);

        // Check accelerometer acquisition rate and set if needed
        }

        DSI_Headset_ChooseChannels(h, "SND,ACCT,ACCX,ACCY,ACCZ", NULL, 0);
        // Choose the accelerometer channels: ACCT,ACCX,ACCY,ACCZ
...
as shown in the `buffering` demo below:
    ...
    int BufferingDemo( int argc, const char * argv[] )
    {
        ...
        printf( "t0,t1,ExcessSamples,OverflowSamples,SamplesPerBatch,TargetExcessSamples\n" );

        if( bufferAheadSec > 0.5 )
            DSI_Headset_ReallocateBuffers( h, bufferAheadSec * 2.0, 1.0 ); CHECK
        // By default we have a 1-second buffer for the signal and a 1-second buffer for
        // impedances, but if we want to buffer ahead by anything more than (or even approaching)
        // that 1 second, then we need to reallocate the signal buffers.
        ...
        {
            t0 = DSI_Headset_WaitForBatch( h ); CHECK
            // This blocking call aims to ensure that batches of samples are picked
            // up at exactly regular intervals (t0 timestamps should be very regular)
            for( channelIndex = 0; channelIndex < DSI_Headset_GetNumberOfChannels( h ); channelIndex++ )
            {
                DSI_Channel c = DSI_Headset_GetChannelByIndex( h, channelIndex ); CHECK
                for( sampleIndex = 0; sampleIndex < samplesPerBatch; sampleIndex++ )
            ...
            printf( "%9.4f,%9.4f,%3lu,%3lu,%3d,%3d\n",
                t0,                                           // t0 timestamps should be very regular
                t1 = DSI_Headset_SecondsSinceConnection( h ), // t1 timestamps may be slightly less so
...
END OF DSI API DOCUMENTATION.  THANK YOU FOR YOUR ATTENTION
    ...
    int GlobalHelp( int argc, const char * argv[] )
    {
        fprintf( stderr,
            ...
            "\n"
            "       signals, accel, impedances, montages, naming, buffering, version, info\n"
            "\n"
