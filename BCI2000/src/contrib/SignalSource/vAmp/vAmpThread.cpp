////////////////////////////////////////////////////////////////////////////////
// $Id: vAmpThread.cpp 7195 2023-02-07 18:31:03Z mellinger $
  ...
  mLock( false )
{
    ...
    for (UINT i = 0; i < m_tblChanInfo[0].size(); i++)
    {
        ...
        if (i < UINT(m_nEEGChannels)) // EEG
        {
            ...
            ci.dResolution = double(m_DeviceProp[0].ResolutionEeg * 1e6); // V > muV
        }
        else if (m_nAUXChannels > 0 && // AUX present
            i >= UINT(m_nEEGChannels) && i < m_tblChanInfo[0].size() -1 ) // AUX
        {
            ci.nType = DEVICE_CHAN_TYPE_AUX;
            ci.dResolution = double(m_DeviceProp[0].ResolutionAux * 1e6); // V > muV
        }
		else // Digital port.
		{
	...
	acquireEventRead = CreateEventA(NULL, true, false, "ReadEvent");
