/******************************************************************************
 * $Id: BioRadioADC.cpp 7195 2023-02-07 18:31:03Z mellinger $
                              ...
                              SignalProperties& outSignalProperties ) const
{
  ...
  if( Parameter("SourceChGain")->NumValues() != Parameter("SourceCh") )
    bcierr << "# elements in SourceChGain has to match total # channels";
  if( Parameter("SourceChOffset")->NumValues() != Parameter("SourceCh") )
    bcierr << "# elements in SourceChOffset has to match total # channels";
  ...
  if( !goodSourceChGain )
    bcierr << "SourceChGain is supposed to be ones";

  if( !goodSourceChOffset )
    bcierr << "SourceChOffset is supposed to be zeros";

  for (int ch = ZERO; ch<Parameter("SourceCh"); ch++)
   if(int(Parameter("SourceChGain")(ch)) != Parameter("SourceChGain")(ch))
        bcierr << "SourceChGain is supposed to be a integer.";

  for (int ch = ZERO; ch<Parameter("SourceCh"); ch++)
   if(int(Parameter("SourceChOffset")(ch)) != Parameter("SourceChOffset")(ch))
        bcierr << "SourceChOffset is supposed to be a integer.";
  ...
  {
    bcierr<<"Channels enabled must be between 1-8 (inclusive) and be a natural number.";
    errorFlag = XERROR;
  }
  ...
  {
    bcierr<<"Sampling frequency must exist in the following set: { 768, 600, 480, 384, 256, 128}";
    errorFlag = XERROR;
  }
  ...
  {
    bcierr<<"The value of the sample block size must be be a multiple of 10, and <= 120.";
    errorFlag = XERROR;
  }
 // Check to see if the voltage selected is within the defined range
  ...
  {
    bcierr<<"Voltage range index must exist in the following set: {0, 1, 2, 3, 4, 5, 6 ,7}.";
    errorFlag = XERROR;
  }
  ...
  if(errorFlag == XNO_ERROR)
  {
    ...
    double vRange = bioutils::GetBioRadioRangeValue(Parameter("VoltageRange"));
    mTempFile.Open();
    if(bioutils::WriteBioRadioConfig( static_cast<int>(Parameter( "SamplingRate" ).InHertz()),
                                      BIT_RES,
                                      vRange,
                                      mTempFile))
    {
      bcierr<<"Could not write temporary file.";
      errorFlag = XERROR;
    }
    mTempFile.Close();
  ...
  if(errorFlag == XNO_ERROR)
  {
    ...
    if(! radio.Start(bioutils::GetPort(port)) && port != AUTO)
    {
      radio.Stop();
    }
    else
    {
     ...
     for(int i = ZERO; i < ALLPORTS; ++i)
     {
       ...
       {
         if(!radio.Start(bioutils::GetPort(i)))
         {
           radio.Stop();
           foundIt = TRUE;
           port = i;
    ...
    if(!radio.Start(bioutils::GetPort(port)))
    {
      radio.Purge();      // set internal buffer to zero
      if(radio.Program(mTempFile.Name()))
      { radio.Stop();       //kill
        bcierr<<"Error programming device, path may be incorrect.";
      }
      radio.Stop();       //kill
    }
    else
    {
      bcierr<<"Communication failure with the Bioradio150.";
      bcierr<<"Make sure the BioRadio is turned on and plugged in, and COM port is properly selected.";
    }
  }
  }
...
BioRadioADC::Initialize( const SignalProperties&, const SignalProperties& )
{
  ...
  mTempFile.Open();
  bioutils::WriteBioRadioConfig( mSamplerate,
                                 BIT_RES,
                                 vRange,
                                 mTempFile );
  mTempFile.Close();
