...
Biosemi2Client::Biosemi2Client()
    : mpUsbdata(new char[USB_DATA_SIZE]), mpData(new char[BUFFER_SIZE_IN_BYTES]), mpDataAsInt(NULL), mMk2(false),
      mBattLow(false), mSamplingRate(-1), mSampleBlockSize(0), mDesiredSamplingRate(-1), mDesiredSampleBlockSize(-1),
      mNumChannels(-1), mMode(0), mDevice(NULL), mpBufferCursorPos(new DWORD), mIntsBuffered(0), mOldBufferCursorPos(0),
      mStartPos(0), mNextPos(0), mIntsAvailable(0), mNumIntsToRead(0), mIsDataReadyCalledYet(false),
      mDecimationFactor(1), mWasDriverSetup(false), mpDataBlock(NULL)
{
...
void Biosemi2Client::setupDriver()
{
    ...
    *******************************************************************************/
    ...
    {
        bcierr << "Could not connect to device.";
        return;
    }
    ...
    *******************************************************************************/
    ...
    {
        bcierr << "Could not initialize USB interface.";
        return;
    }
    ...
    *******************************************************************************/
    ...
    if (!USB_WRITE(mDevice, mpUsbdata))
    {
        bcierr << "Could not enable the handshake.";
        return;
    }
...
bool Biosemi2Client::initialize(int desiredSamplingRate, int desiredSampleBlockSize, int desiredNumChannels)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        {
            bcierr << "Could not get device status.";
            return false;
        }
        Sleep(10);
    }
    ...
    bool ignored;
    decodeStatus(mpDataAsInt, 0, mMk2, mBattLow, mMode, mNumChannels, mSamplingRate, ignored, ignored);
    ...
    if (0 != mSamplingRate % mDesiredSamplingRate)
    {
        bcierr << "Sampling rate requested: " << mDesiredSamplingRate
               << " does not evenly divide biosemi sampling rate: " << mSamplingRate;
        return false;
    }
...
int Biosemi2Client::determineNumChannelsMk1(int mode) const
{
    ...
    case 8:
        result = 258;
        if (mode == 8)
            result += NUM_AIB_CHANNELS; // 2 + 256 + 32 = 290
        break;
    case 1:
    case 5:
        result = 130;
...
int Biosemi2Client::determineNumChannelsMk2(int mode) const
{
    ...
    case 8:
        result = 282;
        if (mode == 8)
            result += NUM_AIB_CHANNELS; // 2 + 280 + 32 = 314
        break;
    case 5:
        result = 154;
        break;
...
int Biosemi2Client::determineSamplingRate(int mode) const
{
    ...
    case 0:
    case 4:
    case 8:            // AIB mode (which means the ADC's must be in speed mode 4 according to
                       // http://www.biosemi.com/faq/adjust_samplerate.htm )
        result = 2048; // 2 kHz
        break;
    case 1:
...
void Biosemi2Client::decodeStatus(const int *data, int position, bool &mk2, bool &battLow, int &mode, int &numChannels,
...
bool Biosemi2Client::isDataValid(const int *data, int startPos) const
{
    ...
    for (int sample = 0; sample < mSampleBlockSize; sample++)
    {
        ...
        {
            datum = mpData[(sample * mNumChannels + channel + startPos) % Biosemi2Client::BUFFER_SIZE_IN_INT];
...
void Biosemi2Client::isDataReady()
{
    ...
    *******************************************************************************/
    ...
    while (mIntsAvailable < mNumIntsToRead)
    {
        ...
        if (!READ_POINTER(mDevice, mpBufferCursorPos))
        {
            bcierr << "Could not read data from device.";
            return;
        }
        ...
        *mpBufferCursorPos /= BYTES_PER_INT;
        ...
        {
            mIntsAvailable += 0 + *mpBufferCursorPos + BUFFER_SIZE_IN_INT - mOldBufferCursorPos;
        }
...
int Biosemi2Client::calcIndex(int sample, int channel, int startPos) const
{
    return (startPos + sample * mNumChannels * mDecimationFactor + channel) % Biosemi2Client::BUFFER_SIZE_IN_INT;
}
int Biosemi2Client::calcIndex(int sample, int channel) const
{
...
Biosemi2Client::DataBlock::DataBlock(const Biosemi2Client *BIOSEMI) : mpBiosemi(BIOSEMI)
{
}
...
{
    int val, out = 0;
    for (int i = 0, offset = mpBiosemi->mStartPos; i < mpBiosemi->mDecimationFactor;
         i++, offset += mpBiosemi->mNumChannels)
    {
...
unsigned int Biosemi2Client::DataBlock::getAllTriggers(int sample) const
{
    ...
    {
        val =
            (unsigned int)mpBiosemi->mpDataAsInt[mpBiosemi->calcIndex(sample, Biosemi2Client::STATUS_CHANNEL, offset)];
...
int Biosemi2Client::DataBlock::getSignal(int sample, int channel) const
{
    if (sample > mpBiosemi->mDesiredSampleBlockSize)
    {
        bcierr << "Sample out of bounds error.";
        return 0;
    }
