...
class DASUtils
{
  public:
    ...
    static int GetADRangeCode(float inMin, float inMax);

    // Check whether a numerical range is compatible with an UL range code.
    // Considers UNIPOLAR and BIPOLAR range codes.
    static bool ADRangeCompatible(int inRangeCode, float inMin, float inMax);
    ...
    // FIFO buffer.
    static int GetTransferBlockSize(int inBoardNumber, long &outBlockSize);

    // Check whether a board supports certain options, and return the combination
    // we consider 'best'.
    static int GetBoardOptions(int inBoardNumber, int inChannels, long &ioCount, long &ioSamplingRate, int inADRange,
                               int &ioOptions);
    ...
    static int BackgroundScan(int inBoardNumber, int inLowChan, int inHighChan, long inDataCount, long &ioRate,
