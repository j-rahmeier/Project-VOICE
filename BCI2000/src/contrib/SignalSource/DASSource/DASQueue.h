...
class DASQueue : public std::queue<short>
{
  public:
    struct DASInfo
    {
        long boardNumber, samplingRate, sampleBlockSize, numChannels;
    ...
    operator bool() const
    {
        return mFailstate == ok;
    }
    bool is_open() const
    {
        return mShouldBeOpen && mFailstate == ok;
    }
  ...
  private:
    ...
    int mFailstate, mBoardNumber, mFreqMultiplier, mChannels, mHWChannels;
    USHORT *mDataBuffer;
    long mDataBufferSize, mReadCursor, mWriteCursor;
    bool mShouldBeOpen;
    ...
    static void CALLBACK BoardEventCallback(int inBoardNumber, unsigned inEventType, unsigned inEventData,
                                            void *inUserData);
};
