...
class CtfNeuroSrv : public NeuroSrv
{
  ...
  public:
    CtfNeuroSrv(const regex_t *inChannelNameRegex = NULL, double inFreqCorrectionFactor = 1.0,
                int inDataOutputFormat = DataTypeRaw16bit, bool inDoHPFiltering = true,
                std::ostream &ioElocFile = std::cout, std::ostream &ioSourceParamFile = std::cout);
    virtual ~CtfNeuroSrv();
