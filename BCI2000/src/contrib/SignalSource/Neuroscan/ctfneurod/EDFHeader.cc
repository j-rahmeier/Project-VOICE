////////////////////////////////////////////////////////////////////////////////
// $Id: EDFHeader.cpp 8073 2024-04-24 18:36:43Z mellinger $
...
size_t EDFHeader::Length() const
{
    const size_t h = 8 + 80 + 80 + 16 + 8 + 44 + 8 + 8 + 4, ch = 16 + 80 + 5 * 8 + 80 + 8 + 32;
    return h + Channels.size() * ch;
}

std::ostream &EDFHeader::Serialize(std::ostream &os) const
{
    ...
    PutArray<Str<32>>(os, Channels, &ChannelInfo::Reserved);

    return os;
}

std::istream &EDFHeader::Unserialize(std::istream &is)
{
    ...
    GetArray<Str<32>>(is, Channels, &ChannelInfo::Reserved);
