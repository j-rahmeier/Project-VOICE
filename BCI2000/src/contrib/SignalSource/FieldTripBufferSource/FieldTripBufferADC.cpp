////////////////////////////////////////////////////////////////////////////////
// $Id: FieldTripBufferADC.cpp 7195 2023-02-07 18:31:03Z mellinger $
                               ...
                               SignalProperties& outSignalProperties ) const
{
  ...
  if( pfSocket < 0 ) 
  {
    bcierr << "Cannot establish a connection to the FieldTrip buffer";
  } 
  else
  {
    headerdef_t hdr;
    if (!readHeader(pfSocket, hdr))
	  bcierr << "Error while trying to read header from FT buffer";
	else
	{
		...
		switch(hdr.data_type) {
			...
			case DATATYPE_FLOAT32:
				...
				break;
			default:
				bcierr << "Datatype not supported yet";
				return;
		}

		if( Parameter( "SourceCh" ) != hdr.nchans )
          bcierr << "The number of channels in the FT buffer does not match the SourceCh parameter";
...
FieldTripBufferADC::Initialize( const SignalProperties&, const SignalProperties& )
{
  ...
  {
    bcidbg( 2 ) << "State " << *i << " will be read from buffer events";
  }
  isRunning = 0;  
}
...
FieldTripBufferADC::Process( const GenericSignal&, GenericSignal& Output )
{
  if (mSocket < 0)
  {
	bcierr << "No connection to FieldTrip buffer";
    return;
  }
  ...
  {
	bcierr << "Error while polling for data from FT buffer";
	return;
  }

  if (nSamp < mNumSamplesRead + blockSize) 
  {
	bcierr << "Timeout for reading data from FT buffer";
	return;
  }
  ...
  {
	bcierr << "Error while reading data from FT buffer";
	return;
  }
  ...
  {
	  bcierr << "Error while reading events from FT buffer";
	  return;
  }
  ...
  while (offset + sizeof(eventdef_t) <= databuf.size()) {
    ...
    std::string type = std::string( evbuf + offset + sizeof(eventdef_t), evdef->type_numel );
      ...
      if( i != mEventStates.end() )
      {
        ...
        bciwarn << "Ignoring event with type '" << type << "'";
	  }
	}
...
FieldTripBufferADC::ConfigureStateSet( StateSet& stateSet ) const
{
  ...
  {
    std::string state = Parameter( "EventStates" )( i );
