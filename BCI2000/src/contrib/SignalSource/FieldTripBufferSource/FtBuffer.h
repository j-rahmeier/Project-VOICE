...
class FtBufferResponse {
	...
	bool checkGetEvents(SimpleStorage *evtStore = NULL) const {
		if (m_response == NULL) return false;
		if (m_response->def == NULL) return false;
		if (m_response->def->version != VERSION) return false;
		if (m_response->def->command != GET_OK) return false;
		...
		if (evtStore != NULL) {
			unsigned int len = m_response->def->bufsize;
			if (!evtStore->resize(len)) return false;
			if (len>0) memcpy(evtStore->data(), m_response->buf, len);
		}
		return true;
	}
