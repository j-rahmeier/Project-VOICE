//   $Id: PythonFilter.cpp 7686 2023-11-02 20:22:58Z dmehta $
//
	...
	dll_loaded(false)
{
	try {
		...
		//bciout << "Dll to be loaded: " << dllname.c_str() << std::endl;
...
{
	PyObject* params = PyDict_New(); // initialize empty python dictionary for parameters
	PyObject* value; // initialize empty value object to be assigned later

	for (int i = 0; i < Parameters->Size(); ++i) { // for each parameter in Parameters
		const Param& p = Parameters->ByIndex(i); // param p is the parameter at index i
		
		if (p.NumRows() == 0 || p.NumColumns() == 0) { // if p has no columns or rows (is not a list or matrix)
			value = PyString_FromString(""); // value is now an empty string
			PyDict_SetItemString(params, p.Name().c_str(), value); // create key-value pair in params dictionary where key is the name of p and the value is empty (???)
			Py_DecRef(value); // idk man maybe
			continue;
		}
		if (p.Type().size() >= 6 && strcmp(p.Type().c_str() + p.Type().size() - 6, "matrix") == 0) {
			...
			for (int i = 0; i < nrows; i++) {
				...
				for (int j = 0; j < ncols; j++) { 
					...
					if (p.Value(i, j).Kind() >= 2) {
						const Param& q = *p.Value(i, j).AsParam();
						int entries = max(q.NumRows(), q.NumColumns());

						PyObject* tempList = PyList_New(entries);

						for (int k = 0; k < entries; k++) {
							PyObject* newVal = PyString_FromString(((std::string)q.Value(k)).c_str()); 
							PyList_SetItem(tempList, k, newVal);
						}
						PyList_SetItem(tempRow, j, tempList); 
					}
					else {
			...
			for (int i = 0; i < entries; i++) {
				if (p.Value(i).Kind() >= 2) {
					const Param& q = *p.Value(i).AsParam();
					int entries = max(q.NumRows(), q.NumColumns());
					...
					PyList_SetItem(list, i, tempList);
				}
				else {
	...
	for (int i = 0; i < Parameters->Size(); ++i) {
		const Param& p = Parameters->ByIndex(i);
...
FILTER_NAME::ReceiveParametersFromPython()
{
	...
	for (int ip = 0; ip < Parameters->Size(); ++ip) {
		...
		const Param& p = Parameters->ByIndex(ip);
...
FILTER_NAME::SendStatesToPython() const
{
	...
	size_t sourceSamplesPerBlock = Parameter("SampleBlockSize");
	if (elSize != 4) FAIL("internal error: unsupported architecture - sizeof(uint32_t) is not 8");
	for (const auto& s : *States) {
		const char* name = s.Name().c_str();
		//size_t nEl = (s.Kind() == State::EventKind || s.Kind() == State::StreamKind) ? sourceSamplesPerBlock : 1;
		
		pyobj = PyBytes_FromStringAndSize(NULL, sourceSamplesPerBlock * elSize);
		if (!pyobj) FAIL("failed to allocate memory for state buffer");
		uint32_t* vals = (uint32_t*)PyBytes_AsString(pyobj);
		for (size_t el = 0; el < sourceSamplesPerBlock; el++)
			* vals++ = State(name)(el);
		PyDict_SetItemString(states, name, pyobj);
		// Py_DecRef(pyvalue);
		//bciout << name;
	}
	//bciout << states;
...
{
	PyObject* pyobj = CallHook("_get_states");
	PyObject* state_value;
	//long value;
	size_t elSize = sizeof(uint32_t);

	// StateMap m;
	size_t sourceSamplesPerBlock = Parameter("SampleBlockSize"); // for Event states only
	for (const auto& s : *States) {
		const char* name = s.Name().c_str();
		...
		if (elSize != 4) FAIL("internal error: unsupported architecture - sizeof(int) is not 4");
		if (!pyobj) FAIL("No python signal data available (null pointer)");
		
		
		state_value = PyDict_GetItemString(pyobj, name); // state_value doesn't need to be DECREFfed - it returns only a *borrowed* reference
		if (!PyBytes_Check(state_value)) FAIL("python state is supposed to be encoded as a bytes object");
		uint32_t* vals = (uint32_t*)PyBytes_AsString(state_value); // must use PyInt_AsLong - Float_AsDouble returns strange numbers
		size_t bytesSent = PyBytes_Size(state_value);
		if (bytesSent == elSize) State(name) = *vals;
		else if(bytesSent == sourceSamplesPerBlock * elSize)
		{
			//if (State(name)->Kind() != State::EventKind && State(name)->Kind() != State::StreamKind) FAIL("cannot set multiple elements per block for non-Event state '" << name << "'");
			//if (value == -1 && PyErr_Occurred()) PyErr_Clear(); // in case of an exception, we set the state to -1 and ignore the exception.
			for (size_t el = 0; el < sourceSamplesPerBlock; el++)
			{
				uint32_t value = *vals++;
				if( State(name)(el) != value ) State(name)(el) = value;
			}
		}
		else FAIL("python state '" << name << "' contains an unexpected number of bytes (expected " << elSize << " or " << elSize * sourceSamplesPerBlock << ", got " << bytesSent << ")" );
	}
	Py_DecRef(pyobj);
}
...
FILTER_NAME::ConvertPyObjectToProperties(PyObject* pyOutSignalProperties, SignalProperties& outSignalProperties) const
{
	...
	PhysicalUnit valueUnit = outSignalProperties.ValueUnit();
	ConvertPyDictToPhysicalUnit(PyDict_GetItemString(pyOutSignalProperties, "ValueUnit"), valueUnit);
	outSignalProperties.ValueUnit() = valueUnit;
}
...
#ifdef _WIN32
  AllocConsole();
  if(title && *title) SetConsoleTitleA(title);
...
FILTER_NAME::HandlePythonError(std::string msg, bool errorCodeReturned) const
{
	...
	if (error == 0) return;
	bcierr << "End user error: " << error << " "<< isForEndUser << std::endl;
	if (isForEndUser) throw EndUserError(report.c_str());

	stay_open = 1;
	if (msg.length()) msg = "Python error during " + msg + "Report: " + report;
