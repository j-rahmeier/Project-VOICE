//   $Id: PythonWrapper.cpp 7462 2023-06-30 14:27:11Z mellinger $
//  
...
int LoadPythonLinks(const char *dllname)
{
	...
	if(dllname && *dllname) {
		...
		if (!dll) {
			...
			bcierr << "Failed to find a dynamic library named " << names << ". Is python installed?" << std::endl;
		}
	}
