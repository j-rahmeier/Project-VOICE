...
class WebcamController : public WebcamClient
{
public:
  WebcamController();
  ~WebcamController();
  ...
  struct Parameters
  {
      int width = 640, height = 480, fps = 15, videoprofile = 1, compressionFactor = 8;
      std::string url;
  ...
  void Preflight(int camIndex, const Parameters&);
...
protected:
  void OnWebcamFrameNumber(uint64_t) override;
  void OnWebcamFrameData(int, int, const void*) override;
  void OnWebcamMessage(const std::string&) override;
