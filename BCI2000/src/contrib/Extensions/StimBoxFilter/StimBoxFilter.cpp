////////////////////////////////////////////////////////////////////////////////
// $Id: StimBoxFilter.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
// Helper Functions
static bool parseLabel( const std::string& inLabel, int &outCh, int &outMode );
static bool stimBoxError( int error );
...
StimBoxFilter::Preflight( const SignalProperties &Input, SignalProperties &Output ) const
{
  ...
  if( portNum )
  {
    ...
    if( stimBoxError( gSTIMboxinit( &stimBox, portNum, 256, 1, 1 ) ) )
      bcierr << "Could not init g.STIMbox on COM" << portNum;
    if( stimBoxError( gSTIMboxclose( stimBox ) ) )
      bcierr << "Could not close connection to g.STIMbox.";

    // Validate the Output Matrix
    ParamRef outputMat = Parameter( "DigitalOutputs" );
    if( outputMat->NumColumns() < 1 )
      bcierr << "Parameter DigitalOutputs should have at least one column.";
    ...
    for( int r = 0; r < outputMat->NumRows(); r++ )
    {
      ...
      {
        bciout << "Invalid DigitalOutput Label " << label << "; Ignored.";
        continue;
      }
...
StimBoxFilter::Initialize( const SignalProperties &Input, const SignalProperties &Output )
{
  ...
  if( mPortNumber )
  {
    ...
    if( stimBoxError( gSTIMboxgetDriverVersion( driverVersion ) ) )
      bcierr << "Could not retrieve g.STIMbox driver version.";
    bcidbg( 0 ) << "g.STIMbox Driver Version: " << driverVersion;

    // Connect to the device
    if( stimBoxError( gSTIMboxinit( &mStimBox, mPortNumber, 256, 1, 1 ) ) )
      bcierr << "Could not init g.STIMbox.";
    if( stimBoxError( gSTIMboxreset( mStimBox ) ) )
      bcierr << "Could not reset g.STIMbox.";
    ...
    //  bcierr << "Could not determine g.STIMbox hardware version.";
    //bcidbg( 0 ) << "g.STIMbox Hardware Version: " << hwVersion;
  ...
  if( mPortNumber )
  {
    ...
    if( !stimBoxError( gSTIMboxgetConnStatus( mStimBox, state ) ) )
    {
      ...
      if( state && stimBoxError( gSTIMboxclose( mStimBox ) ) )
        bcierr << "Could not close g.STIMbox.";
    }
    else
      bcierr << "Could not determine if g.STIMbox is still running.";
  }
}
...
StimBoxFilter::Process( const GenericSignal &Input, GenericSignal &Output )
{
  ...
  if( mPortNumber )
  {
    ...
    if( stimBoxError( gSTIMboxgetConnStatus( mStimBox, state ) ) || state == 0 )
      bcierr << "Error communicating with g.STIMbox -- Connection Interrupted.";
    ...
    std::vector< int > portUpdate;
    std::vector< double > portFreqUpdate;
    for( int p = 0; p < NUM_OUTPUT_PORTS; p++ )
    {
      // Determine if this port is active or not
      double value = 0;
      std::map< int, Expression >::iterator exp = mDigitalOutputExpMap.find( p );
      ...
      if( mModeselektor[p] == 1 )
      {
        ...
        value = std::max( 1.0, std::min( ( double )value, 50.0 ) );
    ...
    if( stimBoxError( gSTIMboxsetPortState( mStimBox, mOutputPorts.Size(), &mOutputPorts[0], &mOutputPortStates[0] ) ) )
      bcierr << "Error communicating with g.STIMbox -- setPortState failed.";
    if( !portUpdate.empty() )
      if( stimBoxError( gSTIMboxsetFrequency( mStimBox, mOutputPorts.Size(), &mOutputPorts[0], &mOutputPortFreqs[0] ) ) )
        bcierr << "Error communicating with g.STIMbox -- setFrequency failed.";
  }
}
...
StimBoxFilter::DisablePorts()
{
  ...
  std::vector< int > states = std::vector< int >( NUM_OUTPUT_PORTS, 0 );
  Vector< int > ports;
...
StimBoxFilter::StimBoxThread::OnExecute()
{
  ...
  if( stimBoxError( gSTIMboxstopInputMonitoring( mpFilter->mStimBox ) ) )
    bcierr << "Could not stop gSTIMbox input monitoring";
...
stimBoxError( int error )
{
  if( error )
  {
    ...
    if( !errormessage )
      bciout << "g.STIMbox Error: Unable to retrieve error message!";
    else
    {
      if( gSTIMboxgetErrorMessage( error, messagelength, errormessage ) )
        bciout << "g.STIMbox Error: Unable to retrieve error message!";
      else
        bciout << "g.STIMbox Error: " << std::string( errormessage );
      delete [] errormessage;
    }
    return true;
  }
...
bool
parseLabel( const std::string& inLabel, int &outCh, int &outMode )
{
  outCh = outMode = 0;
 
 // Tokenize
  std::stringstream ss( inLabel );
  std::string ch;
  std::getline( ss, ch, 'F' );
  std::istringstream( ch ) >> outCh;

  // Find Modifiers
  if( inLabel.find( 'F' ) != std::string::npos
   || inLabel.find( 'f' ) != std::string::npos )
    outMode = 1;
