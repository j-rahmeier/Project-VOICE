...
INT32 ELCALLTYPE receive_data_file_feedback_v2(char *src, char *dest, INT16 dest_is_path,
                                            void(ELCALLTYPE *progress)(unsigned int size,unsigned int received));
...
/*! @ingroup datafile
	...
	to eyelink_send_command().
 	...
 	@param src           Name of local file (including extension).
	@param dest          Name of eye tracker file to write to (including extension).
	@param dest_is_path  If nonzero, appends file name to &lt;dest&gt; as a directory path.
	@param progress      A function pointer, that takes size and received size integers.
			   This allows, one to display progress bar on file transfer.
...
INT32 ELCALLTYPE send_data_file_feedback(char *src, char *dest, INT16 dest_is_path,
                                            void(ELCALLTYPE *progress)(unsigned int size,unsigned int received));
...
/*! @ingroup datafile
	Opens an EDF file, closes any existing file.
 	...
 	@param name Name of data file,which can only use letters(uppercase and lowercase),
		        numbers(0-9),and underscore(appears from 2nd place). Name extension
				can only be edf.
...
/*! @ingroup priority
	...
	the minimum time this function takes, so that this function can act as a useful delay.
 	...
 	@remarks Warning:  Under Windows XP, on single core machines, this call will lock out all 
				keyboard input.  The Task Manager will take about 30 seconds to respond to 
				CTRL-ALT-DEL, so press this once and be patient.  The keyboard functions well  
				in realtime mode under Windows 2000.  This function has little or no effect under
 				Windows 95/98/Me. Keyboard, mouse, and sound may be disabled in some OS.
	
	@remarks Under Windowns Xp the application need to run under administrator credentials to
		get the realtime priorities.

    @remarks  Under Windows Vista and later, the application need to be run as "Run as Administrator"
		to get realtime priorities.
...
#define  BX_TXFILE		32  /*!< Transfer and draw to host using tcp             */
