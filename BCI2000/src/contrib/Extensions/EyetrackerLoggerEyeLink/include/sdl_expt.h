...
/*! Returns screen resolution in pixels and colour depth in bits per pixel of the current display mode, 
	 and fills the respective pointers to integer arguments accordingly. 
	 The main purpose of this function is to obtain the current resolution of the desktop environment 
	 before the call to initialize the main window.
	 @param csw Current Screen Width in pixels.
	 @param csh Current Screen Height in pixels.
	 @param ccd Current colour depth in bits per pixel.
	 @return \c 1 if success, \c 0 if failure to detect display mode.
	 @remark This function utilizes GDI under Windows, Quartz under Mac OS X and SDL under Ubuntu (requires SDL1.2.10 and up). 
 It may fail if none of these combinations are present.
	 ...
	 {
		 int screen_w, screen_h, pixel_d; // screen width, height and pixel depth
		 ...
		 if (get_current_display_information(&screen_w, &screen_h, &pixel_d)){
			 printf("Setting video mode to: %dx%dx%d\n", screen_w, screen_h, pixel_d);
			 mainwindow = SDL_SetVideoMode(screen_w,screen_h,pixel_d,SDL_SWSURFACE|SDL_FULLSCREEN); // set video mode
		 }else{
			 printf("Failed to detect current video mode. Setting to default: 800x600x32");
...
*/	 
int ELCALLTYPE get_current_display_information(int* csw, int* csh, int* ccd);
