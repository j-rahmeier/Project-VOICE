//////////////////////////////////////////////////////////////////////
// $Id: 5DTDataGloveUltraLogger.cpp 7993 2024-04-04 20:44:05Z mellinger $
...
{
	if(!m_enabled) {
		if(CountGloves()) bciout << "One or more datagloves are connected, but disabled. To enable acquisition, start the source module with --LogDataGlove=1";
		return;
	}
}
...
DataGloveLogger::AssignGloves(DataGloveThreadList & assigned) const
{
	...
	for(int iRequest = 0; iRequest < nGlovesRequested; iRequest++) {
		...
		if(found) {
			...
			EmptyList(available);
			EmptyList(assigned);
			bcierr << "could not find a match for requested glove #" << iRequest+1 << " (handedness requested: " << sWanted << ")";
		}
	}
	EmptyList(available);
}
...
DataGloveThread::DataGloveThread(const DataGloveLogger *parent, int usbPortNumber, bool deriv)
{
	...
	if(mpGlove == NULL) {
		...
		if (m_type != FD_GLOVE5U_USB && m_type != FD_GLOVE14U_USB) {
			m_err << "unrecognized data glove type on " << m_portString << std::endl;
		}
	}
}
	...
	while( !Terminating() ) {
		...
		for(int i = 0; i < MAX_SENSORS; i++) {
			if(currentData[i] != mPreviousOutput[i])
				bcievent << "Glove" << m_index << "Sensor" << std::setfill('0') << std::setw(2) << (i+1) << " " << currentData[i];
