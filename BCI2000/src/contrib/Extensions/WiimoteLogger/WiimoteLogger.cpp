#define report(x) #x<<"="<<(x)<<"; "
/////////////////////////////////////////////////////////////////////////////
// $Id: WiimoteLogger.cpp 7195 2023-02-07 18:31:03Z mellinger $
  ...
  while(m_numFound < MAX_WIIMOTES)
  {
    ...
    if(w)
    {
      ...
      bcidbg( 0 ) << "Wiimote " << m_numFound << " detected with battery at "
                  << (int)w->BatteryPercent << "%.";
    }
    w = NULL;
  }
  if( m_numFound == 0 )
  {
    bcierr << "No unused Wiimotes detected - if Wiimotes are connected, please disconnect, reconnect, and try again";
    return;
  }
  if( wiimoteIREnable )
    bcidbg( 0 ) << m_numFound << " Wiimotes Detected - Logging IR";
  else
    bcidbg( 0 ) << m_numFound << " Wiimotes Detected";
  ...
  if( wiimoteEnable )
  {
    for( unsigned int i = 0; i < m_numFound; i++ )
    {
      if( !m_wiimotes[i]->IsConnected() )
        bcierr << "Wiimote " << (i+1) << " has disconnected.";
  ...
  while( !Terminating() )
  {
    ...
    else
    {
        bcierr << "Wiimote " << (m_wiimoteNum+1) << " has disconnected.  "
               ...
               << "Please reconnect and restart the program.";
    }
  }
