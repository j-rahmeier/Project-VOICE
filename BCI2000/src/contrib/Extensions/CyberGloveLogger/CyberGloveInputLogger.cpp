//////////////////////////////////////////////////////////////////////
// $Id: CyberGloveInputLogger.cpp 7462 2023-06-30 14:27:11Z mellinger $
...
CyberGloveInputLogger::OpenInterface()
{
	...
	if (m_hinstLib == NULL) {
		if(m_enabled) bcierr << "Failed to find CGS_VirtualHandDevice.dll";
		return;
	}
	...
	if(!m_enabled) {
		if(CountGloves()) bciout << "One or more CyberGloves connected, but disabled. To enable acquisition, start the source module with --LogCyberGlove=1";
		return;
	}
	if(!good) {
		if(m_enabled) bcierr << "Not all the required functions were found in the dynamic link library CGS_VirtualHandDevice.dll";
		return;
	}
}
...
{   
	...
	if (checkKey(HKEY_LOCAL_MACHINE, "SOFTWARE\\CyberGlove Systems\\VirtualHand Drivers", "Device Registry File"))
	 ...
	 {//drivers not installed
	  bcierr << "CyberGlove drivers not installed!! Please install CyberGloveSystems 32 bit drivers!!";
	  return 0;
	 }
	...
	if(gloveConnLeft!=NULL)
		...
		//if(nGloves==0) {nGloves=0; bciout << "Max 2 CyberGloves supported.\n Naming Convention: RightGlove and LeftGlove in CyberGlove DCU accordingly.";}
	if(nGloves>2) {nGloves=0; bciout << "Max 2 CyberGloves supported.";}
	return nGloves;
}
...
CyberGloveInputLogger::AssignGloves(CyberGloveThreadList & assigned) const
{
	...
	for(int iRequest = 0; iRequest < nGlovesRequested; iRequest++) {
		std::string sWanted = handedness(iRequest);
		int hWanted;
		...
		if(     sWanted == "L" || sWanted == "l" || sWanted == "left"  || sWanted == "LEFT"  || sWanted == "0") hWanted = 0;
			...
			EmptyList(available);
			EmptyList(assigned);
			bcierr << "unrecognized DataGloveHandedness string \"" << sWanted << "\"";
		}
		...
		if(found) {
			...
			EmptyList(available);
			EmptyList(assigned);
			bcierr << "could not find a match for requested cyberglove #" << iRequest+1 << " (handedness requested: " << sWanted << ")";// 
		}
	}
	EmptyList(available);
	...
	while( !Terminating() ) {
		...
		for(int i = 0; i < (MAX_SENSORS+1); i++) { 
			if(currentData[i] != mPreviousOutput[i])
				bcievent << "CyberGlove" << m_index << "Sensor" << std::setfill('0') << std::setw(2) << (i+1) << " " << currentData[i];
