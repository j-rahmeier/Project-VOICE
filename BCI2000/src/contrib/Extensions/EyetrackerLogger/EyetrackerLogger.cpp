/////////////////////////////////////////////////////////////////////////////
// $Id: EyetrackerLogger.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
// **************************************************************************
bool EyetrackerLogger::Connect(const std::string &address, int port) const
{
  ...
  if(res != TET_NO_ERROR )
  {
    ...
    char pErrBuf[255];
    bcierr << "Failed to connect to Eyetracker. Tried Address: " << address << ", Port: " << port;
    // TODO: next error never gets issued for some reason
    bcierr << "Reason: " << DLLFUNC_POINTER_CALL(Tet_GetLastError)() << " - " << DLLFUNC_POINTER_CALL(Tet_GetLastErrorAsText)(pErrBuf);
    return false;
  }
...
bool EyetrackerLogger::Disconnect() const
{
  ...
  if(res != TET_NO_ERROR )
  {
    ...
    char pErrBuf[255];
    bcierr << "Failed to disconnect from Eyetracker.";
    bcierr << "Problem: " << DLLFUNC_POINTER_CALL(Tet_GetLastError)() << " - " << DLLFUNC_POINTER_CALL(Tet_GetLastErrorAsText)(pErrBuf);
    return false;
  }
...
// Returns:    True or False (Success or Failure)
// **************************************************************************
bool EyetrackerLogger::LoadCalibration(const std::string &filepath) const
{
  ...
  if(res != TET_NO_ERROR )
  {
    //Tell the user what's up.
    char pErrBuf[255];
    bcierr << "Calibration File Load Failed. -- Tried to load: " << filepath;
    bcierr << "Problem: " << DLLFUNC_POINTER_CALL(Tet_GetLastError)() << " - " << DLLFUNC_POINTER_CALL(Tet_GetLastErrorAsText)(pErrBuf);
    return false;
  }
...
bool EyetrackerLogger::PrintSerialNumbers() const
{
  ...
  if(res != TET_NO_ERROR )
  {
    //Tell the user what's up.
    char pErrBuf[255];
    bcierr << "Failed to retreive serial numbers.";
    bcierr << "Problem: " << DLLFUNC_POINTER_CALL(Tet_GetLastError)() << " - " << DLLFUNC_POINTER_CALL(Tet_GetLastErrorAsText)(pErrBuf);
    return false;
  }

  bcidbg( 0 ) << "Eyetracker Found, Serial Numbers: " << pSerNo1 << ", " << pSerNo2;
  return true;
}
...
{
  bool eyetrackerEnable = false;
  std::string eyetrackerAddress;
  std::string eyetrackerCalibrationFilepath;
  ...
  if( eyetrackerEnable )
  {
    eyetrackerAddress = ( std::string )Parameter( "NetworkLocation" );
    ...
    if(Connect(eyetrackerAddress, eyetrackerPort))
    {
      ...
      if(PerformSystemCheck())
      {
        ...
        {
          bcidbg( 0 ) << "Connected to eyetracker successfully.";
        }
      }
    }
...
void EyetrackerLogger::Initialize()
{
  ...
  if(m_eyetrackerEnable)
  {
    ...
    m_eyetrackerAddress = ( std::string )Parameter( "NetworkLocation" );
...
: m_DataOK( false ),
  m_prevDataOK( false )
{
  //Initialize everything to its proper value
  m_running = true;
  m_disabled = false;
  m_eyetracker = logger;
...
EyetrackerLogger::EyetrackerThread::~EyetrackerThread()
{
  if(m_eyetracker->Disconnect())
    bcidbg( 10 ) << "3. Disconnect() - Logging of eyetracker stopped.";
}
...
// Returns:    Always zero
// **************************************************************************
int EyetrackerLogger::EyetrackerThread::OnExecute()
{
  if( m_running )
  {
...
void EyetrackerLogger::EyetrackerThread::StartTracking()
{
  ...
  if(res != TET_NO_ERROR )
  {
    //Tell the user what's up.
    char pErrBuf[255];
    bcidbg( 0 ) << "Failed to start Eyetracking.";
    bcidbg( 0 ) << "Problem: " << DLLFUNC_POINTER_CALL(Tet_GetLastError)() << " - " << DLLFUNC_POINTER_CALL(Tet_GetLastErrorAsText)(pErrBuf);
  }
  else
    bcidbg( 0 ) << "Successful Eyetracking Session";
}
...
void EyetrackerLogger::EyetrackerThread::StopTracking()
{
  ...
  if(res != TET_NO_ERROR )
  {
    //Tell the user what's up.
    char pErrBuf[255];
    bcierr << "Failed to stop tracking.";
    bcierr << "Problem: " << DLLFUNC_POINTER_CALL(Tet_GetLastError)() << " - " << DLLFUNC_POINTER_CALL(Tet_GetLastErrorAsText)(pErrBuf);
  }
  else
    bcidbg( 10 ) << "2. Tet_Stop - Stopped Eyetracking";
