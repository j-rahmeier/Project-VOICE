#!/bin/bash
#####################################################################
# $Id: retrieve_helppages.sh 6784 2022-06-08 16:43:49Z mellinger $
...
#
# (C) 2000-2022, BCI2000 Project
# http://www.bci2000.org
#####################################################################

INITIALWD=`pwd`;
SERVER="www.bci2000.org"
BASE="$SERVER/mediawiki"
TARGET="../../../doc/htmlhelp"
TMPFILE="~cookies"
TMPEXT="~tmp"
TOCFILE="htmlhelp.toc"

# Log in to the HTMLhelp account.
wget --spider --keep-session-cookies --save-cookies $TMPFILE \
     http://$BASE/index.php/Special:UserLogin?helplogin=1
...
do
  ( 
    ...
    sed '/<li class=\"toclevel-.*<\/a>/!d' < $i |
    sed 's/.*<li class=\"toclevel-\([0-9]*\)[^\"]*\"><a href=\"[^#]*#\([^\"]*\)\"><span class=\"tocnumber\">[^<]*<\/span> <span class=\"toctext\">\([^<]*\).*/\1 \2 \3/'
  ) >> $TOCFILE
done;
echo >> $TOCFILE
