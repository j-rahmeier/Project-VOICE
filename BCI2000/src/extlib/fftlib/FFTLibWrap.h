////////////////////////////////////////////////////////////////////////////////
// $Id: FFTLibWrap.h 7195 2023-02-07 18:31:03Z mellinger $
...
inline FFTLibWrapper::Real &RealFFT::Input(int index)
{
    Assert(mpInputData != 0 && index < mFFTSize);
    return static_cast<Real *>(mpInputData)[index];
}

inline const FFTLibWrapper::Real &RealFFT::Output(int index) const
{
    Assert(mpOutputData != 0 && index < mFFTSize);
    return static_cast<Real *>(mpOutputData)[index];
}

inline FFTLibWrapper::Complex &ComplexFFT::Input(int index)
{
    Assert(mpInputData != 0 && index < mFFTSize);
    return static_cast<Complex *>(mpInputData)[index];
}

inline const FFTLibWrapper::Complex &ComplexFFT::Output(int index) const
{
    Assert(mpOutputData != 0 && index < mFFTSize);
