////////////////////////////////////////////////////////////////////////////////
// $Id: ComModule.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
//   with the MIDL-defined .tlb library contained in your module. There is only
//   one library supported per module.
...
Module::GetLocation()
{
  if( spLocation == NULL )
  {
    ...
    if( pPath != 0 )
    {
      spLocation = new wchar_t[::wcslen( pPath ) + 2];
...
Module::GetTypeLib()
{
  if (!spTypeLib && FAILED(::LoadTypeLibEx(GetFileName(), REGKIND_NONE, spTypeLib.Assignee()))) {
    std::wstring info = L"Could not load type library" + std::wstring(GetFileName())
                      + L"\n-- make sure DLL file contains a TYPELIB resource";
    ::MessageBoxW(NULL, info.c_str(), L"Fatal Error", MB_OK|MB_ICONERROR);
  }
  return spTypeLib;
}
...
{
  bool unregisterTypeLib = false;
  Ptr<ITypeLib> pTypeLib;
  if (!FAILED(::LoadTypeLibEx(GetFileName(), REGKIND_NONE, pTypeLib.Assignee())))
  {
      unregisterTypeLib = true;
  ...
  if (unregisterTypeLib && TypeLibKeyExists(inType, true))
      result = SELFREG_E_TYPELIB;
  return result;
}
...
Module::AvailableToAllUsers()
{
  ...
  TRUSTEEA trustee = { 0, NO_MULTIPLE_TRUSTEE, TRUSTEE_IS_SID, TRUSTEE_IS_UNKNOWN, reinterpret_cast<LPSTR>( pUsersGroup ) };
...
Module::RunRegScript( HMODULE inHModule, LPCWSTR inType, LPWSTR inName, LONG_PTR inParam )
{
  ...
  std::string path = DualString( GetLocation() );
  environment["MODULEPATH"] = path;
  environment["MODULEPATH,1"] = ParentDirectory( path );
  environment["MODULEPATH,2"] = ParentDirectory( ParentDirectory( path ) );
  Registrar registrar;
