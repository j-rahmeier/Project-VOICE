////////////////////////////////////////////////////////////////////////////////
// $Id: MEMPredictor.h 7195 2023-02-07 18:31:03Z mellinger $
...
template <typename T> class MEMPredictor : public LinearPredictor<T>
{
  public:
  ...
  public:
    ...
    void TransferFunction(const DataVector &, Ratpoly<T> &) const override;

  private:
    mutable DataVector mWk1, mWk2;
};

// Implementation
template <typename T> MEMPredictor<T>::MEMPredictor()
{
}

template <typename T> void MEMPredictor<T>::TransferFunction(const DataVector &inData, Ratpoly<T> &outResult) const
{
    typedef double D;
    ...
    outResult = Ratpoly<T>(Polynomial<T>(std::sqrt(meanPower)), Polynomial<T>::FromCoefficients(coeff));
}
