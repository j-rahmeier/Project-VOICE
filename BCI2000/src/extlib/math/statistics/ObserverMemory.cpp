////////////////////////////////////////////////////////////////////////////////
// $Id: ObserverMemory.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
Matrix &Matrix::operator*=(Number inN)
{
...
Matrix &Matrix::SubtractOuterProduct(const Vector &inV)
{
...
VectorPtr MemPool::NewVector(size_t inLength)
{
    std::list<VectorPtr::Record>::iterator i = mVectors.begin();
...
MatrixPtr MemPool::NewMatrix(size_t inRows, size_t inCols)
{
    std::list<MatrixPtr::Record>::iterator i = mMatrices.begin();
...
void MemPool::CollectGarbage()
{
    CollectGarbage<Vector>(mVectors);
    CollectGarbage<Matrix>(mMatrices);
}

template <typename T> void MemPool::CollectGarbage(std::list<typename ObjectPtr<T>::Record> &ioList)
{
    typename std::list<typename ObjectPtr<T>::Record>::iterator i = ioList.begin();
    while (i != ioList.end())
        if (i->usage < 1)
            i = ioList.erase(i);
        else
