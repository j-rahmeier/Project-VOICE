////////////////////////////////////////////////////////////////////////////////
// $Id: ObserverBase.h 7195 2023-02-07 18:31:03Z mellinger $
...
class ObserverBase
{
  ...
  public:
    ...
    VectorPtr Variance(
        MemPool & = GlobalMemPool()) const; // Having no definition of unbiased (co)variance for aging observers,
    MatrixPtr Covariance(
        MemPool & = GlobalMemPool()) const; // this is always the biased (co)variance of the distribution.
    MatrixPtr Correlation(
        MemPool & = GlobalMemPool()) const; // Note that not all observers support computation of covariance.
    ...
    VectorPtr CentralMoment(unsigned int, MemPool & = GlobalMemPool())
        const; // Not all observers support computation of general central moments.
  ...
  protected:
    ...
    {
    } // A property of the ObserverBase class has changed.
  ...
  public:
    ...
    virtual MatrixPtr PowerSum2Full(
        MemPool &) const; // Return the full matrix of the 2nd power.
    ...
    virtual VectorPtr InverseCDF(Number, MemPool &)
        const; // The inverse cumulated distribution function for each dimension (non-normalized argument).
  ...
  private:
    ...
    int mConfig;
    Number mWindowLength, mDecayFactor;
    Number mAge;
    Number mQuantileAccuracy, mActualAccuracy;
