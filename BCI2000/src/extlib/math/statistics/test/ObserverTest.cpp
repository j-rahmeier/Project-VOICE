...
{ // Approximation by Winitzki, S., 2008
    ...
    double signx = (x > 0) - (x < 0), b = log(1 - x * x), c = 2 / M_PI / a + b / 2, result = 0;
    result = c * c - b / a;
    result = sqrt(result);
    result -= c;
...
ObserverBase *CreateObserver(const char *inName)
{
    string name(inName);
...
void PrintMatrix(const Matrix &inM)
{
...
int main_(int argc, char **argv)
{
    ...
    bool help = false;
    int randseed = 0, numsamples = -1, metabins = 10, showskewkurtosis = 0;
    double mean = 0, sinefreq = 0, sinevar = 0, noisevar = 0, gaussvar = 0, windowlength = -1, accuracy = -2,
           leftaccuracy = -1, quantile = 0.5;
    const char *left = "FullyConfigured", *right = "WindowObserver";
    ...
    double sineamp = sqrt(2 * sinevar), noiseamp = 2 * sqrt(3 * noisevar), gaussamp = sqrt(gaussvar);
    if (accuracy != -2)
    ...
    try
    {
        ObserverBase *pLeft = CreateObserver(left), *pRight = CreateObserver(right);
        ...
        for (int sample = 1; (sample <= numsamples || numsamples < 0) && !gStop; ++sample)
        {
            ...
            if (quantile >= 0)
            {
                ...
                Number meandiff = p.Mean()()[1] - p.Mean()()[0], dev = sqrt(p.Variance()()[0] + meandiff * meandiff);
                if (dev > eps && metabins > 0)
                {
                    Number center = p.Mean()()[0] + meandiff / 2, resolution = 3 * dev / metabins;
        ...
        if (quantile >= 0)
        {
            ...
            double mean1 = p.Mean()()[0], mean2 = p.Mean()()[1], meandiff = mean1 - mean2, var1 = p.Variance()()[0],
                   var2 = p.Variance()()[1], cov = p.Covariance()()[0][1];
            cout << "\t\t" << '\t' << mean1 << ",\t" << mean2 << ",\t" << var1 << ",\t" << var2 << ",\t" << cov << ",\t"
                 << p.Correlation()()[0][1] << ",\t" << RSquared(p, p)()[0][1] << endl;
            ...
            double dev1 =
                       sqrt(var1 + (meandiff *
                                    meandiff)), // Standard deviation relative to mean2, accounts for mean difference.
                dev2 = sqrt(var2),
                   // To convert from deviation into quantile accuracy, we divide deviation by the slope of the inverse
                   // CDF, which is just the inverse value of the distribution itself.
                invslope = -1;
            ...
            if (invslope != -1)
            {
                cout << "Quantiles: empirical accuracies:" << '\t' << dev1 * invslope << '\t' << dev2 * invslope
                     << endl;
            }
        }
        ...
        if (metabins > 0)
        {
            ...
            double mean1 = h.Mean()()[0], mean2 = h.Mean()()[1], meandiff = mean2 - mean1, var1 = h.Variance()()[0],
                   var2 = h.Variance()()[1], cov = h.Covariance()()[0][1];
            cout << "\t\t" << '\t' << mean1 << ",\t" << mean2 << ",\t" << var1 << ",\t" << var2 << ",\t" << cov << ",\t"
                 << h.Correlation()()[0][1] << ",\t" << RSquared(h, h)()[0][1] << endl;
...
int main(int argc, char **argv)
{
    ...
    bool finished = ExceptionCatcher().SetMessage("Terminating").Run(call);
