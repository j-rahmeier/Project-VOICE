////////////////////////////////////////////////////////////////////////////////
// $Id: ObserverMemory.h 7195 2023-02-07 18:31:03Z mellinger $
...
inline Vector &transform(Vector &v, Number fn(Number))
{
...
{
    explicit Matrix(size_t rows = 0, size_t cols = 0) : std::vector<Vector>(rows, Vector(cols))
    {
    }
...
{ // A smart pointer class that is linked to a minimal object record.
  ...
  public:
    ...
    explicit ObjectPtr(Record &record = sNullRecord) : mpRecord(&record)
    {
        ++mpRecord->usage;
    }
    ObjectPtr(const ObjectPtr &other) : mpRecord(other.mpRecord)
    {
        ++mpRecord->usage;
    }
    ~ObjectPtr()
...
template <typename T> typename ObjectPtr<T>::Record ObjectPtr<T>::sNullRecord = {0, T()};
...
MemPool &GlobalMemPool(); // Provides a default memory pool.
                          ...
                          // Then, locking is only required between a single observation thread, and each computation
                          // thread; no locking is required between computation threads.
