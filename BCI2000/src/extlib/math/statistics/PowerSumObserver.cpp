////////////////////////////////////////////////////////////////////////////////
// $Id: PowerSumObserver.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
PowerSumObserver::PowerSumObserver(int inConfig)
    : ObserverBase(inConfig, Supports), mCovarianceRequired(ImpliedConfig(inConfig) & StatisticalObserver::Covariance),
      mPowerSum0(0)
{
}

void PowerSumObserver::DoChange()
{
    if (mPowerSum1.size() != SampleSize())
        DoClear();
}

void PowerSumObserver::DoAgeBy(unsigned int inCount)
{
    if (DecayFactor() == 1)
        return;
...
void PowerSumObserver::DoObserve(const Vector &inV, Number inWeight)
{
    mPowerSum0 += inWeight;
...
VectorPtr PowerSumObserver::PowerSum1(MemPool &ioPool) const
{
...
VectorPtr PowerSumObserver::PowerSum2Diag(MemPool &ioPool) const
{
    VectorPtr result = ioPool.NewVector(SampleSize());
...
MatrixPtr PowerSumObserver::PowerSum2Full(MemPool &ioPool) const
{
