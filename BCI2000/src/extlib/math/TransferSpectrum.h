////////////////////////////////////////////////////////////////////////////////
// $Id: TransferSpectrum.h 7195 2023-02-07 18:31:03Z mellinger $
...
template <typename T> class TransferSpectrum
{
  public:
    TransferSpectrum() : mFirstBinCenter(0), mBinWidth(1), mNumBins(0), mEvaluationsPerBin(1)
    {
    }
    // Configuration
    ...
    // Processing
    template <typename U, typename V> const TransferSpectrum &Evaluate(const Ratpoly<U> &, std::valarray<V> &) const;

  private:
    TransferSpectrum &Init();

    T mFirstBinCenter, mBinWidth;
    int mNumBins, mEvaluationsPerBin;
    std::valarray<std::complex<T>> mLookupTable;
};

// Implementation
template <typename T> TransferSpectrum<T> &TransferSpectrum<T>::Init()
{
...
const TransferSpectrum<T> &TransferSpectrum<T>::Evaluate(const Ratpoly<U> &inFunction,
                                                         std::valarray<V> &outResult) const
{
    ...
    for (int bin = 0; bin < mNumBins; ++bin)
    {
        ...
        for (int sample = 0; sample < mEvaluationsPerBin; ++sample)
        {
            std::complex<T> value = inFunction.Evaluate(mLookupTable[mEvaluationsPerBin * bin + sample]);
