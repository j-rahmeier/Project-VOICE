////////////////////////////////////////////////////////////////////////////////
// $Id: VisDisplayMemo.cpp 7849 2024-02-21 18:20:24Z mellinger $
...
VisDisplayMemo::VisDisplayMemo(const std::string &inSourceID)
    : VisDisplayLayer(inSourceID), mNumLines(0), mpTextDisplay(new QTextEdit)
{
...
void VisDisplayMemo::OnSetConfig(ConfigSettings &inConfig)
{
    VisDisplayBase::OnSetConfig(inConfig);
    inConfig.Get(CfgID::NumLines, mNumLines);
    if (mNumLines < 1)
        mNumLines = std::numeric_limits<int>::max();
}

void VisDisplayMemo::OnMemo(const QString &inText)
{
#ifdef TODO
#error Remove lines when their number exceeds the numlines property.
#endif
    std::string s = inText.toLocal8Bit().constData();
    size_t pos = 0;
