//////////////////////////////////////////////////////////////////////
// $Id: ParserToken.h 7195 2023-02-07 18:31:03Z mellinger $
...
namespace std
{
inline istream &getline(istream &s, ParserToken &h, char delim = '\n')
{
    return h.GetLine(s, delim);
}
} // namespace std

inline std::ostream &operator<<(std::ostream &os, const ParserToken &s)
{
    return s.InsertInto(os);
}

inline std::istream &operator>>(std::istream &is, ParserToken &s)
{
    return s.ExtractFrom(is);
}

template <typename T> std::istream &ParserToken::ReadUntil(std::istream &is, T predicate)
{
    clear();
