////////////////////////////////////////////////////////////////////////////////
// $Id: ObjectType.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
bool ObjectType::Execute(const std::string &inVerb, CommandInterpreter &inInterpreter) const
{
...
void ObjectType::Help(CommandInterpreter &inInterpreter) const
{
    std::string token = inInterpreter.GetOptionalToken();
    OnHelp(inInterpreter, !::stricmp(token.c_str(), "all"));
}

void ObjectType::ListMethods(std::ostream &os, bool all) const
{
    OnListMethods(os, all);
}

void ObjectType::Initialize(class StateMachine &inStateMachine)
{
    for (TypeDictionary::const_iterator i = Dictionary().begin(); i != Dictionary().end(); ++i)
        (*i)->OnInitialize(inStateMachine);
}

ObjectType *ObjectType::ByName(const char *inName)
{
    for (TypeDictionary::const_iterator i = Dictionary().begin(); i != Dictionary().end(); ++i)
    {
...
ObjectType *ObjectType::Next(const ObjectType *inP)
{
...
void ObjectType::OnHelp(CommandInterpreter &inInterpreter, bool all) const
{
    if (MethodTable()->verb)
    {
...
void ObjectType::OnListMethods(std::ostream &os, bool all) const
{
...
ObjectType::TypeDictionary &ObjectType::Dictionary()
{
