////////////////////////////////////////////////////////////////////////////////
// $Id: ProcessType.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
const ObjectType::MethodEntry ProcessType::sMethodTable[] = {
    METHOD(Create),  {"Start", &Create}, METHOD(Terminate), {"Kill", &Terminate},
    METHOD(WaitFor), METHOD(Show),       METHOD(Hide),      END};

bool ProcessType::Create(CommandInterpreter &inInterpreter)
{
    ...
    bool ok = ProcessUtils::StartProcess(executable, arguments, exitCode, pid);
    if (pid && ok)
...
bool ProcessType::Terminate(CommandInterpreter &inInterpreter)
{
...
bool ProcessType::WaitFor(CommandInterpreter &inInterpreter)
{
    auto pid = GetPidArgument(inInterpreter);
    ...
    inInterpreter.Out() << ((timeLeft >= 0) ? "" : "false");
    return true;
}

bool ProcessType::Show(CommandInterpreter &inInterpreter)
{
...
bool ProcessType::Hide(CommandInterpreter &inInterpreter)
{
    ProcessUtils::Pid pid = GetPidArgument(inInterpreter);
...
std::string ProcessType::ToShellArguments(const std::string &inTokens)
{
    std::string result;
...
ProcessUtils::Pid ProcessType::GetPidArgument(CommandInterpreter &inInterpreter)
{
