////////////////////////////////////////////////////////////////////////////////
// $Id: SystemStates.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
#define ENTRY(x)                                                                                                       \
    ...
    ENTRY(Unavailable), ENTRY(Idle),      ENTRY(Startup),        ENTRY(Initialization), ENTRY(Connected),
    ENTRY(Resting),     ENTRY(Suspended), ENTRY(ParamsModified), ENTRY(Running),        ENTRY(Busy),
#undef ENTRY
};
static size_t sStateCount = sizeof(sSystemStates) / sizeof(*sSystemStates);

const char *SystemStates::Name(int inValue)
{
    const char *result = "";
...
int SystemStates::Value(const std::string &inName)
{
    int result = BCI_None;
