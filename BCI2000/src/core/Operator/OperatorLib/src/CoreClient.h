////////////////////////////////////////////////////////////////////////////////
// $Id: CoreClient.h 7463 2023-06-30 14:59:17Z mellinger $
...
class CoreClient : public CoreConnection, public Lockable<std::recursive_mutex>
{
  public:
    struct Info
    {
        Info()
            : Version(ProtocolVersion::None()), Status("no status available"), MessagesSent(0), MessagesRecv(0),
    ...
    class List : private Lockable<std::mutex>
    {
      public:
        ~List()
        {
        ...
        CoreClient* operator[](size_t i)
        {
            return mList[i];
        }
        const CoreClient* operator[](size_t i) const
        {
            return mList[i];
        }
        bool Wait(int);
