////////////////////////////////////////////////////////////////////////////////
// $Id: Dataplot.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
std::string findGnuplot()
{
    ...
    {
        path.resize(size);
        HRESULT r = ::AssocQueryStringA(ASSOCF_NOTRUNCATE, ASSOCSTR_EXECUTABLE, extension, verb,
                                        const_cast<char *>(path.data()), &size);
        if (FAILED(r))
            path.clear();
        else
        {
...
class Gnuplot : private Lockable<>
{
  public:
    ...
    bool execute(const std::string &s, std::string &error)
    {
        ...
        WithLock(this)
        {
            ...
            if (!mProcess.running())
            {
                mProcess.run();
                io << "set print \"-\"\n"
                   << "set terminal " << sTerminalType << " title \"" << FileUtils::ApplicationTitle() << "\""
                   << std::endl;
            }
    ...
    void exit()
    {
        if (!mProcess.running())
            return;
        WithTryLock(this) std::ostream(&mProcess.standardIO()) << "exit gnuplot" << std::endl;
        if (!mProcess.Wait(Time::Seconds(1)))
        {
            ProcessUtils::Pid pid = mProcess.pid();
...
struct Dataplot::Private
{
    ...
    bool display(const char *newWindowTitle, int w, int h, bool withPos, int x, int y, bool wait)
    {
        ...
        // Input from stdin is broken for gnuplot on windows, resulting in some lines
        // not being executed. We don't set the window title in order to avoid the wrong title
        // popping up when a new window is shown.
        term << sTerminalType << " ";// << mWindowId << " title \"" << promptWindowTitle << "\"";
        if (w > 0 && h > 0)
            term << " size " << w << "," << h;
        if (withPos)
            term << " position " << x << "," << y;
        ...
        if (wait)
        {
            std::ostringstream oss;
            oss << "pause mouse key\n";
            oss << "set terminal " << sTerminalType << " " << mWindowId << " title \"" << windowTitle << "\"\n"
                << "replot\n";
            oss.flush();
            cmd += oss.str();
        }
...
Dataplot::Dataplot(const std::string &title) : p(new Private)
{
    p->mData << "reset\n";
...
std::string Dataplot::addPoints(const std::string &basename, const std::vector<std::pair<double, double>> &points)
{
    std::string name = p->makeVariableName(basename);
    p->mData << "$" << name << " << EOD\n  "; // spaces are a workaround for broken gnuplot stdin
    for (auto v : points)
        p->mData << v.first << " " << v.second << "\n";
    p->mData << "EOD\n\n"; // additional newline is a workaround for broken gnuplot stdin
    return name;
}

std::string Dataplot::addData(const std::string &basename, const std::vector<double> &data, double scale)
{
    std::string name = p->makeVariableName(basename);
    p->mData << "$" << name << " << EOD\n";
    for (auto v : data)
...
std::string Dataplot::makePlotdef(const std::string &varname, const std::string &style, const std::string &title)
{
    return "$" + varname + " title \"" + (title.empty() ? varname : title) + "\" with " + style;
}

void Dataplot::addCommand(const std::string &cmd)
{
    p->mData << Gnuplot::fromUtf8(cmd) << std::endl;
}

bool Dataplot::display(bool wait)
{
    return p->display(nullptr, 0, 0, false, 0, 0, wait);
}

bool Dataplot::display(const std::string &title, bool wait)
{
    return p->display(title.c_str(), 0, 0, false, 0, 0, wait);
}

bool Dataplot::display(const std::string &title, int width, int height, bool wait)
{
    return p->display(title.c_str(), width, height, false, 0, 0, wait);
}

bool Dataplot::display(const std::string &title, int width, int height, int xpos, int ypos, bool wait)
{
