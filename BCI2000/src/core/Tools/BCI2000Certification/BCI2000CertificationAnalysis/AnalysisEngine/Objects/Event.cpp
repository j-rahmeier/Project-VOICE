////////////////////////////////////////////////////////////////////////////////
// $Id: Event.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
AnalysisDataObject *Event::create(Context *p, const ExpressionParser::NodeList &args)
{
...
Event::Event(Context *p, const SampleDataObject *condition, const SampleDataObject *value)
    : AnalysisDataObject(p), mpCondition(condition), mpValue(value), mpSampleClock(nullptr), mNullvalue(0)
{
    mDepends.add(mpCondition).add(mpValue);
}

Event::Event(Context *p, const SampleDataObject *state, double nullvalue)
    : AnalysisDataObject(p), mpCondition(state), mpValue(nullptr), mpSampleClock(nullptr), mNullvalue(nullvalue)
{
    mDepends.add(mpCondition);
}

const Event::InstanceList &Event::instances() const
{
    triggerEvaluation();
    return mInstances;
}

bool Event::onPostEvalSamples()
{
...
void Event::evaluateFromCondition()
{
...
std::string Event::onSummary() const
{
...
bool Event::onGetPlotData(Dataplot::PointList &points) const
{
...
void Event::onGetPlotProperties(PlotProperties &prop) const
{
    prop.with = "points";
