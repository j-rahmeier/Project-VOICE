////////////////////////////////////////////////////////////////////////////////
// $Id: TimeSeriesDataObject.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
TimeSeriesDataObject::TimeSeriesDataObject(Context *p) : SampleDataObject(p), mpSampleClock(nullptr)
{
}

double TimeSeriesDataObject::onCacheSampleValue()
{
    throw bciexception << "TimeSeriesDataObject::onCacheSampleValue(): "
                       << "function unexpectedly called for object " << name() << " of type " << type();
}

double TimeSeriesDataObject::onEvalMemberFunc() const
{
    if (!valid())
        return NaN<double>();
...
bool TimeSeriesDataObject::onGetPlotData(Dataplot::PointList &points) const
{
    if (mpSampleClock)
    {
        const auto &v = data();
...
void TimeSeriesDataObject::onGetPlotProperties(PlotProperties &prop) const
{
