////////////////////////////////////////////////////////////////////////////////
// $Id: InputData.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
AnalysisDataObject *State::create(AnalysisDataObject::Context *p, const std::string &name)
{
    return new State(p, name);
}

AnalysisDataObject *State::create(AnalysisDataObject::Context *p, const ExpressionParser::NodeList &args)
{
    if (args.size() != 1)
...
State::State(Context *p, const std::string &name) : TimeSeriesDataObject(p), mStateName(name)
{
...
double State::onCacheSampleValue()
{
    return mState;
}

std::string State::onSummary() const
{
...
double Channel::onCacheSampleValue()
{
    return context()->reader->RawValue(mChannel, *context()->sample);
}

std::string Channel::onSummary() const
{
    return StringUtils::String() << "'" << mChannelName << "', n=" << mData.size();
}

AnalysisDataObject *Channel::create(AnalysisDataObject::Context *p, const ExpressionParser::NodeList &args)
{
    if (args.size() != 1)
    ...
    if (pString)
        ...
        chName = std::string(StringUtils::String() << args[0]->Evaluate());
