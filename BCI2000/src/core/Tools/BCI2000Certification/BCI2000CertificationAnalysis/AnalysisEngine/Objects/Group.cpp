////////////////////////////////////////////////////////////////////////////////
// $Id: Group.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
{
    Group *p;
    AddNode(Group *p, const ExpressionParser::NodeList &nodes) : p(p)
    {
        AddChildren(nodes);
    }
...
AnalysisDataObject *Group::create(Context *p, const ExpressionParser::NodeList &args)
{
    std::string desc;
    int i = 0;
    if (i < args.size())
...
Group::Group(Context *p, const std::string &desc, const UniqueList &members) : AnalysisDataObject(p), mDescription(desc)
{
    mDepends = members;
}

double Group::add(double d)
{
...
ExpressionParser::Node *Group::onMemberFunction(Context *p, const std::string &function,
...
std::string Group::onDescription() const
{
...
void Group::addPlots(Dataplot &plot, std::vector<std::string> &plotdefs, std::string &xlabel, std::string &ylabel) const
{
