////////////////////////////////////////////////////////////////////////////////
// $Id: FrameStatistics.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
AnalysisDataObject *FrameStatistics::create(AnalysisDataObject::Context *p, const ExpressionParser::NodeList &args)
{
    if (args.size() < 1)
...
JointStatistics *FrameStatistics::onCreateJointStatistics(Context *p) const
{
    return new FrameStatistics(p, nullptr, mFrameDuration);
}

std::string FrameStatistics::onSummary() const
{
    std::ostringstream oss;
    oss << "total " << Pretty(total()) << ", lost " << Pretty(lost()) << " = " << std::setprecision(3)
        << lost() / total() * 100 << "%";
    return oss.str();
}

ExpressionParser::Node *FrameStatistics::onMemberFunction(Context *p, const std::string &function,
...
bool FrameStatistics::onPostEvalSamples()
{
...
double FrameStatistics::total() const
{
    return triggerEvaluation() ? mTotal : NaN<double>();
}

double FrameStatistics::lost() const
{
    return triggerEvaluation() ? mLost : NaN<double>();
}

double FrameStatistics::frameDuration() const
{
    return mFrameDuration;
}

double FrameStatistics::relativeLoss() const
{
