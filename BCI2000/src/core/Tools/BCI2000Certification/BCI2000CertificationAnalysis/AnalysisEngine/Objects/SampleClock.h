////////////////////////////////////////////////////////////////////////////////
// $Id: SampleClock.h 7463 2023-06-30 14:59:17Z mellinger $
...
{ // reference timestamp for each sample in terms of machine clock
  public:
    ...
    SampleClock(Context *, int64_t ignoreBlocksAtBegin = 1, int64_t ignoreBlocksAtEnd = 1);

    double position() const;
    double fileDuration() const;
    double sampleDuration() const;
    double blockDuration() const;
    double blocksIgnoredAtBegin() const;
    double blocksIgnoredAtEnd() const;
  ...
  protected:
    ...
    ExpressionParser::Node *onMemberFunction(Context *, const std::string &,
                                             const ExpressionParser::NodeList &) override;

  private:
    Ratio mNominalRate_Hz, mMeasuredRate_Hz;
    double mFileDuration, mSampleDuration, mBlockDuration, mRsq;
    int64_t mSampleBlockSize, mNumSamples, mIgnoreBlocksAtBegin, mIgnoreBlocksAtEnd;
};
