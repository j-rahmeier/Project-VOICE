////////////////////////////////////////////////////////////////////////////////
// $Id: Timestamp.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
AnalysisDataObject *Timestamp::create(Context *p, const ExpressionParser::NodeList &args)
{
    if (args.Size() != 1)
...
void Timestamp::onPreEvalSamples()
{
    ...
    mWraparound = (1LL << mState->Length());
    mUnit = *context()->timestampUnit;
    mFirstValue = -1;
    mIsConstant = true;
}

double Timestamp::onCacheSampleValue()
{
    if (mFirstValue < 0)
        mFirstValue = mState.AsUnsigned();
    else if (mFirstValue != mState.AsUnsigned())
        mIsConstant = false;
...
bool Timestamp::onPostEvalSamples()
{
    if (mIsConstant)
        invalidate("is constant");
    return true;
}

std::string Timestamp::onSummary() const
{
