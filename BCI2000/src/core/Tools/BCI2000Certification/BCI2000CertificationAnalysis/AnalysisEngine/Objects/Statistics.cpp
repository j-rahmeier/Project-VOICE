...
StatisticsObject::StatisticsObject(Context *p) : AnalysisDataObject(p)
{
}
...
double StatisticsObject::count() const
{
    return triggerEvaluation() ? mStats.count() : NaN<double>();
}

double StatisticsObject::mean() const
{
    return triggerEvaluation() ? mStats.mean() : NaN<double>();
}

double StatisticsObject::var() const
{
    return triggerEvaluation() ? mStats.var() : NaN<double>();
}

double StatisticsObject::sdev() const
{
    return ::sqrt(var());
}

double StatisticsObject::mode() const
{
    return triggerEvaluation() ? mStats.mode() : NaN<double>();
}

double StatisticsObject::quantile(double p) const
{
    return triggerEvaluation() ? mStats.quantile(p) : NaN<double>();
}

const std::vector<double> &StatisticsObject::sortedData() const
{
    triggerEvaluation();
...
JointStatistics *StatisticsObject::createJointStatistics(Context *p) const
{
    return onCreateJointStatistics(p);
}

JointStatistics *StatisticsObject::onCreateJointStatistics(Context *p) const
{
...
std::string StatisticsObject::onSummary() const
{
    ...
    oss << "n=" << Pretty(count()) << ", median=" << std::setprecision(3) << unit().RawToPhysical(median()).Utf8()
        << ", sdev=" << unit().RawToPhysical(sdev()).Utf8();
    return oss.str();
}

ExpressionParser::Node *StatisticsObject::onMemberFunction(Context *p, const std::string &function,
...
bool StatisticsObject::onGetPlotData(Dataplot::PointList &points) const
{
...
void StatisticsObject::onGetPlotProperties(PlotProperties &prop) const
{
    prop.with = "lines";
...
// JointStatistics
JointStatistics::JointStatistics(Context *p) : StatisticsObject(p)
{
}

void JointStatistics::joinWith(const StatisticsObject *p)
{
    mJoinWith.push_back(p);
    mDepends.add(p);
}

bool JointStatistics::onPostEvalSamples()
{
    if (!mJoinWith.empty())
...
void JointStatistics::onGetPlotProperties(PlotProperties &prop) const
{
    if (!mJoinWith.empty())
...
TransformedStatistics::TransformedStatistics(Context *p, const StatisticsObject *pCloneFrom, double d1, double d2)
    : StatisticsObject(p), mpCloneFrom(pCloneFrom), mTranslateBy(d1), mScaleBy(d2)
{
    mDepends.add(pCloneFrom);
}

bool TransformedStatistics::onPostEvalSamples()
{
...
void TransformedStatistics::onGetPlotProperties(PlotProperties &prop) const
{
    if (mpCloneFrom)
...
// Statistics
AnalysisDataObject *Statistics::create(AnalysisDataObject::Context *p, const ExpressionParser::NodeList &args)
{
    if (args.size() < 1)
...
Statistics::Statistics(Context *p, const SampleDataObject *pData) : StatisticsObject(p), mpData(pData)
{
    mDepends.add(pData);
    if (!mpData->valid())
        invalidate();
...
bool Statistics::onPostEvalSamples()
{
