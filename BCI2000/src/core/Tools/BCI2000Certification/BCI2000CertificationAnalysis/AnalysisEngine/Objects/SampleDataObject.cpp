////////////////////////////////////////////////////////////////////////////////
// $Id: SampleDataObject.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
SampleDataObject::SampleDataObject(Context *p) : AnalysisDataObject(p)
{
}

const std::vector<double> &SampleDataObject::data() const
{
    triggerEvaluation();
    return mData;
}

const PhysicalUnit &SampleDataObject::unit() const
{
    triggerEvaluation();
    return mUnit;
}

double SampleDataObject::eval() const
{
    return onEvalMemberFunc();
}

double SampleDataObject::onEvalMemberFunc() const
{
    throw bciexception << "don't know how to eval() an object of type " << type();
}

double SampleDataObject::normalized() const
{
...
double SampleDataObject::translated(double d) const
{
    auto p = new TransformedSampleData(context(), this);
    p->translate(d);
    return context()->store(p);
...
ExpressionParser::Node *SampleDataObject::onMemberFunction(Context *p, const std::string &function,
...
bool SampleDataObject::onGetPlotData(Dataplot::PointList &points) const
{
    const auto &v = data();
...
void SampleDataObject::onGetPlotProperties(PlotProperties &prop) const
{
    prop.with = "lines";
...
bool SampleDataObject::onCanExport(const std::string &) const
{
    return true;
}

bool SampleDataObject::onExport(const std::string &file) const
{
    std::ofstream f(file);
...
TransformedSampleData &TransformedSampleData::normalize()
{
    mNormalize = true;
    return *this;
}

TransformedSampleData &TransformedSampleData::translate(double d)
{
    mTranslateBy = d;
    return *this;
}

TransformedSampleData &TransformedSampleData::scale(double d)
{
    mScaleBy = d;
    return *this;
}

bool TransformedSampleData::onPostEvalSamples()
{
    std::string name;
...
void TransformedSampleData::onGetPlotProperties(PlotProperties &prop) const
{
    if (mpOriginal)
