////////////////////////////////////////////////////////////////////////////////
// $Id: Property.h 7536 2023-08-17 17:46:25Z mellinger $
...
#define PropertyMember(name, type)                                                                                     \
  private:                                                                                                             \
    static Property::Declaration *s##name##Declarator()                                                                \
    {                                                                                                                  \
        static Property::Declaration d = {#name, nullptr};                                                             \
        return &d;                                                                                                     \
    }                                                                                                                  \
                                                                                                                       \
  public:                                                                                                              \
    Property::MemberProperty<type, s##name##Declarator> name;
#define PropertyMembersEnd()                                                                                           \
  private:                                                                                                             \
    Property::EndOfProperties mEndOfProperties;
...
class BasicProperty
{
  ...
  public:
    ...
    BasicProperty &operator=(const BasicProperty &other)
    {
        return assign(other);
    }
    BasicProperty &operator=(const String &s)
    {
        return set(s);
    }
    bool operator==(const BasicProperty &other)
    {
        return equal(other);
    }
    bool operator!=(const BasicProperty &other)
...
template <class T> class IsEmptyAware
{
    typedef char one;
    typedef struct
    {
...
template <class T> struct PropertyType
{
    enum
    {
        value = std::is_pod<T>::value ? PODType : std::is_base_of < std::string,
        T > ::value ? StringType : std::is_base_of < BasicContainer,
        T > ::value ? ContainerType : IsEmptyAware<T>::value ? EmptyAwareType : PlainType
    };
};

template <class T, int = PropertyType<T>::value> class Property;

template <class T> class Property<T, PlainType> : public BasicProperty
{
  private:
    Property(const Property &);

  protected:
    Property(const Declaration *d) : BasicProperty(&mData, d)
    {
        onClear(&mData);
    }
    void clone(const Property &other)
    {
        onAssign(&mData, &other.mData);
        BasicProperty::clone(other);
    }

  public:
    Property &operator=(const Property &other)
    {
        setValue(other.mData);
        return *this;
    }
    Property &operator=(const BasicProperty &other)
    {
        assign(other);
        return *this;
    }
    ...
    {
        return onEqual(&mData, &other.mData);
    }
    bool operator!=(const Property &other) const
    {
        return !onEqual(&mData, &other.mData);
  ...
  protected:
    ...
    void onClear(void *p) override
    {
        mData = T();
    }
    bool onSet(const String &) override
    {
        return false;
    }
    bool onEqual(const void *p1, const void *p2) const override
    {
        return *static_cast<const T *>(p1) == *static_cast<const T *>(p2);
    }
    void onAssign(void *p1, const void *p2) const override
    {
        *static_cast<T *>(p1) = *static_cast<const T *>(p2);
    }
    void onSerialize(std::ostream &os, const void *p) const override
    {
        os << *static_cast<const T *>(p);
    }
    void *onUnserialize(std::istream &is) const override
    {
        T *p = new T;
        is >> *p;
        return p;
    }
    void onDispose(void *p) const override
    {
...
template <class T> class Property<T, PODType> : public Property<T, PlainType>
{
...
template <class T> class Property<T, EmptyAwareType> : public Property<T, PlainType>
{
    typedef Property<T, PlainType> Super;
  ...
  public:
    ...
    Property &setValue(const T &t)
    {
        Super::doSetValue(t.empty() ? nullptr : &t);
        return *this;
    }
  ...
  protected:
    void onClear(void *p) override
    {
        return static_cast<T *>(p)->clear();
    }
};

template <class T> class Property<T, StringType> : public Property<T, EmptyAwareType>
{
  ...
  protected:
    bool onSet(const String &s) override
    {
        Super::doSetValue(s.Empty() ? nullptr : &s);
        return true;
    }
};

template <class T> class Property<T, ContainerType> : public Property<T, EmptyAwareType>
{
    typedef Property<T, EmptyAwareType> Super;
...
template <class U> class Holder : public BasicHolder
{
  ...
  public:
    Holder(U *obj) : BasicHolder(), mClassInit(false)
    {
        onConstructionBegin(obj);
    }
  ...
  private:
    void setValidator(const Declaration *d, std::nullptr_t)
    {
        mValidators.erase(std::remove_if(mValidators.begin(), mValidators.end(),
                                         [d](std::pair<const Declaration *, Validator> &v) { return v.first == d; }),
                          mValidators.end());
    }

    template <class T> void setValidator(const Declaration *d, void (U::*validate)(const T *))
    {
        ...
        {
            auto i = std::find_if(mValidators.begin(), mValidators.end(),
                                  [d](std::pair<const Declaration *, Validator> &v) { return v.first == d; });
  ...
  public:
    ...
    template <class T, class V, Declarator D> void setValidator(MemberProperty<T, D> V::*, std::nullptr_t)
    {
        setValidator(D(), nullptr);
        BasicHolder::revalidate();
    }

    template <class T, Declarator D> void setValidator(MemberProperty<T, D> &prop, void (U::*validate)(const T *))
    {
        setValidator<T>(D(), validate);
        prop.revalidate();
    }
    template <class T, Declarator D> void setValidator(MemberProperty<T, D> &prop, std::nullptr_t)
    {
        setValidator(D(), nullptr);
        prop.revalidate();
    }
  ...
  protected:
    ...
    {
        int value = raw;
        if (sClassState.compare_exchange_strong(value, initializing))
        {
            sClassLock.lock();
            mClassInit = true;
            setHolderOffset(obj);
    ...
    const Proplist &onProperties() const override
    {
        return sProperties();
    }
  ...
  private: // per-class
    ...
    {
        static Proplist instance;
        SyncMemory<>();
        return instance;
    }
    static std::atomic<ptrdiff_t> sHolderOffset;
    ...
    static std::atomic<int> sClassState;
    static std::recursive_mutex sClassLock;
};
template <class U> std::recursive_mutex Holder<U>::sClassLock;
template <class U> std::atomic<int> Holder<U>::sClassState;
template <class U> std::atomic<ptrdiff_t> Holder<U>::sHolderOffset;
...
template <class T, Declarator D> class MemberProperty : public Property<T>
{
  public:
    ...
    MemberProperty(const MemberProperty &other) : Property<T>(D())
    {
        Property<T>::clone(other);
    }
    // assignment operator implicitly called when Holder is assigned from a Holder of same type:
    // and with same validators: keep property state, do not call any validators
    MemberProperty &operator=(const MemberProperty &other)
    {
        Property<T>::clone(other);
        return *this;
    }

    bool operator==(const MemberProperty &other) const
    {
...
class EndOfProperties
{
  public:
    EndOfProperties()
    {
        BasicHolder::constructionEnd();
    }
    EndOfProperties(const EndOfProperties &)
    {
        BasicHolder::constructionEnd();
    }
    EndOfProperties &operator=(const EndOfProperties &)
    {
        return *this;
    }
};
...
template <class T> class Container<T, true> : public BasicContainer
{
  public:
  ...
  private:
    ...
    void do_assign(const Container &c)
    {
        do_clear();
        for (auto p : c.mData)
        {
            mData.push_back(new T(*p));
            mData.back()->setParentProperty(this->parentProperty());
        }
    }
...
template <class T, bool H> std::ostream &operator<<(std::ostream &os, const Container<T, H> &c)
{
    return c.serialize(os);
}
template <class T, bool H> std::istream &operator>>(std::istream &is, Container<T, H> &c)
{
