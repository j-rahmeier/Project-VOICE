...
void P300_Classify(ap::real_2d_array pscore, ap::template_1d_array<unsigned short int, true> &StimulusCode,
                   ap::template_1d_array<unsigned short int, true> &StimulusType,
                   ap::template_1d_array<short int, true> &trialnr, int NumberOfSequences, int NumMatrixRows,
                   int NumMatrixColumns, const std::vector<std::string> &TargetDefinitions, std::vector<double> &result,
    ...
    for (int i = 0; i < numletters; i++)
    {
		...
		codes.clear();
#else // jm Oct 24, 2012 NOTE: This functions seems to invert the meaning of Rows and Columns
            {
                ...
                assert(codes.size() == 3);
                std::set<int>::const_iterator i = codes.begin();
                if (i != codes.end())
                    ++i;
                if (i != codes.end())
                    codecol.push_back(*i++);
    ...
    for (int j = 0; j < NumberOfSequences; j++)
    {
        ...
        for (int i = 0; i < numletters; i++)
        {
            if ((coderow[i] == predictedrow(i, j)) && codecol[i] == predictedcol(i, j))
                correct++;
            if (canpredict)
                predicted.push_back(TargetDefinitions[NumMatrixColumns * (predictedrow(i, j) - 1 - NumMatrixColumns) +
                                                      predictedcol(i, j) - 1]);
        }
        if (canpredict)
