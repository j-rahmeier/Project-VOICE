...
/// @param [in] StimulusOnset ste StimulusOnset.
                    ...
                    const ap::template_1d_array<double, true> &channels, const int MAfilter, const int DecFact)
{
    ...
    int bound_min, bound_max, row, col, row_windowlen, row_channels, numchannels, lenflash, sig_ds_len, siglen;
    double val_temp;
    ...
    ////////////////////////////////////////////////////////////////////////////
    // Section:  Identify changes in StimulusOnset
    for (int i = 1; i < lenflash; i++)
    {
        if (StimulusOnset(i - 1) == 0 && StimulusOnset(i) == 1)
        {
    ...
    for (int j = 0; j < static_cast<int>(tmp.size()); j++)
    {
        ...
        for (int i = 0; i < col; i++)
        {
            ...
            ap::vmove(sig.getrow(j, i * sig_ds_len, ((i + 1) * sig_ds_len) - 1),
                      y_downsampled.getvector(0, sig_ds_len - 1));
        }
