...
void StimulusPresentation_Classify(ap::real_2d_array pscore,
                                   ...
                                   ap::template_1d_array<short int, true> &trialnr, int NumberOfSequences,
                                   int NumberOfChoices, const std::vector<std::string> &TargetDefinitions,
                                   std::vector<double> &result, std::vector<std::string> &predicted)
{
    std::vector<int> range;
    std::vector<int> codes;
    std::vector<int> StimuliTarget;
    ...
    for (int i = 0; i < numletters; i++)
    {
        ...
        if (range.size() != 0)
        {
            ...
            it = std::unique(codes.begin(), codes.end());
