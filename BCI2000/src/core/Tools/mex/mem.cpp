///////////////////////////////////////////////////////////////////////////////
// $Id: mem.cpp 7659 2023-10-14 11:31:18Z mellinger $
...
void BCIMexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    if (PrintVersion(__FILE__, nrhs, prhs))
        return;

    const mxArray *inSignalArray = prhs[0], *inParmsArray = prhs[1];
    ...
    int numSamples = static_cast<int>(::mxGetM(inSignalArray)), numChannels = static_cast<int>(::mxGetN(inSignalArray));
    ...
    double *inSignal = ::mxGetPr(inSignalArray), *inParms = ::mxGetPr(inParmsArray), *p = inParms;
    double modelOrder = *p++, firstBinCenter = *p++, lastBinCenter = *p++, binWidth = *p++, evaluationsPerBin = *p++,
    ...
    if (windowLength * sampleBlockSize > numSamples)
        throw bciexception << "Window size may not exceed the number of samples.";
    ...
    filter.Start();
    int numBins = filter.Output().Elements(), samplesInWindow = windowLength * sampleBlockSize,
        numBlocks = (numSamples - samplesInWindow + sampleBlockSize) / sampleBlockSize;
    const mwSize dims[] = {static_cast<size_t>(numBins), static_cast<size_t>(numChannels),
