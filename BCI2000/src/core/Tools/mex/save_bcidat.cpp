///////////////////////////////////////////////////////////////////////////////
// $Id: save_bcidat.cpp 7659 2023-10-14 11:31:18Z mellinger $
...
struct StateInfo
{
    ...
    template <typename T> void CopyState(int64_t inBlockStart, StateVector &outStatevector) const
    {
        T *data = reinterpret_cast<T *>(mpData) + inBlockStart;
        for (int sample = 0; sample < outStatevector.Samples(); ++sample)
            outStatevector.SetSampleValue(mLocation, mLength, sample, static_cast<State::ValueType>(data[sample]));
    }
};

template <typename T>
void ReadSignal(void *inData, int64_t inNumSamples, int64_t inSampleOffset, GenericSignal &outSignal)
{
...
{ // FilterWrapper classes are friends to the Environment class.
  public:
    FilterWrapper() : mpOutputFormat(NULL)
    {
    }
    ~FilterWrapper()
    {
    ...
    template <class T> void Instantiate()
    {
    ...
    void StartRun(std::ostream &os, const std::string &inName)
    {
        mEnvironment.EnterPhase(Environment::startRun, &mParameters, &mStates);
        EnvironmentBase::ErrorContext("BeginFile", mpOutputFormat);
        mpOutputFormat->BeginFile(os, mParameters, mStates);
        EnvironmentBase::ErrorContext("");
        mEnvironment.EnterPhase(Environment::nonaccess);
    }

    void Write(std::ostream &os, const GenericSignal &inSignal, StateVector &inStatevector)
    {
    ...
    void StopRun(std::ostream &os)
    {
        mEnvironment.EnterPhase(Environment::stopRun, &mParameters, &mStates);
        EnvironmentBase::ErrorContext("EndFile", mpOutputFormat);
        mpOutputFormat->EndFile(os);
        EnvironmentBase::ErrorContext("");
        mEnvironment.EnterPhase(Environment::nonaccess);
    }
...
void BCIMexFunction(int nargout, mxArray *varargout[], int nargin, const mxArray *varargin[])
{
    ...
    default:
        throw bciexception << "Cannot handle signal data in " << mxGetClassName(pSignal) << " format.";
    }
    void *pSignalData = mxGetData(pSignal);
    SignalProperties properties;
    properties.SetChannels(dims[1]).SetType(type);

    if (nargin < 3 || !mxIsStruct(varargin[2]))
        throw bciexception << "No state information given. "
    ...
    for (int i = 0; i < numFields; ++i)
    {
        ...
        case mxSINGLE_CLASS:
            entry.mLength = 32;
            entry.mfpCopyState = &StateInfo::CopyState<float>;
            break;
    ...
    {
        sampleBlockSize = ::atoi(Params.ByPath("SampleBlockSize").Value().c_str());
    ...
    if (Params.Exists("SourceCh"))
    {
        ...
        Params.ByPath("SourceCh").Value() = oss.str();
    }
    else
    {
        std::ostringstream oss;
        oss << "save_bcidat int SourceCh= " << properties.Channels();
        Params.Add(oss.str());
    }
