////////////////////////////////////////////////////////////////////////////////
// $Id: gUSBampADC.cpp 8034 2024-04-16 18:49:38Z mellinger $
...
#define CHECK(x, y)                                                                                                    \
    {                                                                                                                  \
        if (::SetLastError(0), (x) && !(y))                                                                            \
        {                                                                                                              \
            x = false;                                                                                                 \
            bcierr << "line " << __LINE__ << ": " << GetErrorMessage();                                                \
        }                                                                                                              \
    }
...
static std::string GetErrorMessage()
{
    ...
    if (GT_GetLastError(&code, buf) && code != 0)
        return StringUtils::String() << "GT Error " << code << ": " << buf;
...
class IORequest
{
  public:
    ...
    IORequest(HANDLE hFile, size_t bytes, size_t buffers)
        : mFile(hFile), mBuffers(buffers), mWriteCursor(0), mReadCursor(0)
    {
...
struct gUSBampADC::Device
{
    ...
    std::vector<std::shared_ptr<IORequest>> mIORequests;
...
gUSBampADC::Device::Device() : mIsMaster(false), mHandle(0), mGTport(-1), mHWRev(unknown)
{
}
...
bool gUSBampADC::Device::Open(int port)
{
    Assert(mHandle == NULL);
...
void gUSBampADC::Device::AllocateIORequests(size_t numBytes, size_t numBuffers, size_t queueLength)
{
...
int gUSBampADC::Device::DigitalOutputs() const
{
    switch (mHWRev)
    {
    case Device::UA:
...
gUSBampADC::gUSBampADC()
    : mVis("ADCFILT"), m_filterhighpass(0), m_filterlowpass(0), m_notchhighpass(0), m_notchlowpass(0),
      m_filtermodelorder(0), m_filtertype(0), m_notchmodelorder(0), m_notchtype(0), m_blockduration_ms(0),
      m_digitalOutputMode(DigitalOutputMode::none), m_acqmode(normal), m_digitalOut1(false), mAcquireThread(this),
      mSourceBufferSemaphore(NULL), mWakeupEvent(::CreateEventA(nullptr, true, false, nullptr)), mReadCursor(0),
      mIOQueueLength(0)
{
}
...
void gUSBampADC::Publish()
{
  ...
  BEGIN_PARAMETER_DEFINITIONS
    ...
    "Source:gUSBamp:DigitalIO int EnableDigitalOutput= 0 0 0 3 "
        "// enable digital output on block acquisition: "
        " 0: off,"
        " 1: use expressions %28after signal acquisition%29,"
        " 2: timing test,"
        " 3: use expressions %28before signal acquisition%29"
        " (enumeration)",
...
// **************************************************************************
void gUSBampADC::AutoConfig(const SignalProperties &)
{
    mDevicesUsed.clear();
    ...
    std::sort(mDevicesDetected.begin(), mDevicesDetected.end(),
              [](const DevicePtr &a, const DevicePtr &b) { return a->Id() < b->Id(); });
    int i = 0;
    ...
    else if (ActualParameter("DeviceIDMaster") == "auto")
    {
        ...
        if (autoMaster == 0)
            ...
            bciwarn << "Both UA and UB devices present: Auto-configured " << mDevicesUsed[autoMaster]->Id()
                    << " as master";
    }

    int sourceCh = ActualParameter("SourceCh");
    ...
    for (int dev = 0; dev < actualSourceChDevices->NumValues(); ++dev)
    {
        ...
        {
            if (n >= sourceCh)
                throw bcierr << "Inconsistency between SourceCh and SourceChDevices";
...
// **************************************************************************
void gUSBampADC::Preflight(const SignalProperties &Input, SignalProperties &Output) const
{
    if (mDevicesDetected.empty())
    ...
    {
        goodSourceChGain =
            goodSourceChGain && (Parameter("SourceChGain")(ch).InMicrovolts() == 1 || Parameter("SourceChGain") == 1);
        goodSourceChOffset = goodSourceChOffset && (fabs(float(Parameter("SourceChOffset")(ch))) < 0.0001);
    }
    if (!goodSourceChGain)
        bcierr << "SourceChGain must be equal to 1muV for all channels (note that the int16 data type is no longer "
                  "supported)";
    if (!goodSourceChOffset)
        bcierr << "SourceChOffset is supposed to be zero";

    if (Parameter("DeviceIDs")->NumValues() != Parameter("SourceChDevices")->NumValues())
        throw bcierr << "# devices needs to equal # entries in SourceChDevices";
    ...
    if (Parameter("SourceCh") != totalnumchannels)
        throw bcierr << "SourceCh is " << Parameter("SourceCh")
                     << ", but needs to equal sum of all channels over all devices "
                     << (counterChannel ? "plus one counter channel " : "") << "(currently " << totalnumchannels << ")";
    ...
    for (int dev = 0; dev < Parameter("DeviceIDs")->NumValues(); dev++)
    {
        ...
        if (Parameter("EnableDigitalInput") == 1 && Parameter("SourceChDevices")(dev) < 1)
            throw bcierr << "Due to a limitation of the g.USBamp driver, at least one analog channel must be acquired "
    ...
    if (Parameter("SourceChList")->NumValues() > 0)
    {
        ...
        if (allChannels.size() < firstDevChannel)
            throw bcierr << "Duplicate entries found in SourceChList, use unique numbers between 1 and "
                         << 16 * numDevices;
    }
    ...
    for (const auto &d : mDevicesUsed)
    {
        ...
        { // let's check whether the driver complains if we use a wrong sampling rate
            ...
            {
                bcierr << "Could not set sampling rate on device " << d->Id() << " to " << samplerate << "Hz"
    ...
    if (Parameter("FilterEnabled") == 1)
        if (DetermineFilterNumber() == -1)
            bcierr << "Could not find appropriate pass band filter in gUSBamp. Use gUSBampgetinfo tool.";

    // check notch filter settings
    if (Parameter("NotchEnabled") == 1)
        if (DetermineNotchNumber() == -1)
            bcierr << "Could not find appropriate notch filter in gUSBamp. Use gUSBampgetinfo tool.";

    // Set digital output if necessary
    if ((int)Parameter("EnableDigitalOutput") == DigitalOutputMode::expressionsLate || (int)Parameter("EnableDigitalOutput") == DigitalOutputMode::expressionsEarly)
    {
        // Validate matrix
        ParamRef outputMat = Parameter("DigitalOutputEx");
        if (outputMat->NumColumns() < 1)
            bcierr << "Parameter DigitalOutputEx should have at least one column";
        if (outputMat->NumRows() > digitalOutputChannels)
            bcierr << "Parameter DigitalOutputEx has more rows than available digital "
                   << "output channels which is " << digitalOutputChannels << " channels";
        GenericSignal preflightSignal(Input);
        for (int r = 0; r < outputMat->NumRows(); r++)
        {
            // Validate label
            std::string label = outputMat->RowLabels()[r];
            int chan = stoi(label);
            if (chan < 1 || chan > digitalOutputChannels)
                bcierr << "Invalid DigitalOutput label " << label;
            // Validate expression
...
void gUSBampADC::Initialize(const SignalProperties &, const SignalProperties &output)
{
    ...
    bool commonref = true;
    if (Parameter("CommonReference") == 0)
        commonref = false;
    ...
    for (int dev = 0; dev < mDevicesUsed.size(); dev++)
    {
        ...
        if (mEnableDigitalInput)
            for (int i = 0; i < Device::DigitalInputs(); ++i)
                d->mDigInputStates.push_back(
                    State(StringUtils::String() << "DigitalInput" << d->mIndex * Device::DigitalInputs() + i + 1));

        // Setting up digital output
        if (m_digitalOutputMode == DigitalOutputMode::expressionsLate || m_digitalOutputMode == DigitalOutputMode::expressionsEarly)
        {
    ...
    for (auto &d : mDevicesUsed)
    {
        ...
        {
            CHECK(ok, GT_SetGround(
                          d->mHandle,
                          CommonGround)); // connect the grounds from all four blocks on each device to common ground
        ...
        // CHECK( ok, GT_EnableSC(d->mHandle, true) );  // with the short cut mode, a TTL pulse on the SC connector puts
        // the inputs on internal GND (we don't need this?)

        // here, we could check for whether or not the filter settings in the USBamp match what we want; if so; no need
        // to change this might take a long time in the current implementation, we set the same filter and notch on all
        // channels (otherwise, would be many parameters) because it takes so long, set filters only when they have been
        // changed
...
// **************************************************************************
// Function:   DoDigitalOutput
// Purpose:    This method, called by Process(), sets the digital output state
//              according to the compiled expressions of DigitalOutputEx
//
// Parameters: const pointer to signal
// Returns:    N/A
// **************************************************************************
void gUSBampADC::DoDigitalOutput(const GenericSignal * signal)
{
    for (size_t dev = 0; dev < mDevicesUsed.size(); dev++)
    {
        ...
        if (d->HWRev() == Device::UB)
        {
            ...
            {
                bool value = mDigOutputMatrix[d->mDigOutputBase + i].Evaluate(signal);
...
void gUSBampADC::Process(const GenericSignal &input, GenericSignal &signal)
{
    ...
    if (m_digitalOutputMode == DigitalOutputMode::expressionsEarly)
        DoDigitalOutput(NULL);
    ...
    // Digital Output
    if (m_digitalOutputMode == DigitalOutputMode::expressionsLate)
        DoDigitalOutput(&signal); // NB: use-cases in which DigitalOutputEx uses the current sample-block's raw signal are just-about imaginable, if unlikely. Alternatively we could pass NULL here, to make late evaluation perfectly consistent with early in prohibiting use of Signal() in the expression.
...
// Returns:    N/A
// **************************************************************************
int gUSBampADC::AcquireThread::OnExecute()
{
...
// **************************************************************************
bool gUSBampADC::EnumerateDevices(DeviceList &outDevices) const
{
    bool result = true;
...
int gUSBampADC::DetermineFilterNumber() const
{
    ...
    for (int no_filt = 0; no_filt < nof; no_filt++)
    {
        ...
        if ((fabs(filt[no_filt].fu - Parameter("FilterHighPass")) < 0.0001) &&
            (fabs(filt[no_filt].fo - Parameter("FilterLowPass")) < 0.0001) && (filt[no_filt].fs == samplingrate) &&
            (filt[no_filt].order == Parameter("FilterModelOrder")) && (filt[no_filt].type == Parameter("FilterType")))
...
int gUSBampADC::DetermineNotchNumber() const
{
    ...
    for (int no_filt = 0; no_filt < nof; no_filt++)
    {
        ...
        if ((fabs(filt[no_filt].fu - Parameter("NotchHighPass")) < 0.0001) &&
            (fabs(filt[no_filt].fo - Parameter("NotchLowPass")) < 0.0001) && (filt[no_filt].fs == samplingrate) &&
            (filt[no_filt].order == Parameter("NotchModelOrder")) && (filt[no_filt].type == Parameter("NotchType")))
            notchnumber = no_filt;
    }
    delete[] filt;
...
bool gUSBampADC::MeasureImpedance(const DevicePtr &d)
{
    bool ok = true;
    double impedance;
    ...
    for (int ch = 0; ch < d->mChannels.size(); ++ch)
    {
        ...
        memostream << " Ch " << std::right << std::setw(2) << (int)(d->mChannels[ch]);
        memostream << " (" << d->Id() << "/" << std::setw(2) << std::setfill('0') << std::right
                   << (int)(d->mChannels[ch]) << std::setfill(' ') << ")";
        memostream << " " << std::right << std::setw(3) << globalCh + 1;
        if (reportNames)
            memostream << std::left << std::setw(7) << (" (" + (std::string)Parameter("ChannelNames")(globalCh) + ")");
        memostream << ": " << std::right << std::setw(7) << std::fixed << std::setprecision(1) << (impedance / 1000.0)
                   << " kOhm\r";
...
void gUSBampADC::SetDigitalOut1(int value)
{
    DevicePtr& d = mDevicesUsed.back();
    if (d->HWRev() == Device::UB)
    {
        ...
        digOut.pin[0].doSet = TRUE;
        digOut.pin[0].value = value;
        if (!::GT_SetDigitalOutEx(d->mHandle, digOut.d))
            bciwarn << "Could not set digital output for device " << d->Id();
    }
    else if (d->HWRev() == Device::UA)
    {
        if (!::GT_SetDigitalOut(d->mHandle, 1, value))
            bciwarn << "Could not set digital output for device " << d->Id();
    }
}
