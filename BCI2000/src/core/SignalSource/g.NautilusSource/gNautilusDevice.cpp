////////////////////////////////////////////////////////////////////////////////
// $Id: gNautilusDevice.cpp 7463 2023-06-30 14:59:17Z mellinger $
...
void gNautilusDevice::Init(const GDS_ENDPOINT &host, const GDS_ENDPOINT &local, const std::string &deviceName)
{
    mpBuffers = NULL;
    mConfigured = false;
    mQueueIndex = 0;
    ...
    if (res.ErrorCode)
    {
        bcierr << "Error on GDS_Connect: " << res.ErrorMessage;
    }
    ...
    if (mDevice)
    {
        ...
        if (res.ErrorCode)
            bcierr << "Error on GDS_GetConfiguration: " << res.ErrorMessage;
        ...
        if (res.ErrorCode)
            bcierr << "Error on GDS_NAUTILUS_GetAvailableChannels: " << res.ErrorMessage;

        uint32_t mcount = 0;
        char(*electName)[GDS_GNAUTILUS_ELECTRODE_NAME_LENGTH_MAX] =
            new char[GDS_GNAUTILUS_CHANNELS_MAX][GDS_GNAUTILUS_ELECTRODE_NAME_LENGTH_MAX];
        ...
        if (res.ErrorCode)
            bcierr << "Error on GDS_NAUTILUS_GetChannelNames: " << res.ErrorMessage;
...
void gNautilusDevice::BeginAcquisition()
{
    ...
    if (wait_handle == NULL)
    {
        bcierr << "Error: CreateEvent failed with error code " << GetLastError();
    }
    mFirstData = true;
...
void gNautilusDevice::GetData(GenericSignal &Output)
{
    if (WaitForSingleObject(wait_handle, cTimeoutMs) != WAIT_OBJECT_0)
        bcierr << "Error: The data ready event hasn't been triggered within a reasonable time.";
    float *data = mpBuffers[mQueueIndex];
    for (int sample = 0; sample < mExpectedScans; sample++)
    {
        float *sampleData = &data[mChannelPoints * sample];
        // Analog Channels
        std::map<int, int>::iterator itr = mAnalogChannelMap.begin();
        if (mRefIdx == -1)
        ...
        for (; itr != mAccelDataChannelMap.end(); itr++)
            Output(itr->second, sample) =
                ((sampleData[itr->first] + (cAccelRange / 2)) / cAccelRange) * ((1 << NAUTILUS_ACCEL_BITS) - 1);
        ...
        for (; itr != mLinkQualityChannelMap.end(); itr++)
        {
            ...
            {
                bciout << "Link Quality fell below 10%";
                mLink10 = true;
            }
        }
        ...
        for (; itr != mBatteryChannelMap.end(); itr++)
        {
            ...
            {
                bciout << "Battery Level has fallen below 10%";
                mBattery10 = true;
            }
            else if (!mBattery5 && sampleData[itr->first] <= 5.0)
            {
                bciout << "Battery Level has fallen below 5%";
                mBattery5 = true;
            }
        }
...
bool gNautilusDevice::MapAnalogChannel(unsigned int devicech, unsigned int sourcech, bool err)
{
    ...
    {
        if (err)
            bcierr << "Requested channel " << (devicech + 1) << "from g.Nautilus which only has " << cMaxAnalogChannels
                   << " channels.";
        return false;
    }
...
bool gNautilusDevice::MapDigitalChannel(unsigned int devicech, unsigned int sourcech)
{
    ...
    {
        bcierr << "Requested digital channel " << (devicech + 1) << "from g.Nautilus which only has "
               << cMaxDigitalChannels << " digital channels.";
        return false;
    }
            ...
            if (connectedDevices[i].ConnectedDevices[j].DeviceType == GDS_DEVICE_TYPE_GNAUTILUS)
            {
                if (connectedDevices[i].InUse == FALSE)
                {
                    ...
                    if (device.IsOpen())
                    {
                        ...
                        bcidbg(0) << "Connected to device " << connectedDevices[i].ConnectedDevices[j].Name;
                    }
                }
            }
        }
