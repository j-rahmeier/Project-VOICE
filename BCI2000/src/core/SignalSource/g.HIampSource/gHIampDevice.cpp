////////////////////////////////////////////////////////////////////////////////
// $Id: gHIampDevice.cpp 7645 2023-10-05 17:37:31Z mellinger $
...
#define MUST_SUCCEED(x)                                                                                                \
    if (!(x))                                                                                                          \
        throw bcierr << StringUtils::ReplaceAll(#x, "mDevice", mId) << ": " << GetErrorMessage();
...
static std::string PrintFilter(const FILT &f)
{
...
gHIampDevice::Config &gHIampDevice::Config::operator=(const Config &other)
{
...
bool gHIampDevice::Config::operator==(const Config &other) const
{
...
void gHIampDevice::DetectDevices(List &ioList, int inMaxEnumeratorIndex)
{
...
gHIampDevice::gHIampDevice()
    : mHWVersion(NaN(mHWVersion)), mTag(-1), mFirstRequest(false), mCounter(0), mExpectedBytes(0),
      mValuesPerScan(0), mBlockDuration(0), mDevice(NULL)
{
...
gHIampDevice &gHIampDevice::SetId(const std::string &id)
{
    Assert(!IsOpen());
    mId = id;
    return *this;
...
bool gHIampDevice::Open(int inEnumeratorIndex)
{
...
gHIampDevice::TranslateConfiguration(const Config &inConfig) const
{
    ...
    if (inConfig.Notch > 0)
    {
        ...
        {
            bcierr << "No appropriate notch filter could be found, closest is " << PrintFilter(sNotchFilters[notchIdx]);
            notchIdx = -1;
        }
    }
    int bandIdx = -1;
    if (inConfig.HPCorner > 0 || inConfig.LPCorner > 0)
    {
        ...
        for (int i = 0; i < sBandpassFilters.size(); ++i)
        {
            ...
            if (f.fs == inConfig.SamplingRate)
            {
                float hpDist = ::fabs(f.fu - inConfig.HPCorner), lpDist = ::fabs(f.fo - inConfig.LPCorner),
...
gHIampDevice &gHIampDevice::SetConfiguration(const Config &inConfig)
{
    ...
    ::memset(mImpedanceAcquisition.mData.IsActiveElectrode, mConfig.ActiveElectrodes ? 0xff : 0,
...
const gHIampDevice &gHIampDevice::VerifyConfiguration(const Config &inConfig) const
{
    auto config = TranslateConfiguration(inConfig);
    static const struct
    {
        int rate, blockMin, blockMax, chanMax;
    } supported[] = {
        // g.HIamp C API V2.14.03 documentation
        {256, 1, 256, 256},   {512, 2, 256, 256},    {600, 4, 256, 256},    {1200, 8, 256, 256},   {2400, 16, 256, 256},
        {4800, 32, 256, 256}, {9600, 128, 256, 144}, {19200, 256, 256, 80}, {38400, 256, 256, 40},
    };
    bool found = false;
    for (int i = 0; i < sizeof(supported) / sizeof(*supported); ++i)
    {
        ...
        if (inConfig.SamplingRate == s.rate)
        {
            ...
            if (config.BufferSize < s.blockMin || config.BufferSize > s.blockMax)
                bcierr << "Unsupported combination of SamplingRate (" << inConfig.SamplingRate
            ...
            if (inConfig.AnalogChannels.size() > s.chanMax)
                bcierr << "A maximum of " << s.chanMax << " channels may be recorded at a sampling rate of " << s.rate
                       << "Hz";
...
std::string gHIampDevice::GetErrorMessage()
{
    WORD errorCode = 0;
...
bool gHIampDevice::OnQueueRequest(IORequest &request)
{
...
bool gHIampDevice::OnRequestCompletion(IORequest &request)
{
    auto ov = static_cast<OVERLAPPED *>(request.pRequestObject);
    DWORD bytesRead = 0;
...
void gHIampDevice::MoveData(const IORequest &inRequest, GenericSignal &Output)
{
    union {
        void *p;
        float *f;
        uint32_t *i;
    } data = {inRequest.pBuffer};
    ...
    {
        float ref = (mConfig.RefChannel < 0) ? 0.0 : data.f[mConfig.RefChannel];
        if (mConfig.CounterChannel >= 0)
        ...
        // This is wrong. Unlike the sample counter, digital inputs are transmitted
...
void gHIampDevice::StartAcquisition()
{
    mFirstRequest = true;
...
void gHIampDevice::StopAcquisition()
{
...
void gHIampDevice::StartImpedanceAcquisition()
{
    mImpedanceAcquisition.mpSelf = this;
    mImpedanceAcquisition.Start();
}

void gHIampDevice::StopImpedanceAcquisition()
{
    mImpedanceAcquisition.Terminate();
    Assert(!mImpedanceAcquisition.Running());
}

void gHIampDevice::MoveImpedanceData(GenericSignal &Output)
{
    WithLock(mImpedanceAcquisition) for (int sample = 0; sample < Output.Elements();
                                         ++sample) for (auto pair : mConfig.AnalogChannels) if (pair.second >= 0)
        Output(pair.second, sample) = mImpedanceAcquisition.mData.Impedance[pair.first];
}

int gHIampDevice::ImpedanceAcquisition::OnExecute()
{
    ...
    while (!Terminating())
    {
        if (GT_GetImpedance(mpSelf->mDevice, &data))
            WithLock(this)::memcpy(mData.Impedance, data.Impedance, sizeof(data.Impedance));
        else
            WithLock(this)::memset(mData.Impedance, 0xff, sizeof(mData.Impedance));
    }
    return 0;
}
