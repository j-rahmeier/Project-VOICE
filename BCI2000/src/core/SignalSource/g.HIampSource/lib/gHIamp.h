...
//    Copyright (c) 2010-2014 Guger Technologies.
//_____________________________________________________________________________________
...
#pragma pack(1)
    typedef struct _DAC
    {
        ...
        _DAC()
            : Amplitude(TESTSIGNAL_AMPLITUDE), Offset(TESTSIGNAL_OFFSET), Enabled(FALSE), WaveShape(WS_SQUARE),
              Frequency(0)
        {
        }
    } DAC, *PDAC;
...
#pragma pack(1)
    ...
    GHIAMP_API BOOL __stdcall GT_GetOverlappedResult(HANDLE hDevice, LPOVERLAPPED lpOverlapped,
    ...
    GHIAMP_API BOOL __stdcall GT_GetAvailableChannels(HANDLE hDevice, UCHAR *availableChannels,
                                                      WORD availableChannelsLength);
    ...
    GHIAMP_API BOOL __stdcall GT_GetNumberOfSupportedBufferSizes(HANDLE hDevice, float samplingRate,
                                                                 int *numberOfSupportedBufferSizes);
    GHIAMP_API BOOL __stdcall GT_GetSupportedBufferSizes(HANDLE hDevice, float samplingRate,
                                                         WORD *supportedBufferSizes);
