////////////////////////////////////////////////////////////////////////////////
// $Id: gHIampADC.cpp 7645 2023-10-05 17:37:31Z mellinger $
...
gHIampADC::gHIampADC() : p(new Private)
{
...
void gHIampADC::OnPublish()
{
  BEGIN_PARAMETER_DEFINITIONS
    ...
    "Source:Acquisition int IOQueueSize= auto auto 2 % // number of transfer requests to keep posted during "
        "acquisition",
    "Source:Buffering int SourceBufferSize= 2s 2s % % // size of data acquisition ring buffer (in blocks or "
        "seconds)",

    "Source:Multiple%20Devices list DeviceIDs= 1 auto // list of devices to be used",
    "Source:Multiple%20Devices string DeviceIDMaster= auto // deviceID for the device whose SYNC goes to the "
        "slaves",
    "Source:Multiple%20Devices intlist SourceChList= 1 auto // list of channels to digitize, in terms of physical "
        "inputs numbered across all devices",
...
#if BCIDEBUG
    "Source:Debugging int AlwaysReconfigure= 0 0 0 1 // reconfigure amplifier at SetConfig even if configuration "
        "unchanged (boolean)",
#endif
...
void gHIampADC::OnPreflight(SignalProperties &Output) const
{
    if (p->mDevicesDetected.empty())
...
void gHIampADC::OnInitialize(const SignalProperties &Output)
{
    p->mDevicesUsed.clear();
    auto configs = p->BuildAmplifierConfigurations();
...
void gHIampADC::OnStartAcquisition()
{
    if (p->mAcquisitionMode == impedance)
    {
...
void gHIampADC::OnStopAcquisition()
{
    if (p->mAcquisitionMode == impedance)
        for (auto &d : p->mDevicesUsed)
...
void gHIampADC::DoAcquire(GenericSignal &Output)
{
    if (p->mAcquisitionMode == impedance)
    {
...
DeviceConfigs gHIampADC::Private::BuildAmplifierConfigurations() const
{
