////////////////////////////////////////////////////////////////////////////////
// $Id: FeedbackScene2D.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
FeedbackScene2D::FeedbackScene2D(GUI::DisplayWindow &inDisplay)
    : mDisplay(inDisplay), mScalingX(1.0), mScalingY(1.0), mpCursor(NULL), mpBoundary(NULL)
{
}
...
void FeedbackScene2D::OnInitialize()
{
    ClearObjects();

    float width = mDisplay.Width(), height = mDisplay.Height();
    if (width > height)
    {
    ...
    if (boundaryColor != RGBColor(RGBColor::NullColor))
    {
        ...
        mpBoundary->SetColor(boundaryColor).SetFillColor(RGBColor::NullColor);
...
float FeedbackScene2D::CursorRadius() const
{
    GUI::Rect r = mpCursor->ObjectRect();
    GUI::Point p = {r.right - r.left, r.bottom - r.top};
    ObjectToSceneCoords(p, vector);
    return std::max(fabs(p.x), fabs(p.y)) / 2.;
}

void FeedbackScene2D::OnSetCursorPosition(const Vector3D &inPos)
{
...
void FeedbackScene2D::SetCursorVisible(bool inVisible)
{
    inVisible ? mpCursor->Show() : mpCursor->Hide();
}

void FeedbackScene2D::SetCursorColor(RGBColor inColor)
{
    mpCursor->SetColor(inColor).SetFillColor(inColor);
}

int FeedbackScene2D::NumTargets() const
{
    return static_cast<int>(mTargets.size());
}

bool FeedbackScene2D::TargetHit(int inIdx) const
{
    return Shape::AreaIntersection(*mpCursor, *mTargets.at(inIdx));
}

float FeedbackScene2D::CursorTargetDistance(int inIdx) const
{
    GUI::Point targetCenter = mTargets.at(inIdx)->Center(), cursorCenter = mpCursor->Center();
...
void FeedbackScene2D::SetTargetVisible(bool inVisible, int inIdx)
{
    inVisible ? mTargets.at(inIdx)->Show() : mTargets.at(inIdx)->Hide();
}

void FeedbackScene2D::SetTargetColor(RGBColor inColor, int inIdx)
{
    mTargets.at(inIdx)->SetColor(inColor).SetFillColor(inColor);
}

void FeedbackScene2D::ClearObjects()
{
    delete mpBoundary;
    mpBoundary = NULL;
    delete mpCursor;
...
void FeedbackScene2D::SceneToObjectCoords(GUI::Point &ioPoint, int inKind) const
{
    switch (inKind)
    {
    case vector:
...
void FeedbackScene2D::ObjectToSceneCoords(GUI::Point &ioPoint, int inKind) const
{
    switch (inKind)
    {
    case vector:
