////////////////////////////////////////////////////////////////////////////////
// $Id: FeedbackDemoTask.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
FeedbackDemoTask::FeedbackDemoTask()
    : mRunCount(0), mTrialCount(0), mCursorPosX(0.0), mCursorPosY(0.0), mCursorSpeedX(0.0), mCursorSpeedY(0.0),
      mpWindow(NULL), mpScene(NULL), mpSceneView(NULL), mpLabel(NULL), mpTarget(NULL), mpCursor(NULL)
{
...
void FeedbackDemoTask::OnPreflight(const SignalProperties &Input) const
{
    ...
    if (RGBColor(Parameter("CursorColor")) == RGBColor(RGBColor::NullColor))
        bcierr << "Invalid RGB value in CursorColor";

    if (Parameter("FeedbackDuration").InSampleBlocks() <= 0)
        bcierr << "FeedbackDuration must be greater 0";

    if (Input.IsEmpty())
        bcierr << "Requires at least one entry in control signal";

    if (Input.Channels() > 1)
        bciout << "Will ignore additional channels in control signal";
}

void FeedbackDemoTask::OnInitialize(const SignalProperties & /*Input*/)
{
...
void FeedbackDemoTask::OnTrialBegin()
{
    ++mTrialCount;
    ...
    if (State("TargetCode") > 0)
    {
        ...
        QRectF newTargetRect(targetRect.left(), targetRect.height() * (State("TargetCode") - 1), targetRect.width(),
                             targetRect.height());
...
void FeedbackDemoTask::OnTrialEnd()
{
...
void FeedbackDemoTask::OnFeedbackBegin()
{
    mCursorPosX = 0.0;
...
void FeedbackDemoTask::OnFeedbackEnd()
{
...
void FeedbackDemoTask::DoPreRun(const GenericSignal &, bool & /*doProgress*/)
{
}

void FeedbackDemoTask::DoPreFeedback(const GenericSignal &, bool & /*doProgress*/)
{
}
...
void FeedbackDemoTask::DoPostFeedback(const GenericSignal &, bool & /*doProgress*/)
{
}

void FeedbackDemoTask::DoITI(const GenericSignal &, bool & /*doProgress*/)
{
}

void FeedbackDemoTask::SetLabel(const char *text, RGBColor &color)
{
    QFont labelFont;
    ...
    mpLabel->setPos((mpWindow->width() / 2) - (fm.boundingRect(text).width() / 2), (mpWindow->height() / 2) - (fm.height() / 2));
